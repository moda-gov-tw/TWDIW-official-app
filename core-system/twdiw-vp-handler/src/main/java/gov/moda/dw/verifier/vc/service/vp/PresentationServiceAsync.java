...
public class PresentationServiceAsync {
    ...
    try {
      if (presentationList == null || presentationList.isEmpty()) {
        throw new VpException(
            VpException.ERR_PRES_INVALID_PRESENTATION_VALIDATION_REQUEST,
            "invalid presentation validation request");
      }
      LOGGER.info("vp number = {}", presentationList.size());
      List<PresentationValidationResponseDTO> validationResponseList =
          validateVPs(presentationList);
      ...
      VpException vpException =
          new VpException(VpException.ERR_PRES_VALIDATE_VP_ERROR, "fail to validate vp");
  ...
  private List<PresentationValidationResponseDTO> validateVPs(List<String> presentationList)
    ...
    for (int vpIndex = 0; vpIndex < presentationList.size(); vpIndex++) {
      ...
      PresentationValidationResponseDTO presentationValidationResponseDTO =
          validateVP(presentation, vpIndexInfo);
  ...
  private PresentationValidationResponseDTO validateVP(String presentation, VpIndexInfo vpIndexInfo)
    ...
    JwtVerifiablePresentation jwtVp =
        JwtVerifiablePresentation.fromCompactSerialization(presentation);
    PresentationValidateTask presentationValidateTask = new PresentationValidateTask();

    // step 1: validate vp content
    PresentationValidateTask.ContentResult vpContentResult =
        presentationValidateTask.validateJwtContent(jwtVp);
    ...
    if (!vpContentOK || vcList == null || vcList.isEmpty()) {
      throw new VpException(
          VpException.ERR_PRES_VALIDATE_VP_CONTENT_ERROR, "fail to validate vp content");
    }
    ...
    if (holderPublicKeySet.size() != 1) {
      throw new VpException(
          VpException.ERR_PRES_HOLDER_PUBLIC_KEY_INCONSISTENT,
          "holder public key is inconsistent in vc");
    }
    ...
    if (!vpProofOK) {
      throw new VpException(
          VpException.ERR_PRES_VALIDATE_VP_PROOF_ERROR, "fail to validate vp proof");
    }
  ...
  private List<VcData> validateVCsAsync(List<String> vcList, VpIndexInfo vpIndexInfo)
      throws VpException {
    ...
    for (int i = 0; i < vcList.size(); i++) {
      ...
      int vcIndex = i;
      futureTasks.add(
          futureTaskService.doFutureTask(
              () -> {
  ...
  private FutureTaskResult<VcData> doValidateVC(
      String credential, VpIndexInfo vpIndexInfo, int vcIndex) {
    try {
  ...
  public VcData validateVC(String credential, VpIndexInfo vpIndexInfo, int vcIndex)
    ...
    if (iss == null || iss.isBlank()) {
      throw new VpException(
          VpException.ERR_CRED_INVALID_ISSUER_DID_FORMAT, "issuer's DID is null or empty");
    }

    CredentialValidateTask credentialValidateTask = new CredentialValidateTask();

    // step 2: validate vc content
    CredentialValidateTask.ContentResult vcContentResult =
        credentialValidateTask.validateJwtContent(jwtVc, disclosureList);
    boolean vcContentOK = vcContentResult.isContentOK();
    VerifiableCredential vc = vcContentResult.getVc();
    if (!vcContentOK || vc == null) {
      throw new VpException(
          VpException.ERR_CRED_VALIDATE_VC_CONTENT_ERROR, "fail to validate vc content");
    }
    String sub = jwtClaimsSet.getSubject();

    List<CompletableFuture<? extends FutureTaskResult<?>>> futureTasks = new ArrayList<>();
    List<CompletableFuture<FutureTaskResult<StatusListCheckResult>>> statusListCheckTasks =
        new ArrayList<>();

    // step 3: validate vc status
    for (StatusListInfo statusListInfo : vcContentResult.getStatusListInfo()) {
      CompletableFuture<FutureTaskResult<StatusListCheckResult>> validateVcStatus =
          futureTaskService.validateVcStatus(statusListInfo);
      statusListCheckTasks.add(validateVcStatus);
    }

    // step 4: validate issuer's DID status
    CompletableFuture<FutureTaskResult<Integer>> validateDidStatus =
        futureTaskService.validateDidStatus(iss);
    ...
    CompletableFuture<FutureTaskResult<String>> validateVcSchema =
        futureTaskService.validateVcSchema(schemaUrl, claims);
    futureTasks.add(validateVcSchema);

    // step 6: validate vc proof
    CompletableFuture<FutureTaskResult<Map<String, Object>>> validateVcProof =
        futureTaskService.validateVcProof(
            jwtVc, iss, vcContentResult.getJku(), vcContentResult.getKid());
    ...
    if (checkFutureTaskValidateResult(futureTasks)) {
      LOGGER.info(
          "vp[{}] -> vc[{}]: vc validate tasks are completed", vpIndexInfo.vpIndex(), vcIndex);
    }

    return new VcData()
        .setVpPath(getVpPath(vpIndexInfo))
  ...
  private List<VcData> getVcDataFromFutureTask(
      List<CompletableFuture<FutureTaskResult<VcData>>> completableFutures) throws VpException {
  ...
  private boolean checkFutureTaskValidateResult(
      List<CompletableFuture<? extends FutureTaskResult<?>>> completableFutures)
      throws VpException {
    CompletableFuture.allOf(completableFutures.toArray(new CompletableFuture[0])).join();
  ...
  private boolean resolveStatusListCheckResult(
      List<CompletableFuture<FutureTaskResult<StatusListCheckResult>>> results) throws VpException {
  ...
  private String getVcLocation(VpIndexInfo vpIndexInfo, int vcIndex) {
    return "("
        + "vp_path="
        + (vpIndexInfo.isArray() ? "$[" + vpIndexInfo.vpIndex() + "]" : "$")
        + ","
        + "vc_path="
  ...
  private String getSub(List<VcData> vcData) throws VpException {
    ...
    if (subList.isEmpty()) {
      ...
      throw new VpException(
          VpException.ERR_PRES_HOLDER_PUBLIC_KEY_INCONSISTENT, "vc sub is inconsistent");
    }
