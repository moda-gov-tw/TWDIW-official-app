...
public class ResourceLoadService {
  ...
  public Map<String, Object> loadIssuerPublicKey(String jku, String kid) throws VpException {
    if (jku == null || jku.isBlank() || kid == null || kid.isBlank()) {
      throw new VpException(
          VpException.ERR_CRED_LACK_OF_ISSUER_PUBLIC_KEY, "public key information not found in vc");
    }

    LOGGER.info("(jku, kid) = ({}, {})", jku, kid);

    // step 2: connect to issuer
    HttpUtils.Builder builder =
    ...
    if (preloadSetting.isHttpProxyEnable()) {
      builder
          .setUseProxy(preloadSetting.isHttpProxyEnable())
    ...
    if (statusCode != HttpStatus.OK.value() || content == null || content.isBlank()) {
      String errorMessage =
          "fail to load issuer's public key. (http status code, response content) = ("
              + statusCode
              + ", "
              + content
              + ")";
    ...
    if (keys == null || keys.isEmpty()) {
      throw new VpException(
          VpException.ERR_CONN_INVALID_ISSUER_PUBLIC_KEY, "invalid issuer's public key");
    }

    // find the matched key by kid
    Map<String, Object> publicKey =
        keys.stream().filter(m -> kid.equals(m.get("kid"))).findFirst().orElse(null);

    if (publicKey == null || publicKey.isEmpty()) {
      throw new VpException(
          VpException.ERR_CONN_NO_MATCHED_ISSUER_PUBLIC_KEY, "no matched issuer's public key");
    }
  ...
  public String loadStatusList(String statusListUrl) throws VpException {
    ...
    if (statusCode != HttpStatus.OK.value() || content == null || content.isBlank()) {
      String errorMessage =
          "fail to load issuer's status list. (http status code, response content) = ("
              + statusCode
              + ", "
              + content
              + ")";
    ...
    String statusList =
        Optional.of(statusListResponseDTO).map(StatusListResponseDTO::getStatusList).orElse(null);

    if (statusList == null || statusList.isBlank()) {
      throw new VpException(
          VpException.ERR_CONN_INVALID_ISSUER_STATUS_LIST, "invalid issuer's status list");
    }
  ...
  public String loadSchema(String schemaUrl) throws VpException {
    ...
    if (statusCode != HttpStatus.OK.value() || content == null || content.isBlank()) {
      String errorMessage =
          "fail to load issuer's schema. (http status code, response content) = ("
              + statusCode
              + ", "
              + content
              + ")";
  ...
  public int callFrontendToQueryDid(String did) throws VpException {
    ...
    if (statusCode != HttpStatus.OK.value() || content == null || content.isBlank()) {
      String errorMessage =
          "fail to query DID. (http status code, response content) = ("
              + statusCode
              + ", "
              + content
              + ")";
