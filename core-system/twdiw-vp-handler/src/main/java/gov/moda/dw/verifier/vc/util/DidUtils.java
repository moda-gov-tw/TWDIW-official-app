  ...
  public static Map<String, Object> extractIssuerPublicKey(String did) throws VpException {
    ...
    if (parts.length != 3) {
      throw new VpException(
          VpException.ERR_CRED_INVALID_ISSUER_DID_FORMAT,
          "the format of issuer's DID DOES NOT match did:key:xxxxx");
    }
    ...
    if (multibase == null || multibase.isBlank() || !multibase.startsWith("z")) {
      throw new VpException(
          VpException.ERR_CRED_INVALID_ISSUER_DID_FORMAT,
          "the multibase value of issuer's DID DOES NOT start with `z`");
    }
    ...
    try {
      ...
      if (mbBytes.length == 0 || !Arrays.equals(Arrays.copyOfRange(mbBytes, 0, 3), prefix)) {
        throw new VpException(
            VpException.ERR_CRED_INVALID_ISSUER_DID_FORMAT,
            "fail to decode issuer's DID (invalid hex data)");
      }
      ...
      Map<String, Object> publicKey = Optional.of(jwk).map(JsonUtils::jsToMap).orElse(null);

      if (publicKey == null || publicKey.isEmpty()) {
        throw new VpException(
            VpException.ERR_CRED_INVALID_ISSUER_DID_FORMAT, "fail to decode issuer's DID");
      }
