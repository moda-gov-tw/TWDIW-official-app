  ...
  public ContentResult validateJwtContent(JwtVerifiableCredential jwtVc) throws VpException {
    ...
    try {
      ...
      try {
        ...
        if (map != null && !map.isEmpty()) {
          ...
          Map<String, Object> claims =
              (vc.getCredentialSubject() != null) ? vc.getCredentialSubject().getClaims() : null;
          encodedList =
          ...
          if (encodedList != null && !encodedList.isEmpty()) {
            ...
            LOGGER.info(
                "[check status list encoded list]: FAIL -> invalid status list encoded list");
            contentOK = false;
          }
      ...
      throw new VpException(
          VpException.ERR_SL_VALIDATE_STATUS_LIST_CONTENT_ERROR,
          "fail to validate status list content");
    }
  }
  ...
  public boolean validateJwtProof(
    ...
    try {
      if (issuerPublicKey == null || issuerPublicKey.isEmpty()) {
        throw new VpException(
            VpException.ERR_SL_LACK_OF_ISSUER_PUBLIC_KEY, "lack of issuer's public key");
      }
      ...
      LOGGER.error(e.getMessage(), e);
      throw new VpException(
          VpException.ERR_SL_VALIDATE_STATUS_LIST_PROOF_ERROR,
          "fail to validate status list proof");
    }
