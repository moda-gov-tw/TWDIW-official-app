...
public class HttpUtils {
  ...
  public Tuple.Pair<Integer, String> postText(String text) {
    return post(new StringEntity(text, StandardCharsets.UTF_8), ContentType.TEXT_PLAIN.toString());
  }
  ...
  public Tuple.Pair<Integer, String> postForm(List<NameValuePair> form) {
    return post(
        new UrlEncodedFormEntity(form, StandardCharsets.UTF_8),
  ...
  private Tuple.Pair<Integer, String> post(HttpEntity entity, String contentType) {
    ...
    try {
      ...
      if (enableLog) {
        // LOGGER.info("HTTP REQ " + symbol + httpPost.getRequestLine() + "HTTP REQ " + symbol +
