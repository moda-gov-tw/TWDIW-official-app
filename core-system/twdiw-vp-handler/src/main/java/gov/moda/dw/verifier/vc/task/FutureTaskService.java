...
public class FutureTaskService {
  ...
  public <T> CompletableFuture<FutureTaskResult<T>> doFutureTask(
      Supplier<FutureTaskResult<T>> supplier, String errorMessage) {
  ...
  public CompletableFuture<FutureTaskResult<Integer>> getDidStatus(String issuer) {
    return CompletableFuture.supplyAsync(
            ...
            vcResourceExecutor)
        .exceptionally(ex -> exceptionHandler("fail to get did status", ex));
  }

  public CompletableFuture<FutureTaskResult<String>> validateVcSchema(
      String schemaUrl, Map<String, Object> claims) {
    return getVcSchema(schemaUrl)
        .thenApply(
            result -> {
              if (result.isSuccess()) {
  ...
  public CompletableFuture<FutureTaskResult<String>> getVcSchema(String schemaUrl) {
    return CompletableFuture.supplyAsync(
            ...
            vcResourceExecutor)
        .exceptionally(ex -> exceptionHandler("fail to get vc schema", ex));
  }

  public CompletableFuture<FutureTaskResult<Map<String, Object>>> validateVcProof(
      JwtVerifiableCredential jwtVc, String issuer, String jku, String kid) {
    return getIssuerPublicKey(issuer, jku, kid)
        .thenApply(
            result -> {
              if (result.isSuccess()) {
  ...
  public CompletableFuture<FutureTaskResult<Map<String, Object>>> getIssuerPublicKey(
      String issuer, String jku, String kid) {
    return CompletableFuture.supplyAsync(
            () -> {
              try {
                LOGGER.debug("load IssuerPublicKey");
                Map<String, Object> issuerPublicKey = DidUtils.extractIssuerPublicKey(issuer);
                return new FutureTaskResult<Map<String, Object>>(true)
                    .setResultData(issuerPublicKey);
              } catch (VpException e) {
                try {
                  Map<String, Object> issuerPublicKey =
                      resourceLoadService.loadIssuerPublicKey(jku, kid);
                  return new FutureTaskResult<Map<String, Object>>(true)
                      .setResultData(issuerPublicKey);
            ...
            vcResourceExecutor)
        .exceptionally(ex -> exceptionHandler("fail to get issuerPublicKey", ex));
  }

  public CompletableFuture<FutureTaskResult<StatusListCheckResult>> validateVcStatus(
      StatusListInfo statusListInfo) {
    return getStatusList(statusListInfo.statusListCredentialURL())
        .thenApply(
            result -> {
              ...
              if (result.isSuccess()) {
                try {
                  ...
                  StatusListCheckResult statusListCheckResult =
                      statusListCheckTask.validate(statusListIndex, statusList);
                  return new FutureTaskResult<StatusListCheckResult>(true)
                      .setResultData(statusListCheckResult);
                } catch (VpException e) {
  ...
  public CompletableFuture<FutureTaskResult<String>> getStatusList(String statusListUrl) {
    return CompletableFuture.supplyAsync(
            ...
            vcResourceExecutor)
        .exceptionally(ex -> exceptionHandler("fail to get statusList", ex));
  }
