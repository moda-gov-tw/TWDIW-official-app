...
export const useRoleStore = defineStore("role", () => {
  ...
  function getRoles() {
    ...
    api
      ...
      .then((response) => {
        ...
        if (code !== "0" || !data) {
          $n.error(response?.data?.msg || t("role.error.unknownError"));
        }
      ...
      .catch((error) => {
        console.error(`${t("role.error.fetchRoleListError")}：${error}`);
      });
  }
    ...
    return api
      ...
      .then((response) => {
        if (response && response.data && response.data.code === "0") {
          ...
          const errMsg = response?.data?.msg ?? t("role.error.unknownError");
          loading.value = false;
  ...
  function editRole(role) {
    ...
    if (
      ...
      const warningMessage = t("role.error.cannotDisable");
    ...
    return api
      ...
      .then((response) => {
        if (
          ...
          const message = t("role.success.edit", { name });
          $n.success(message);
  ...
  async function deleteRole(role) {
    ...
    if (
      ...
      const warningMessage = t("role.error.cannotDelete");
      $n.warn(warningMessage);
      loading.value = false;
      return Promise.reject(new Error(warningMessage));
    }
    ...
    try {
      ...
      if (response && response.data && response.data.code === "0") {
        clearSelectedRole();
        $n.success(t("role.success.delete", { name: role.roleName }));
      } else {
        const errorMessage =
          response?.data?.msg || t("role.error.unknownError");

        $n.error(`${t("role.error.deleteRoleError")}：${errorMessage}`);
      }
    } catch (err) {
    ...
    await api
      ...
      .then((response) => {
        if (
          ...
          $n.success(t("role.success.giveResource"));
          loading.value = false;
        } else {
          if (response.data.msg) {
            $n.error(response.data.msg);
