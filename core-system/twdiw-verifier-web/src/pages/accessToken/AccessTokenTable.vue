      ...
      >
        <div>
          <RoundSearchbar
            ...
            :placeholder="
              t('input', { input: ' ' + t('accessToken.table.name') })
            "
            @search="onSearch"
          >
            <template #form>
...
const actions = [
  ...
  },
  {
    ...
    method: (props) => {
      ...
      $d.confirm(
        t("confirm"),
        t("accessToken.message.confirmChangeState", {
          name: props.row.accessTokenName,
          action: t("enabled")
        }),
        () => {
  ...
  },
  {
    ...
    method: (props) => {
      ...
      const config = {
        title: t("confirm"),
        message: `${t(
          "accessToken.message.confirmDelete"
...
const fields = ref([
  {
    ...
    inputType: "input",
    field: "accessTokenName",
    hint: t("input", { input: " " + t("accessToken.table.name") })
  },
  {
    name: "ownerName",
    label: t("accessToken.table.ownerName"),
    inputType: "input",
    field: "ownerName",
    hint: t("input", { input: " " + t("accessToken.table.ownerName") })
  },
  {
    name: "beginDate",
    ...
    inputType: "date",
    field: "beginDate",
    hint: t("input", { input: " " + t("createTime.startDate") })
  },
  {
    name: "endDate;",
    ...
    inputType: "date",
    field: "endDate",
    hint: t("input", { input: " " + t("createTime.endDate") })
  },
  {
    name: "orgId",
    ...
    inputType: "input",
    field: "orgId",
    hint: t("input", { input: " " + t("accessToken.table.orgId") })
  },
  {
    name: "enabled",
    label: t("accessToken.table.enabled"),
    ...
    .then(async (response) => {
      if (response && response.data && response.data.code) {
        if (response.data.code === "0") {
          ...
          $n.success(t("accessToken.success.delete"));
    ...
    .then(async (response) => {
      if (response && response.data && response.data.code) {
        if (response.data.code === "0") {
          ...
          $n.success(t("accessToken.success.changeState"));
          await getAllAccessTokens();
        } else {
          if (response.data.msg) {
            $n.error(response.data.msg);
