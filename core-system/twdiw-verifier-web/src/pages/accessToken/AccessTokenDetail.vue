  ...
  <div class="column q-pa-sm q-gutter-sm">
    ...
    <template v-else>
      ...
      <div class="row full-width items-center">
        ...
        <q-btn
          outline
          :label="t('accessToken.button.edit')"
          icon="edit"
          color="accent"
          class="q-pa-sm"
          @click="openDialog"
          ...
          <tbody>
            <tr v-for="row in tableRow" :key="row.field">
              ...
              <td class="text-left" style="white-space: pre-line">
                ...
                <span
                  v-else-if="
                    row.field === 'createTime' || row.field === 'dataRole1'
                  "
                >
...
const tableRow = [
  ...
  {
    label: t("accessToken.table.updateTime"),
    field: "dataRole1"
  }
];
...
const setFormDataFromSelected = () => {
  if (selected.value) {
    ...
    isActivated.value = selected.value.state === "true" ? "啟用" : "停用";
  }
};
    ...
    .then(async (response) => {
      if (response && response.data && response.data.code) {
        if (response.data.code === "0") {
          ...
          $n.success(t("accessToken.success.edit"));
