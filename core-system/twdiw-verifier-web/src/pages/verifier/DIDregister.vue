              ...
              <q-card-section class="q-pt-none">
                <ul class="blue-ls">
                  ...
                  <li class="meali">
                    ...
                    >
                      {{ t("didregister.noticeList.this") }}</a
                    >
          ...
          <template v-slot:body-cell-name="props">
            <q-td :props="props">
              <span>
                ...
                >{{ getCertLabelName(props.row.name) }}
              </span>
            </q-td>
          </template>
            ...
            <q-td :props="props">
              <div>
                <q-input
                  ...
                  hide-bottom-space
                  :placeholder="getCertPlaceHolder(props.row.name)"
                  dense
        ...
        >
          <template v-slot:body-cell-name="props">
            <q-td :props="props">
              <span>
                <span class="text-red">*</span
                >{{ getDidVerifyLabelName(props.row.name) }}
              </span>
            </q-td>
          </template>
          <template v-slot:body-cell-value="props">
            <q-td :props="props">
              <div>
                <q-input
                  ...
                  hide-bottom-space
                  :placeholder="getDidVerifyPlaceHolder(props.row.name)"
                  dense
                  input-style="text-align: center"
...
const isDIDRegistered = ref(false);
...
const getCertLabelName = (name) => {
  const labels = {
    PID: t("didregister.labelName.PID"),
    orgID: t("didregister.labelName.orgID"),
    PIN: t("didregister.labelName.PIN")
  };
  return labels[name] || t("didregister.undefined");
};

const getCertPlaceHolder = (name) => {
  const certPlaceHolder = {
    PID: t("input", { input: t("didregister.labelName.PID") }),
    orgID: t("input", { input: t("didregister.labelName.orgID") }),
    PIN: t("input", { input: t("didregister.labelName.PIN") })
  };
  return certPlaceHolder[name] || t("input", { input: t("didregister.data") });
};

const getCardTypeName = (name) => {
  const values = {
    MOICA: t("didregister.cardTypeName.MOICA"),
    MOEACA: t("didregister.cardTypeName.MOEACA"),
    XCA: t("didregister.cardTypeName.XCA"),
    GCA: t("didregister.cardTypeName.GCA")
  };
  return values[name] || t("didregister.no");
};

const getDidVerifyLabelName = (name) => {
  const labels = {
    serviceBaseURL: t("didregister.labelName.serviceBaseURL"),
    token: "Access-Token"
  };
  return labels[name] || t("didregister.undefined");
};

const getDidVerifyPlaceHolder = (name) => {
  const placeHolder = {
    serviceBaseURL: t("input", {
      input: t("didregister.labelName.serviceBaseURL")
    }),
    token: t("didregister.valid.token")
  };
  return placeHolder[name] || t("input", { input: t("didregister.data") });
};

const getLabelName = (name) => {
  const labels = {
    nameTw: t("didregister.labelName.nameTw"),
    nameEn: t("didregister.labelName.nameEn"),
    info: t("didregister.labelName.info"),
    taxId: t("didregister.labelName.taxId"),
    serviceBaseURL: t("didregister.labelName.serviceBaseURL"),
    token: t("didregister.labelName.token")
  };
  return labels[name] || t("didregister.undefined");
};
...
const getPlaceHolder = (name) => {
  const placeHolder = {
    nameTw: t("input", { input: t("didregister.labelName.nameTw") }),
    nameEn: t("input", { input: t("didregister.labelName.nameEn") }),
    info: t("input", { input: t("didregister.labelName.info") }),
    taxId: t("input", { input: t("didregister.labelName.taxId") }),
    serviceBaseURL: t("input", {
      input: t("didregister.labelName.serviceBaseURL")
    }),
    token: t("didregister.valid.token")
  ...
  if (!didVerifyInputData.value.token) {
    ...
    try {
      const requestBody = {
        ...
        baseUrl: didVerifyInputData.value.serviceBaseURL,
...
const issueHandler = async () => {
  ...
  if (!allValid) {
    ...
    signHandler(
      ...
      JSON.stringify(verifierOrg.value)
    );
  }
};
...
onMounted(async () => {
  ...
  try {
    ...
    if (response.data.data.registered) {
      ...
      if (response.data.data.verifierData) {
        DIDStatus.value = t(
          `didregister.status.${response.data.data.verifierData.data.status}`
        );

        registerTime.value = yyyyMMddHHmmss(
          response.data.data.verifierData.data.createdAt * 1000
        );

        inputData["nameTw"] = response.data.data.verifierData.data.org.name;
        inputData["nameEn"] = response.data.data.verifierData.data.org.name_en;
        inputData["info"] = response.data.data.verifierData.data.org.info;
        inputData["taxId"] = response.data.data.verifierData.data.org.taxId;
        inputData["serviceBaseURL"] =
          response.data.data.verifierData.data.org.serviceBaseURL || " ";
        inputData["token"] = response.data.data.token;
        cardType.value = response.data.data.verifierData.data.org.x509_type;
        ...
        $n.error(t("didregister.error.verifierEndChain"));
      }
    } else {
      ...
      errMsgTax.value = t("validation.onlyNumDashAllowed");
  ...
  generateListener(serialNumber, "click", async () => {
    if (isCert.value === false) {
      ...
      registerLoading.value = true;
      delete verifierOrg.value.access_token;
      delete verifierOrg.value.orgType;
      const requestBody = {
        org: verifierOrg.value,
