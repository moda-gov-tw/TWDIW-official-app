...
const columns = [
  ...
  {
    name: "state",
    label: t("roleChange.table.enabled"),
    sortable: true,
    field: "state",
    align: "left"
  },
...
const submitSearch = async () => {
  ...
  if (isValid) {
    ...
    if (searchForm.value.roleId) {
      ...
      searchQuery.value = t("roleChange.actionTypeAdd");
    } else if (searchForm.value.logType?.value === "modify") {
      searchQuery.value = t("roleChange.actionTypeModify");
    } else if (searchForm.value.logType?.value === "delete") {
      searchQuery.value = t("roleChange.actionTypeDelete");
