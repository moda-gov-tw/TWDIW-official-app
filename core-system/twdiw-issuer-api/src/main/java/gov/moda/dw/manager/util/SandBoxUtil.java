...
public class SandBoxUtil {
  ...
  //        return
  // "http://frontend.service.modadw.dev.webe.hinet.services/api/schema/56801904/VirtualCardCredential/V1/d285201c-56f2-48b8-82a5-40369d595bdb";
  //    }
  ...
  public static String createMetadata(
    ...
    credentialDefinitionTypeArray.add(
        SecurityUtils.getJwtUserObject().get(0).getOrgId() + "_" + vcItemDTO.getSerialNo());
    ...
    proofTypesSupportedJwt.set(
        "proof_signing_alg_values_supported", proofTypesSupportedJwtProofSigningArray);
    ...
    virtualCardCredential.set(
        "cryptographic_binding_methods_supported", cryptographicBindingMethodsSupported);
    virtualCardCredential.set(
        "credential_signing_alg_values_supported", credentialSigningAlgValuesSupported);
    ...
    metadataObject.put(
        "credential_issuer", credentialIssuer + SecurityUtils.getJwtUserObject().get(0).getOrgId());
    metadataObject.put(
        "credential_endpoint",
        credentialEndPoint + SecurityUtils.getJwtUserObject().get(0).getOrgId() + "/credential");
  ...
  public static String create502iSchemaFrom104iResponse(
      String respSchema, String unitTypeExpire, int lengthExpire, String metadata)
  ...
  /** 將字串遮罩，只保留最後 2 個字，前面補 6 個星號。 如果輸入長度小於 2，則直接在前面補上 6 個星號。 */
