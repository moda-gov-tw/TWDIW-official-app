...
public class Ams341wService {
  ...
  public Ams341wApiTrackResDTO getAllApiTrack(Pageable pageable, Ams341wReqDTO ams341wReqDTO) {
    ...
    try {
      ...
      // 查詢
      //            Page<ApiTrackDTO> findAll =
      // apiTrackQueryService.findByCriteria(apiTrackCriteria, pageable);

      Specification<ApiTrack> specification =
          (root, query, criteriaBuilder) -> {
            List<Predicate> predicates = new ArrayList<>();

            if (StringUtils.isNotEmpty(ams341wReqDTO.getSource())) {
              predicates.add(
                  criteriaBuilder.like(root.get("source"), "%" + ams341wReqDTO.getSource() + "%"));
            }

            if (StringUtils.isNotEmpty(ams341wReqDTO.getJhiFrom())) {
              predicates.add(
                  criteriaBuilder.like(
                      root.get("jhiFrom"), "%" + ams341wReqDTO.getJhiFrom() + "%"));
            }

            if (StringUtils.isNotEmpty(ams341wReqDTO.getUri())) {
              predicates.add(
                  criteriaBuilder.like(root.get("uri"), "%" + ams341wReqDTO.getUri() + "%"));
            }

            if (StringUtils.isNotEmpty(ams341wReqDTO.getStatusCode())) {
              if ("success".equalsIgnoreCase(ams341wReqDTO.getStatusCode())) {
                predicates.add(
                    root.get("statusCode").in(Arrays.asList("200", "201", "202", "204")));
                ...
                predicates.add(
                    criteriaBuilder.equal(root.get("statusCode"), ams341wReqDTO.getStatusCode()));
              }
            }

            if (StringUtils.isNotEmpty(ams341wReqDTO.getBeginDate())
                && StringUtils.isNotEmpty(ams341wReqDTO.getEndDate())) {
              DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy/MM/dd");
              Instant beginDate =
                  DateUtils.convertToInstant(
                      ams341wReqDTO.getBeginDate().replace("-", "/"), formatter, true);
              Instant endDate =
                  DateUtils.convertToInstant(
                      ams341wReqDTO.getEndDate().replace("-", "/"), formatter, false);
      ...
      HttpHeaders headers =
          PaginationUtil.generatePaginationHttpHeaders(
              ServletUriComponentsBuilder.fromCurrentRequest(), findAll);
  ...
  private ApiTrackCriteria setCriteriaFromSearch(Ams341wReqDTO ams341wReqDTO) {
    ...
    if (StringUtils.isNotEmpty(ams341wReqDTO.getSource())) {
      apiTrackCriteria.setSource(
          StringFilterUtils.toContainStringFilter(ams341wReqDTO.getSource()));
    }

    if (StringUtils.isNotBlank(ams341wReqDTO.getJhiFrom())) {
      apiTrackCriteria.setJhiFrom(
          StringFilterUtils.toContainStringFilter(ams341wReqDTO.getJhiFrom()));
    }
    ...
    if (StringUtils.isNotEmpty(ams341wReqDTO.getStatusCode())) {
      if ("success".equals(ams341wReqDTO.getStatusCode())
          || "fail".equals(ams341wReqDTO.getStatusCode())) {
        //                if
        // (ResType.WEB.getCode().equals(ams341wReqDTO.getServiceId().toLowerCase())) {
        //                    if ("success".equalsIgnoreCase(ams341wReqDTO.getStatusCode())) {
        //
        // apiTrackCriteria.setStatusCode(StringFilterUtils.toInStringFilter(Arrays.asList("200",
        // "201", "202", "204")));
        //                    }
        //                    if ("fail".equalsIgnoreCase(ams341wReqDTO.getStatusCode())) {
        //
        // apiTrackCriteria.setStatusCode(StringFilterUtils.toNotInStringFilter(Arrays.asList("200",
        // "201", "202", "204")));
        //                    }
        //                }
        //                if
        // (ResType.API.getCode().equals(ams341wReqDTO.getServiceId().toLowerCase())) {
        //                    if ("success".equalsIgnoreCase(ams341wReqDTO.getStatusCode())) {
        //
        ...
        //                }
        if ("success".equalsIgnoreCase(ams341wReqDTO.getStatusCode())) {
          apiTrackCriteria.setStatusCode(
              StringFilterUtils.toInStringFilter(Arrays.asList("200", "201", "202", "204")));
        }
        if ("fail".equalsIgnoreCase(ams341wReqDTO.getStatusCode())) {
          apiTrackCriteria.setStatusCode(
              StringFilterUtils.toNotInStringFilter(Arrays.asList("200", "201", "202", "204")));
          apiTrackCriteria.setStatusCode(StringFilterUtils.toIsNullFilter());
        }
      } else {
        apiTrackCriteria.setStatusCode(
            StringFilterUtils.toEqualStringFilter(ams341wReqDTO.getStatusCode()));
      }
    }
    ...
    if (beginDate != null && endDate != null) {
      if (endDate.isAfter(beginDate)) {
        apiTrackCriteria.setTimestamp(
            InstantFilterUtils.toInstantFilterBetween("timestamp", beginDate, endDate));
      } else {
