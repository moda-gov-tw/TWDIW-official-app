...
/** Service Implementation for managing {@link gov.moda.dw.manager.domain.VCDataStatusLog}. */
@Service
@Transactional
public class VCDataStatusLogService {
  ...
  public Optional<VCDataStatusLogDTO> partialUpdate(VCDataStatusLogDTO vCDataStatusLogDTO) {
    ...
    return vCDataStatusLogRepository
        .findById(vCDataStatusLogDTO.getId())
        .map(
            existingVCDataStatusLog -> {
              vCDataStatusLogMapper.partialUpdate(existingVCDataStatusLog, vCDataStatusLogDTO);

              return existingVCDataStatusLog;
            })
        .map(vCDataStatusLogRepository::save)
        .map(vCDataStatusLogMapper::toDto);
  }
