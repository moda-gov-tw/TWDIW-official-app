  ...
  public Dwvc101iResDTO getExtVcData(String vcUid) {
    ...
    if (!StringUtils.contains(vcUid, "_") && !StringUtils.equals("all", vcUid)) {
      throw HttpXxxErrorExceptionHandler.genHttpClientErrorException(
          ...
          StatusCode.DWVP_VCUID_NOT_VALID.getCode(),
          StatusCode.DWVP_VCUID_NOT_VALID.getMsg());
    }
    ...
    if (StringUtils.equals("all", vcUid)) {
      ...
      // 單筆查詢
      vcItemList =
          customVCItemRepository.findBySerialNoAndBusinessIdAndIsTempAndTypeIn(
              serialNo, businessId, isTemp, type);
    }
    ...
    if (CollectionUtils.isEmpty(vcItemList)) {
      throw HttpXxxErrorExceptionHandler.genHttpClientErrorException(
          HttpStatus.BAD_REQUEST,
          StatusCode.DWVP_VC_ITEM_NOT_FOUND.getCode(),
          StatusCode.DWVP_VC_ITEM_NOT_FOUND.getMsg());
    }
