...
public class CustomScheduleResource {
  ...
  public void checkIsAdmin() {
    boolean isAdmin =
        customScheduleService.checkIsAdminResult(
   ...
   *     scheduleDTO, or with status {@code 400 (Bad Request)} if the schedule has already an ID.
  ...
  public ResponseEntity<ScheduleDTO> createSchedule(@Valid @RequestBody ScheduleDTO scheduleDTO)
    ...
    if (scheduleDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new schedule cannot already have an ID", ENTITY_NAME, "idexists");
    }
    Long userId =
        customScheduleService.findUserId(SecurityUtils.getJwtUserObject().get(0).getUserId());
    ...
    return ResponseEntity.created(new URI("/api/schedules/" + scheduleDTO.getId()))
        .headers(
            HeaderUtil.createEntityCreationAlert(
                applicationName, true, ENTITY_NAME, scheduleDTO.getId().toString()))
        .body(scheduleDTO);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of schedules in
   *     body.
   */
  @PreAuthorize("hasAuthority('vcSchema_clearData')")
  @GetMapping("")
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the scheduleDTO,
  ...
  public ResponseEntity<List<VCItemDataDTO>> getScheduleVcData(
    ...
    if (scheduleDTO.isPresent()) {
      ScheduleDTO scheduleDTO1 = scheduleDTO.get();
      Page<ClearVcItemDataDTO> clearVcItemDataPage =
          customVCItemDataRepository.findClearVcItemDataList(
              VcItemDataValidType.INACTIVE.getCode(),
              scheduleDTO1.getId(),
              scheduleDTO1.getLastRunDatetime(),
              pageable);

      page =
          clearVcItemDataPage.map(
              clearVcItemData -> {
      ...
      throw new BadRequestAlertException(
          "schedule not found", ENTITY_NAME, "schedule not found for scheduleId:" + scheduleId);
    }
    HttpHeaders headers =
        PaginationUtil.generatePaginationHttpHeaders(
            ServletUriComponentsBuilder.fromCurrentRequest(), page);
