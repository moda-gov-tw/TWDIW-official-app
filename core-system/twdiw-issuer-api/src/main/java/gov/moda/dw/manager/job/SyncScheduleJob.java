...
public class SyncScheduleJob extends QuartzJobBean {
  ...
  protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
    try {
      ...
      quartzWorkerService
          ...
          .forEach(
              (scheduleId, dto) -> {
                ...
                if (notExistInDB) {
                  try {
                    quartzWorkerService.delJob(
                        QuartzWorkerService.cleanVCDataTaskNamePrefix + scheduleId);
      ...
      all.stream()
          ...
          .forEach(
              dto -> {
                ...
                ScheduleDTO existsScheduleDTO =
                    quartzWorkerService.getRunScheduleMap().putIfAbsent(scheduleId, dto);
      ...
      log.info(
          "SyncScheduleJob CurrentJob Count Before/After :{}/{}",
          beforeSize,
          quartzWorkerService.getRunScheduleMap().size());
    } catch (Exception ex) {
