...
public class MenuTreeService {
  ...
  public MenuTreeDTO readTree(Long leftId, RelType relType, ResType resType) {
    try (Connection conn = dataSource.getConnection()) {
      log.info(
          "MenuTreeService-readTree 取得前端顯示授權內容開始 leftId={}, relType={}, resType={}",
          leftId,
          relType,
          resType);
      ...
      // 查詢使用哪個db
      // String dbName =
      // dataSource.getConnection().getMetaData().getDatabaseProductName().toLowerCase();
      ...
      if (dbName.contains("postgresql")) {
        // 查詢產樹節點順序
        rowData =
            customResLayerRepository.createPostgresqlTree(
                resType.getCode(), StateType.ENABLED.getCode());
      } else if (dbName.contains("microsoft sql server")) {
        // 查詢產樹節點順序
        rowData =
            customResLayerRepository.createMssqlTree(
                resType.getCode(), StateType.ENABLED.getCode());
      } else {
      ...
      for (Map<String, Object> row : rowData) {
        String parentId =
            row.get("parent_id") == null ? "" : Long.toString((Long) row.get("parent_id"));
      ...
      while (iterator.hasNext()) {
        ...
        if (this.checkNodeChildren(entry.getValue())) {
          log.debug(
              "MenuTreeService-checkNodeChildren 根節點 {} 沒有子節點, 移除掉.", entry.getValue().getLabel());
          iterator.remove();
        }
      }
      ...
      log.info(
          "MenuTreeService-readTree 取得前端顯示授權內容結束 leftId={}, relType={}, resType={}",
          leftId,
          relType,
          resType);
