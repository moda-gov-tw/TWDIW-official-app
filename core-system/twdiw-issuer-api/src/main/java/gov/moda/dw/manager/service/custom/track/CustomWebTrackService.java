...
public class CustomWebTrackService extends TrackService {
  ...
  private static final List<WhitelistRequestDto> ENCRYPT_REQUEST_WHITELIST =
      List.of(
  ...
  // 加密 ResponseBody 白名單
  private static final List<WhitelistRequestDto> ENCRYPT_RESPONSE_BODY_WHITELIST =
      List.of(
          new WhitelistRequestDto("POST", "/api/modadw311w"), // 帳號異動相關(POST)
          new WhitelistRequestDto("GET", "/api/modadw311w"), // 帳號查詢相關(GET)
  ...
  public CustomWebTrackService(
      ApiTrackService apiTrackService, AccessTokenProvider accessTokenProvider) {
    ...
    try {
      ...
      if (req == null) {
        throw new BadRequestAlertException(
            "HttpServletRequest not found!", "AOP", "HttpServletRequest not found!");
      }
      ...
      boolean isReqWhitelisted =
          ENCRYPT_REQUEST_WHITELIST.stream()
              .anyMatch(
                  request ->
                      StringUtils.equalsIgnoreCase(request.getMethod(), req.getMethod())
      ...
      boolean isResWhitelisted =
          ENCRYPT_RESPONSE_BODY_WHITELIST.stream()
              .anyMatch(
                  request ->
                      StringUtils.equalsIgnoreCase(request.getMethod(), req.getMethod())
                          && StringUtils.startsWith(req.getRequestURI(), request.getUri()));
      ...
      if (StringUtils.isNotBlank(jwt)
          ...
          && !this.accessTokenProvider.validateJwtToken(jwt)) {
