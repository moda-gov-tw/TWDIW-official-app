...
public class Ams321wService {
  ...
  public Page<RoleDTO> getAllRole(Ams321wReqDTO ams321ReqDTO, Pageable pageable) {
    ...
    try {
      ...
      if (StringUtils.isNotEmpty(ams321ReqDTO.getCategoryId())) {
        roleCriteria.setRoleId(
            StringFilterUtils.toContainStringFilter(ams321ReqDTO.getCategoryId()));
      }

      if (StringUtils.isNotEmpty(ams321ReqDTO.getCategoryName())) {
        roleCriteria.setRoleName(
            StringFilterUtils.toContainStringFilter(ams321ReqDTO.getCategoryName()));
      }
      ...
      if (opRole.isPresent()) {
        ...
        if (StateType.ENABLED.getCode().equals(roleDTO.getState())) {
          ...
          if (RoleType.DEFAULT_ROLE.getCode().equals(roleDTO.getRoleId())
            ...
            ams321WRoleResponseDTO.setStatusCode(
                StatusCode.ROLE_DEFAULT_ROLE_AND_ADMIN_CANNOT_BE_DISABLED);
  ...
  public Ams321wUpdateResultDTO updateRole(Ams321wReqDTO ams321ReqDTO) {
    ...
    try {
      ...
      if (opRole.isPresent()) {
        ...
        log.warn(
            "Ams321wService-updateRole 查不到該角色, 無法執行編輯角色, role table id= {}", ams321ReqDTO.getId());
        ams321WRoleResponseDTO.setStatusCode(StatusCode.ROLE_NOT_EXISTS);
      }
  ...
  public StatusCode deleteRole(Ams321wReqDTO ams321ReqDTO) {
    ...
    try {
      ...
      if (opRole.isPresent()) {
        ...
        log.warn(
            "Ams321wService-deleteRole 查不到該角色, 無法執行刪除角色, role table id= {}", ams321ReqDTO.getId());
    ...
    try {
      ...
      if (opRole.isPresent()) {
        ...
        List<ResDTO> resDTOList =
            relService.roleAction(
                role, ams321wRelDTO.getResList(), AuthorityAction.ROLE_CONFER_RES);
        if (resDTOList != null) {
          // 新增異動紀錄
          ...
          List<Ams321wRoleConferResDTO> ams321wRoleResResponseDTOList =
              this.resDTOtoConferResList(resDTOList);
          ...
          log.info(
              "Ams321wService-saveConferRes 角色賦予權限完成 共計{}筆, roleId={}",
              resDTOList.size(),
              role.getRoleId());
        } else {
          log.warn(
        ...
        log.warn(
            "Ams321wService-saveConferRes 查不到該角色, 無法設定授權功能, role table id= {}",
            ams321wRelDTO.getId());
  ...
  public Ams321wRoleConferRultDTO searchRoleConferRes(Ams321wReqDTO ams321ReqDTO) {
    ...
    try {
      ...
      log.error(
          "Ams321wService-searchRoleConferRes 發生錯誤, 錯誤原因為={}", ExceptionUtils.getStackTrace(ex));
      return null;
    }
  }
  ...
  public Ams321wTreeNodeDTO getTree(Ams321wReqDTO ams321ReqDTO) {
    ...
    try {
      // 取得結構
      MenuTreeDTO menuTree =
          menuTreeService.readTree(ams321ReqDTO.getId(), RelType.ROLETORES, ResType.WEB);
      if (menuTree != null) {
  ...
  private void saveRoleLog(RoleDTO roleDTO, LogType logType) {
    log.info(
        "Ams321wService-saveRoleLog 開始紀錄角色{}異動 RoleDTO={} LogType={}",
        logType.getName(),
        roleDTO,
        logType);
    ...
    log.info(
        "Ams321wService-saveRoleLog 角色{}異動紀錄完成 RoleDTO={} LogType={}",
        logType.getName(),
        roleDTO,
        logType);
  }
