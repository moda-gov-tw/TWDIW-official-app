...
public class DwSandBoxVC101WResource {
   ...
   *     fieldDTO, or with status {@code 400 (Bad Request)} if the field has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PreAuthorize("hasAnyAuthority('vc_fields_basic','vc_fields_normal')")
  @PostMapping("")
  public ResponseEntity<List<FieldDTO>> createField(@Valid @RequestBody List<FieldDTO> fieldDTO)
      throws Exception {
    ...
    return ResponseEntity.created(new URI("/api/fields/" + fieldDTO.getId()))
        .headers(
            HeaderUtil.createEntityCreationAlert(
                applicationName, true, ENTITY_NAME, fieldDTO.getId().toString()))
        .body(field);
  }
  ...
  public ResponseEntity<List<FieldDTO>> getAllFields(
      FieldCriteria criteria, @org.springdoc.core.annotations.ParameterObject Pageable pageable) {
    log.debug("REST request to get Fields by criteria: {}", criteria);

    if ((criteria.getType().getContains() != null
            && criteria.getType().getContains().contains("NORMAL"))
        || (criteria.getType().getEquals() != null
            && criteria.getType().getEquals().equals("NORMAL"))
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the fieldDTO, or
