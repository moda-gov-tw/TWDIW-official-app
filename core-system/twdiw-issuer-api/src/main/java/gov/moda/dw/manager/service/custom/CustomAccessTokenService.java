...
public class CustomAccessTokenService {
  ...
  public AccessToken saveAccessToken(Ams351wReqDTO ams351wReqDTO) {
    try {
      ...
      accessToken.setState(
          StateType.ENABLED.getEnName().equals(ams351wReqDTO.getState())
              ? StateType.ENABLED.getEnName()
              : StateType.DISABLED.getEnName());
      ...
      log.info("CustomAccessTokenService-saveAccessToken，建立AccessToken完成");
      return save;
    } catch (Exception ex) {
      log.error(
          "CustomAccessTokenService-saveAccessToken-發生錯誤，原因為:{}", ExceptionUtils.getStackTrace(ex));
      return null;
    }
  }
  ...
  public AccessToken updateAccessToken(Ams351wReqDTO ams351wReqDTO, AuthorityAction actionType) {
    try {
      ...
      Optional<AccessToken> dbAccessToken =
          this.accessTokenRepository.findById(ams351wReqDTO.getId());
      ...
      if (dbAccessToken.isPresent()) {
        ...
        if (actionType.equals(AuthorityAction.ACCESS_TOKEN_UPDATE_RES)) {
          ...
          dbAccessToken
              .get()
              .setState("true".equals(ams351wReqDTO.getState()) ? "enabled" : "disabled");
        } else if (actionType.equals(AuthorityAction.ACCESS_TOKEN_CHANGE_ACTIVATED)) {
          dbAccessToken
              .get()
              .setState("enabled".equals(dbAccessToken.get().getState()) ? "disabled" : "enabled");
        }
        dbAccessToken.get().setDataRole1(formattedDate);
        AccessToken update = this.accessTokenRepository.save(dbAccessToken.get());
        log.info(
            "CustomAccessTokenService-updateAccessToken，更新AccessToken完成，accessToken:{}",
            update.getAccessToken());
        return update;
      }
    } catch (Exception ex) {
      log.error(
          "CustomAccessTokenService-updateAccessToken-發生錯誤，原因為:{}",
  ...
  public AccessToken saveAccessToken4CreateUser(Ams351wReqDTO ams351wReqDTO) {
    try {
      ...
      accessToken.setCreateTime(this.getCurrentTimeInTaiwan());
      accessToken.setActype(ams351wReqDTO.getActype());

      AccessToken save = this.accessTokenRepository.save(accessToken);
      log.info(
          "CustomAccessTokenService-saveAccessToken4CreateUser，建立AccessToken(for CreateUser)完成，accessToken:{}",
          save.getAccessToken());
      return save;
    } catch (Exception ex) {
      log.error(
          "CustomAccessTokenService-saveAccessToken4CreateUser-發生錯誤，原因為:{}",
          ExceptionUtils.getStackTrace(ex));
      return null;
    }
  }
  ...
  public void deleteAccessToken(String owner) {
    try {
      ...
      log.error(
          "CustomAccessTokenService-deleteAccessToken-發生錯誤，原因為:{}",
          ExceptionUtils.getStackTrace(ex));
    }
  }
