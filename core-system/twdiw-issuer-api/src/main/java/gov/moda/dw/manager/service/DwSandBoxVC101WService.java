...
public class DwSandBoxVC101WService {
  ...
  public List<FieldDTO> save(List<FieldDTO> fieldDTO) throws Exception {
    ...
    List<Field> fieldsByEname =
        fieldRepository.findByBusinessIdAndEnameIn(
            SecurityUtils.getJwtUserObject().get(0).getOrgId(), ename);

    if (!fieldsByEname.isEmpty()) {
      String nameList = String.join(",", ename);
      throw new BadRequestAlertException(
          "The following names are duplicated: 「" + nameList + "」", ENTITY_NAME_FIELD, "exists");
    }
  ...
  public Optional<FieldDTO> findOne(Long id) {
    ...
    Field field =
        fieldRepository.findByBusinessIdAndId(
            SecurityUtils.getJwtUserObject().get(0).getOrgId(), id);
    if (field == null) {
  ...
  public void delete(Long id) {
    ...
    if (field.isPresent()) {
      ...
      List<VCItemField> vcItemFieldList =
          vcItemFieldRepository.findByBusinessIdAndEname(
              field.get().getBusinessId(), field.get().getEname(), field.get().getType());

      if (!vcItemFieldList.isEmpty()) {
        throw new BadRequestAlertException("This field is used", "", "used");
      }

      fieldRepository.deleteByBusinessIdAndId(
          SecurityUtils.getJwtUserObject().get(0).getOrgId(), id);
    }
  }
}
