...
public class Ams304wResource {
  ...
  public void getVerifyCode(@RequestBody NonceDTO reqNonce, HttpServletResponse response) {
    ...
    Nonce nonce =
        this.nonceRepository
            .findOneBySId(reqNonce.getsId())
            .orElseThrow(() -> new BadRequestAlertException("Nonce is null", "nonce", "nonceNull"));
  ...
  public ResponseEntity<Nonce> validateCaptcha(@RequestBody NonceDTO req) {
    try {
      ...
      Nonce nonce =
          this.nonceRepository
              .findOneBySIdAndCaptchaCode(req.getsId(), req.getCaptchaCode())
              .orElseThrow(
                  () -> {
                    // 當驗證碼錯誤時，先嘗試找到該筆 nonce 並刪除
                    this.nonceRepository
                        .findOneBySId(req.getsId())
