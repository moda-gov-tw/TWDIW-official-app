...
/** 電子證件資料服務實作類別 負責處理電子證件資料的建立、驗證和儲存等相關操作 包含有個資和無個資兩種版本的處理邏輯 */
@Service
@Transactional
public class Dwvc100iService {
  ...
  /**
   * 檢查卡片日期格式 格式必須為 YYYYMMDD，例如：20241231
   *
   * @param date 日期字串
  ...
  private void checkCname(VCItem vcItemEntity, CreateVCItemDataDTO vCItemDataDTO) {
    if (CollectionUtils.isNotEmpty(vCItemDataDTO.getFields())) {
      List<VCItemField> vcItemFields =
          customVCItemFieldRepository.findByVcItemId(vcItemEntity.getId());
      Map<String, String> enameToChameMap =
          vcItemFields.stream()
              .filter(
                  field ->
                      StringUtils.isNotBlank(field.getEname())
                          && StringUtils.isNotBlank(field.getCname()))
  ...
  public CustomVCItemDataDTO createVCHasDataForWeb(CreateVCItemDataDTO vCItemDataDTO) {
    ...
    // 根據ID查詢VC項目
    Optional<VCItem> vcitemEntityOpt =
        customVCItemRepository.findByIdAndIsTempFalse(vCItemDataDTO.getVcId());

    // 如果找不到VC項目，拋出例外
    if (vcitemEntityOpt.isEmpty()) {
      throw HttpXxxErrorExceptionHandler.genHttpClientErrorException(
          HttpStatus.NOT_FOUND, "11001", "vcUid不存在");
    }
   ...
   * @return 建立成功的電子證件資料
   */
  private CustomVCItemDataDTO createVCHasData(
      VCItem vcitemEntity, CreateVCItemDataDTO vcItemDataDTO) {
    ...
    if (StringUtils.isBlank(vcItemDataDTO.getExpiredDate())) {
      ...
      expireDatetime =
          DateUtils.calculate(
              createDatetime, vcitemEntity.getUnitTypeExpire(), vcitemEntity.getLengthExpire());
    } else {
      // 如果指定了到期日，使用指定的到期日
    ...
    DwIssuerOidVci101iRes qrCodeAndDeepLinkAndWarnings =
        this.getQrCodeAndDeepLinkAndWarnings(
            vcitemEntity, transactionId, vcItemDataDTO.getTxCode(), vcItemDataDTO.getCids());
    String deepLink =
        Optional.ofNullable(qrCodeAndDeepLinkAndWarnings)
            .map(DwIssuerOidVci101iRes::getLink)
            .orElse("");
  ...
  private void validateFields(List<CreateVCItemDataField> fields, Long vcItemId) {
    ...
    // 驗證欄位資料
    List<RegularExpressionFieldInvalidDTO> invalidFields =
        this.validateFieldsWithRegex(fields, vcItemFieldList);

    // 如果有無效的欄位，拋出例外
    if (!invalidFields.isEmpty()) {
      String result =
          invalidFields.stream()
              .flatMap(field -> field.getInvalid().stream())
  ...
  private List<RegularExpressionFieldInvalidDTO> validateFieldsWithRegex(
      List<CreateVCItemDataField> fields, List<VCItemField> vcItemFieldList) {
    ...
    List<RegularExpression> regularExpressionList =
        regularExpressionRepository.findByType("require");
    List<Pattern> requireRegularPattern =
        regularExpressionList.stream()
    ...
    for (CreateVCItemDataField field : fields) {
      ...
      if (Boolean.FALSE.equals(vcItemField.getIsRequired())
          && StringUtils.isBlank(field.getContent())) {
        continue;
      }

      // 驗證欄位資料
      RegularExpressionFieldInvalidDTO fieldCheckResult =
          this.validateFieldWithRegex(
              field, vcItemField, regularExpressionList, requireRegularPattern);
  ...
  private RegularExpressionFieldInvalidDTO validateFieldWithRegex(
    ...
    for (int i = 0; i < requireRegularPattern.size(); i++) {
      ...
      boolean matches =
          RegexUtils.isMatchWithTimeout(requireRegularPattern.get(i), field.getContent());
    ...
    // 檢查欄位特定正則表達式
    Pattern pattern =
        RegexUtils.matchWithTimeout(vcItemField.getRegularExpression().getRegularExpression());
   ...
   */
  private DwIssuerOidVci101iRes getQrCodeAndDeepLinkAndWarnings(
      VCItem vcItem, String transactionId, String txCode, List<String> cids) {
    // 產生JWT
    String jwtForApi101i =
        vcItemDataService.getJwtForApi101iRequest(
   ...
   */
  private DwIssuerOidVci101iRes getQrCodeAndDeepLinkAndWarnings(
      String jwtForApi101i, String transactionId, VCItem vcItem, List<String> cids) {
    // 如果JWT為空，回傳空字串
   ...
   * @return VC項目資料DTO
   */
  private CustomVCItemDataDTO convertToDTO(
      VCItemData vCItemData,
      CreateVCItemDataDTO vCItemDataDTO,
      String deepLink,
      DwIssuerOidVci101iWarningsRes warningsRes) {
   ...
   * @return
   */
  private String rebuildVcItemFieldContentJson(
      VCItemData vcItemData, CreateVCItemDataDTO vCItemDataDTO) {
    List<VCItemDataFieldDTO> vcItemDataFieldDTOList = new ArrayList<>();
    try {
      ObjectMapper objectMapper = new ObjectMapper();

      // 將 vc_data content 資料重新組合，補上欄位是否必填
      vcItemDataFieldDTOList =
          objectMapper.readValue(
              vCItemDataDTO.fieldsToJson(), new TypeReference<List<VCItemDataFieldDTO>>() {});

      for (VCItemDataFieldDTO vcItemDataFieldDTO : vcItemDataFieldDTOList) {
        VCItemField vcItemField =
            customVCItemFieldRepository.findByVcItemIdAndEname(
                vcItemData.getVcItem().getId(), vcItemDataFieldDTO.getEname());
      ...
      throw new InternalErrorAlertException(
          "error to rebuild VCItemFieldContentJson: ", ex.getMessage(), "");
    }
  }
  ...
  private void validateRequiredFieldsData(
      List<CreateVCItemDataField> fields, List<VCItemField> vcItemFieldList) {
    // 檢查必填欄位是否有值
    for (VCItemField vcItemField : vcItemFieldList) {
      // 只檢查必填欄位
      if (vcItemField.getIsRequired() != null && vcItemField.getIsRequired()) {
        ...
        CreateVCItemDataField userField =
            fields.stream()
        ...
        if (userField == null || StringUtils.isBlank(userField.getContent())) {
          throw HttpXxxErrorExceptionHandler.genHttpClientErrorException(
              HttpStatus.BAD_REQUEST, "11001", "缺少參數或參數不合法");
        }
      }
    }
  }
