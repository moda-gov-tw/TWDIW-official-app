...
public interface CustomResLayerRepository extends ResLayerRepository {
  ...
  // )
  //    SELECT REPLICATE(' ', cte.lvl * 2) + cte.CHILD_CODE AS CODE, r.res_name, cte.CHILD_ID AS ID,
  // cte.dn, cte.lvl, cte.odr FROM cte_rel AS cte
  //    JOIN res AS r ON cte.CHILD_CODE=r.res_id
  //    WHERE r.type_id='web' or r.type_id='node' 這邊換成自己要的查詢條件
  //    ORDER BY cte.odr"
  @Query(
      value =
          "WITH cte_rel (parent_id, parent_code, child_id, child_code, odr, dn) AS (\n"
              + "    SELECT\n"
              + "        parent_id,\n"
              + "        parent_code,\n"
              + "        child_id,\n"
              ...
              + "ORDER BY cte.odr;",
      nativeQuery = true)
  List<Map<String, Object>> createMssqlTree(
      @Param("typeId") String typeId, @Param("state") String state);
  ...
  @Query(
      value =
        ...
        """,
      nativeQuery = true)
  List<Map<String, Object>> createPostgresqlTree(
      @Param("typeId") String typeId, @Param("state") String state);
}
