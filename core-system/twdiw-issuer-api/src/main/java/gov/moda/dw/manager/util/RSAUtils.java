...
public class RSAUtils {
   ...
   * @return
   */
  public static String publicEncryptReturnBase32(String data, RSAPublicKey publicKey) {
    try {
      Cipher cipher = Cipher.getInstance(TRANSFORMATION);
      OAEPParameterSpec oaepParameterSpec =
          new OAEPParameterSpec(
      ...
      return new Base32().encodeAsString(cipher.doFinal(data.getBytes(CHARSET)));
    } catch (Exception e) {
      log.error(ExceptionUtils.getStackTrace(e));
      throw new AmsBadCredentialsException("encryptFail", "解密字串[" + data + "]時遇到異常");
    }
  ...
  public static String privateDecrypt(String data, RSAPrivateKey privateKey) {
    try {
      ...
      // java.util.Base64.getDecoder().decode(data), privateKey.getModulus().bitLength()),
   ...
   * @return
   */
  public static String privateDecryptFromBase32(String data, RSAPrivateKey privateKey) {
    try {
      Cipher cipher = Cipher.getInstance(TRANSFORMATION);
      OAEPParameterSpec oaepParameterSpec =
          new OAEPParameterSpec(
      ...
      byte[] dataDecodeFromBase32 = base32.decode(data);
      return new String(cipher.doFinal(dataDecodeFromBase32));
    } catch (Exception e) {
      log.error(ExceptionUtils.getStackTrace(e));
      throw new AmsBadCredentialsException("decryptFail", "解密字串[" + data + "]時遇到異常");
    }
  ...
  public static String privateEncrypt(String data, RSAPrivateKey privateKey) {
    try {
      ...
      // data.getBytes(Charset.defaultCharset()), privateKey.getModulus().bitLength())
      //            );
      return Base64.encodeBase64String(cipher.doFinal(data.getBytes(CHARSET)));
    } catch (Exception e) {
      log.error(ExceptionUtils.getStackTrace(e));
      throw new AmsBadCredentialsException("encryptFail", "加密字串[" + data + "]時遇到異常");
    }
  }
  ...
  public static String privateDecryptFromBase32(String data, String privateKey)
      throws NoSuchAlgorithmException, InvalidKeySpecException {
    return privateDecrypt(data, getPrivateKey(privateKey));
  }
  ...
  public static String publicDecrypt(String data, RSAPublicKey publicKey) {
    try {
      ...
      //            return new String(
      //                rsaSplitCodec(cipher, Cipher.DECRYPT_MODE,
      // java.util.Base64.getDecoder().decode(data), publicKey.getModulus().bitLength()),
      //                Charset.defaultCharset()
      //            );
      return new String(cipher.doFinal(java.util.Base64.getDecoder().decode(data)));
    } catch (Exception e) {
