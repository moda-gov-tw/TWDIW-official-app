...
public class CustomVCDataStatusLogService {
   ...
   * 建立一筆 VC 資料狀態紀錄
   *
   * <p>此方法會根據提供的參數建立一筆 VCDataStatusLog 紀錄，並寫入資料庫。
   *
   * @param vcCid VC 憑證編號
   * @param vcItemId vcItemId
   * @param transactionId 交易序號
   * @param valid 狀態 [0: ACTIVE(有效)、1: SUSPENDED(停用)、2: REVOKED(撤銷)、301: INACTIVE(沒掃 QR Code)]
  ...
  public List<VCDataStatusLogDTO> getStatusLogByVcCidOrderByCrDatetimeDesc(String vcCid) {
    return vcDataStatusLogMapper.toDto(
        customVCDataStatusLogRepository.findByVcCidOrderByCrDatetimeDesc(vcCid));
  }
   ...
   * @param valid 狀態 [0: ACTIVE(有效)、1: SUSPENDED(停用)、2: REVOKED(撤銷)、301: INACTIVE(沒掃 QR Code)]
   */
  private void validCreateStatusLog(
      String vcCid, Long vcItemId, String transactionId, Integer valid) {
