...
public class Ams303wService {
  ...
  @Autowired private CustomBwdParamQueryService customBwdParamQueryService;

  @Autowired private CustomBwdHistoryRepository customBwdHistoryRepository;
  ...
  public StatusCode isPasswordValid(IsPasswordValidReqDTO reqDTO) {
    ...
    try {
      ...
      criteria.setRuleId(
          StringFilterUtils.toEqualStringFilter(BwdRuleType.BwdMaximumAge.getRuleId()));
      List<BwdParamDTO> bwdParamDTOList = this.customBwdParamQueryService.findByCriteria(criteria);
      if (bwdParamDTOList.isEmpty()) {
        log.warn(
            "[Ams303wService-isPasswordValid]，{}",
            StatusCode.LOGIN_IS_PASS_WORD_VALID_RULE_NOT_FOUND);
      ...
      final List<BwdHistory> bwdHistoryList =
          this.customBwdHistoryRepository.findAllByUserIdOrderByCreateTimeDesc(reqDTO.getLogin());
      ...
      if (StringUtils.isNotBlank(maxValidPeriod)) {
        ...
        if (!bwdHistoryList.isEmpty()) {
          ...
          log.info(
              "[Ams303wService-isPasswordValid]，{}",
              StatusCode.LOGIN_IS_PASS_WORD_VALID_NO_HISTORY_RECORD.getMsg());
