...
public class RemoteApiService {
  ...
  public String issuerVc203i(String vcCid) {
    ...
    ResponseEntity<String> stringResponseEntity =
        client.put().uri(urlPrefix + vcCid + "/revocation").retrieve().toEntity(String.class);
  ...
  // vc data qrcode
  public DwIssuerOidVci101iRes callApi101i(
      String nonce, String vcOrgId, DwIssuerOidVci101iReq reqContent) {
  ...
  public VC503iResp callApi503i(String get503iUri, String credentialType, String metadata) {
    ...
    try {
      ...
      log.error(
          "callApi503iForExternalCall-HttpServerErrorException  {}",
          ex.getMessage(),
      ...
      log.error(
          "callApi503iForExternalCall-error {}", ex.getMessage(), ExceptionUtils.getStackTrace(ex));
      throw ex;
    }
  ...
  public VCSchemaResDto getVCSchema(String businessId) {
    ...
    try {
      ...
      UriComponentsBuilder uriBuilder =
          UriComponentsBuilder.fromUriString(url).queryParam("businessId", apiBusinessId);
      String apiUrl = uriBuilder.toUriString();
      log.info("getVCSchema-url: {}", apiUrl);
      ResponseEntity<VCSchemaResDto> stringResponseEntity =
          client.get().uri(uriBuilder.build().toString()).retrieve().toEntity(VCSchemaResDto.class);

      return stringResponseEntity.getBody();
    } catch (HttpClientErrorException ex) {
      log.error(
          "getVCSchema-HttpServerErrorException  {}",
          ex.getMessage(),
          ExceptionUtils.getStackTrace(ex));
      VCApiErrorResDTO vcApiErrorRes = ex.getResponseBodyAs(VCApiErrorResDTO.class);
      throw new InternalErrorAlertException(
  ...
  public String getVCInfo(String url) {
    ...
    try {
      ...
      if (errorBody.contains("credential_issuer_meta is empty or null")
          || errorBody.contains("12006")
          || errorBody.contains("No data found matching the criteria")
          || errorBody.contains("11901")) { // 無資料回傳空陣列
        return "{\"credential_configurations_supported\":{}}";
      } else if (e.getStatusCode() == HttpStatus.NOT_FOUND) {
        throw new BadRequestAlertException(
            "Call RemoteApiService ApiError",
            errorBody,
            "RemoteApiService ApiResponse not found error in getVCInfo()");
      } else {
  ...
  public FindIssuerResDTO getDIDInfoById(String did) {
    ...
    if (StringUtils.isAllBlank(did)) {
      throw new BadRequestAlertException(
          StatusCode.DID_INIT_FIND_ISSUER_ERROR.getCode(),
          "Not_Exist",
    ...
    RestTemplate restTemplate = new RestTemplate();
    ResponseEntity<FindIssuerResDTO> findIssuerResonse =
  ...
  public IssuerVC203iRes callApiVc203i(String vcCid, String action) {
    ...
    try {
      ResponseEntity<DwissuerVC203iResp> entity =
          client
              .put()
              .uri(urlPrefix + vcCid + "/" + action)
              .retrieve()
      ...
      if (null != body) {
        log.info(
            "callApiVc203i success action : {}, vcCid : {}, credentialStatus : {}",
            action,
            vcCid,
      ...
      log.warn(
          "callApiVc203i error action : {}, message : {}", action, e.getResponseBodyAsString());
      ...
      log.warn(
          "callApiVc203i unexpected error, action={}, vcCid={}, message={}",
          action,
          vcCid,
          ExceptionUtils.getStackTrace(ex));
      throw ex;
