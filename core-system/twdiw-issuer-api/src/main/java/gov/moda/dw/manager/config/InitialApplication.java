...
public class InitialApplication implements ApplicationRunner {
   ...
   *
   * <p>流程說明： 1. 查詢所有 user_name LIKE 'INIT_%' 的使用者 2. 解析 username，取出底線後的值進行加密 3. 將加密後的結果批次更新至資料庫
   */
  private void encryptedUserName() {
    log.info("InitialApplication run encryptedUserName start");

    // 1. 取得所有 INIT_% 開頭的使用者
    List<ExtendedUserResDTO> extendedUserList =
        customExtendedUserRepository.findByUserName("INIT_%");
    ...
    List<UserNameUpdate> updates =
        extendedUserList.stream().map(this::buildUpdateInfo).filter(Objects::nonNull).toList();
    ...
    // 3. 執行批次更新
    updates.forEach(
        u -> customExtendedUserRepository.updateByUserId(u.encryptedUserName(), u.userId()));
  ...
  private UserNameUpdate buildUpdateInfo(ExtendedUserResDTO user) {
    ...
    if (parts.length < 2) {
      log.warn(
          "Invalid INIT_* format. Skip userId={}, userName={}", user.getUserId(), originalUserName);
      return null;
    }
