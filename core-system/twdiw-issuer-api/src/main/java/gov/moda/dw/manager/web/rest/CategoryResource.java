...
import gov.moda.dw.manager.service.CategoryQueryService;
import gov.moda.dw.manager.service.CategoryService;
import gov.moda.dw.manager.service.criteria.CategoryCriteria;
...
/** REST controller for managing {@link gov.moda.dw.manager.domain.Category}. */
@RestController
...
public class CategoryResource {
   ...
   *     category, or with status {@code 400 (Bad Request)} if the category has already an ID.
  ...
  public ResponseEntity<Category> createCategory(@Valid @RequestBody Category category)
    ...
    if (category.getId() != null) {
      throw new BadRequestAlertException(
          "A new category cannot already have an ID", ENTITY_NAME, "idexists");
    }
    category = categoryService.save(category);
    return ResponseEntity.created(new URI("/api/categories/" + category.getId()))
        .headers(
            HeaderUtil.createEntityCreationAlert(
                applicationName, true, ENTITY_NAME, category.getId().toString()))
        .body(category);
  }
   ...
   *     category, or with status {@code 400 (Bad Request)} if the category is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the category couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/{id}")
  ...
  /**
   * {@code PATCH /categories/:id} : Partial updates given fields of an existing category, field
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     category, or with status {@code 400 (Bad Request)} if the category is not valid, or with
   *     status {@code 404 (Not Found)} if the category is not found, or with status {@code 500
   *     (Internal Server Error)} if the category couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/{id}",
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the category, or
