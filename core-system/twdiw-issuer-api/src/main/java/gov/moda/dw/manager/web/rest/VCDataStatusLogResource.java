...
/** REST controller for managing {@link gov.moda.dw.manager.domain.VCDataStatusLog}. */
@RestController
...
public class VCDataStatusLogResource {
  ...
  public VCDataStatusLogResource(
      VCDataStatusLogService vCDataStatusLogService,
   ...
   *     vCDataStatusLogDTO, or with status {@code 400 (Bad Request)} if the vCDataStatusLog has
   *     already an ID.
  ...
  public ResponseEntity<VCDataStatusLogDTO> createVCDataStatusLog(
    ...
    if (vCDataStatusLogDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new vCDataStatusLog cannot already have an ID", ENTITY_NAME, "idexists");
    }
    ...
    return ResponseEntity.created(new URI("/api/vc-data-status-logs/" + vCDataStatusLogDTO.getId()))
        .headers(
            HeaderUtil.createEntityCreationAlert(
                applicationName, true, ENTITY_NAME, vCDataStatusLogDTO.getId().toString()))
   ...
   *     vCDataStatusLogDTO, or with status {@code 400 (Bad Request)} if the vCDataStatusLogDTO is
   *     not valid, or with status {@code 500 (Internal Server Error)} if the vCDataStatusLogDTO
   *     couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/{id}")
  public ResponseEntity<VCDataStatusLogDTO> updateVCDataStatusLog(
      ...
      @Valid @RequestBody VCDataStatusLogDTO vCDataStatusLogDTO)
  ...
  /**
   * {@code PATCH /vc-data-status-logs/:id} : Partial updates given fields of an existing
   * vCDataStatusLog, field will ignore if it is null
   *
   * @param id the id of the vCDataStatusLogDTO to save.
   * @param vCDataStatusLogDTO the vCDataStatusLogDTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     vCDataStatusLogDTO, or with status {@code 400 (Bad Request)} if the vCDataStatusLogDTO is
   *     not valid, or with status {@code 404 (Not Found)} if the vCDataStatusLogDTO is not found,
   *     or with status {@code 500 (Internal Server Error)} if the vCDataStatusLogDTO couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
  ...
  public ResponseEntity<VCDataStatusLogDTO> partialUpdateVCDataStatusLog(
      @PathVariable(value = "id", required = false) final Long id,
      @NotNull @RequestBody VCDataStatusLogDTO vCDataStatusLogDTO)
      throws URISyntaxException {
    LOG.debug(
        "REST request to partial update VCDataStatusLog partially : {}, {}",
        id,
        vCDataStatusLogDTO);
    if (vCDataStatusLogDTO.getId() == null) {
      throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
   ...
   *     vCDataStatusLogs in body.
   */
  @GetMapping("")
  public ResponseEntity<List<VCDataStatusLogDTO>> getAllVCDataStatusLogs(
      VCDataStatusLogCriteria criteria,
   ...
   *     vCDataStatusLogDTO, or with status {@code 404 (Not Found)}.
   */
  @GetMapping("/{id}")
