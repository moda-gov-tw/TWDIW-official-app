...
public class CustomIssuerHealthWebExtension extends HealthEndpointWebExtension {
  ...
  public WebEndpointResponse<HealthComponent> health(
    ...
    if (buildProperties != null) {
      log.info(
          "CustomIssuerHealthWebExtension: Health check triggered, twdiw-release={}, image-version={}",
          buildProperties.get("twdiw-release.version"),
          buildProperties.getVersion());
    } else {
      log.info(
          "CustomIssuerHealthWebExtension: Health check triggered, but buildProperties is null.");
    }
    ...
    if (body instanceof Health health) {
      ...
      if (buildProperties != null) {
        twdiwReleaseMap.put(
            "moda-digitalwallet-docker-issuer-manager-service",
            (String) buildProperties.get("twdiw-release.version"));
        imageVersionMap.put(
            "moda-digitalwallet-docker-issuer-manager-service", buildProperties.getVersion());
      }
      ...
      if (vcServiceInfo != null) {
        if (vcServiceInfo.containsKey("twdiw-release")) {
          twdiwReleaseMap.put(
              "moda-digitalwallet-docker-issuer-vc-service", vcServiceInfo.get("twdiw-release"));
        }
        if (vcServiceInfo.containsKey("image-version")) {
          imageVersionMap.put(
              "moda-digitalwallet-docker-issuer-vc-service", vcServiceInfo.get("image-version"));
        }
      }
      ...
      if (oid4vciServiceInfo != null) {
        if (oid4vciServiceInfo.containsKey("twdiw-release")) {
          twdiwReleaseMap.put(
              "moda-digitalwallet-docker-issuer-oid4vci-service",
              oid4vciServiceInfo.get("twdiw-release"));
        }
        if (oid4vciServiceInfo.containsKey("image-version")) {
          imageVersionMap.put(
              "moda-digitalwallet-docker-issuer-oid4vci-service",
              oid4vciServiceInfo.get("image-version"));
        }
      }
