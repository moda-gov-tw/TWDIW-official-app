...
public class DwSandBoxVC301WResource {
  ...
  public DwSandBoxVC301WResource(
      ...
      DwSandBoxVC301WService dwSandBoxVC301WService) {
  ...
  @PostMapping("")
  public ResponseEntity<VCItemDataDTO> createVCItemData(
      @Valid @RequestBody CreateVCItemDataDTO vCItemDataDTO) {
  ...
  public ResponseEntity<VcItemDataFieldDataResDTO> getVCItemDataDetail(
      @PathVariable("vccid") String vccidEncode)
      throws NoSuchAlgorithmException, InvalidKeySpecException {
    log.debug(
        "REST request to get VCItemDataDetail from table vc.credential vcid: {}", vccidEncode);

    String vccidDecode =
        RSAUtils.privateDecryptFromBase32(vccidEncode, RSAUtils.getPrivateKey(privateKey));
