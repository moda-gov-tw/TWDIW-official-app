...
public class OrgResource {
  ...
  public OrgResource(
      OrgService orgService, OrgRepository orgRepository, OrgQueryService orgQueryService) {
   ...
   *     orgDTO, or with status {@code 400 (Bad Request)} if the org has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("")
  public ResponseEntity<OrgDTO> createOrg(@Valid @RequestBody OrgDTO orgDTO)
      throws URISyntaxException {
    LOG.debug("REST request to save Org : {}", orgDTO);
    if (orgDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new org cannot already have an ID", ENTITY_NAME, "idexists");
    }
    orgDTO = orgService.save(orgDTO);
    return ResponseEntity.created(new URI("/api/orgs/" + orgDTO.getId()))
        .headers(
            HeaderUtil.createEntityCreationAlert(
                applicationName, true, ENTITY_NAME, orgDTO.getId().toString()))
        .body(orgDTO);
  }
   ...
   *     orgDTO, or with status {@code 400 (Bad Request)} if the orgDTO is not valid, or with status
   *     {@code 500 (Internal Server Error)} if the orgDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/{id}")
  ...
  /**
   * {@code PATCH /orgs/:id} : Partial updates given fields of an existing org, field will ignore if
   * it is null
   *
   * @param id the id of the orgDTO to save.
   * @param orgDTO the orgDTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     orgDTO, or with status {@code 400 (Bad Request)} if the orgDTO is not valid, or with status
   *     {@code 404 (Not Found)} if the orgDTO is not found, or with status {@code 500 (Internal
   *     Server Error)} if the orgDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/{id}",
  ...
  public ResponseEntity<List<OrgDTO>> getAllOrgs(
      OrgCriteria criteria, @org.springdoc.core.annotations.ParameterObject Pageable pageable) {
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the orgDTO, or
