...
public interface CustomVCCredentialRepository extends VCCredentialRepository {
  ...
  @Query(
      value = REVOKE_VC_ITEM_DATA_BY_CID_SQL,
      countQuery = REVOKE_VC_ITEM_DATA_BY_CID_SQL,
      nativeQuery = true)
  Optional<VCCredentialRevokeVcItemDataDTO> findRevokeVcItemDataByCid(@Param("cid") String cid);

  @Query(
      value = REVOKE_VC_ITEM_DATA_LIST_SQL,
      countQuery = REVOKE_VC_ITEM_DATA_LIST_SQL,
      nativeQuery = true)
  List<VCCredentialRevokeVcItemDataDTO> findRevokeVcItemDataList(@Param("validType") int validType);

  @Query(
      value = REVOKE_VC_CREDENTIAL_LIST_SQL,
      countQuery = REVOKE_VC_CREDENTIAL_LIST_SQL,
      nativeQuery = true)
  List<VCCredentialRevokeVcItemDataDTO> findRevokeVcCredentialList(
      @Param("validType") int validType);

  @Query(
      value = REVOKE_VC_CREDENTIAL_NOT_IN_LOG_LIST_SQL,
      countQuery = REVOKE_VC_CREDENTIAL_NOT_IN_LOG_LIST_SQL,
      nativeQuery = true)
  List<VCCredentialRevokeVcItemDataDTO> findRevokeVcCredentialLogList(
      @Param("validType") int validType);
