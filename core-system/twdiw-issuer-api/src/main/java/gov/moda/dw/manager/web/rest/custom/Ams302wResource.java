...
public class Ams302wResource {
  ...
  @Autowired private Ams311wService ams311wService;
  ...
  public ResponseEntity<Ams302wValidateResetKetResDTO> validateResetKey(
      @RequestBody String resetKey) {
  ...
  public ResponseEntity<FinishPasswordResetResponse> finishPasswordReset(
      @RequestBody KeyAndPasswordVM request) {
    ...
    try {
      ...
      return ResponseEntity.internalServerError()
          .body(new FinishPasswordResetResponse("-1", e.getMessage()));
    }
  }
  ...
  public ResponseEntity<ResponseDTO> finishPasswordChange(@RequestBody BwdChangeDTO req) {
    ...
    try {
      // 解密RSA
      String currentBwd =
          RSAUtils.privateDecrypt(req.getCurrentBwd(), RSAUtils.getPrivateKey(privateKey));
