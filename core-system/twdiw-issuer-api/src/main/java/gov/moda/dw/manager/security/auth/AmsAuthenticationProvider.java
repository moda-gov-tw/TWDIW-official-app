...
public class AmsAuthenticationProvider extends DaoAuthenticationProvider {
  ...
  @Autowired private CustomExtendedUserRepository customExtendedUserRepository;
  ...
  protected void additionalAuthenticationChecks(
      UserDetails userDetails, AmsUsernamePasswordAuthenticationToken authentication)
      throws AuthenticationException {
    if (authentication.getCredentials() == null) {
      this.logger.debug("Failed to authenticate since no credentials provided");
      throw new BadCredentialsException(
          this.messages.getMessage(
              "AbstractUserDetailsAuthenticationProvider.badCredentials", "帳號或密碼錯誤"));
    }
    // ! 取得 密碼規則 中的登入段落 需檢核的
    // * 取得 密碼規則
    ...
    BwdRuleType ruleType =
        this.bwdParamCustomService.preRuleCheckForLogin(userDetails.getUsername(), ruleIdMap);
    // * 重新包裝 回應訊息
    if (null != ruleType) {
      String msg;
    ...
    if ("AP".equalsIgnoreCase(extendedUser.getUserTypeId())) {
      ...
      if (!this.passwordEncoder.matches(
          presentedPassword, userDetails.getPassword() + authentication.getNonceId())) {
        this.logger.debug("Failed to authenticate since password does not match stored value");
    ...
    // * 檢查「後項」 及 取得錯誤訊息
    ruleType =
        this.bwdParamCustomService.postRuleCheckForLogin(userDetails.getUsername(), ruleIdMap);
    // * 重新包裝 回應訊息
    if (null != ruleType) {
      String msg;
      log.warn("登入失敗: 帳號={} , 理由={}", userDetails.getUsername(), msg = ruleType.getMsg());
  ...
  public Authentication getAuthentication(String token) {
    ...
    AmsUsernamePasswordAuthenticationToken authentication =
        new AmsUsernamePasswordAuthenticationToken(jwt.getSubject(), "", "");
  ...
  private ExtendedUser getExtendedUser(String username) {
    ...
    if (extendedUserOpt.isEmpty()) {
      log.error(
          "AmsAuthenticationProvider-authenticate 找不到該user在extendedUser的資料. username={}", username);
      throw new BadCredentialsException(
          this.messages.getMessage(
              "AbstractUserDetailsAuthenticationProvider.badCredentials", "缺少資料"));
    }
