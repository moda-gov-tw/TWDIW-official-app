...
public class AccessTokenProvider {
  ...
  public StatusCode validateToken(String accessToken) {
    ...
    if (!AC_TYPE_UNLIMITED.equals(accessTokenDTO.getActype())) {
      log.warn(
          "AccessTokenProvider-validateToken，AccessToken限期異常，accessTokenDTO:{}", accessTokenDTO);
      return StatusCode.TOKEN_AC_TYPE_EXCEPTION;
    }
    if (StateType.DISABLED.getEnName().equals(accessTokenDTO.getState())) {
      log.warn(
          "AccessTokenProvider-validateToken，AccessToken未啟用，accessTokenDTO:{}", accessTokenDTO);
  ...
  public Authentication getAuthenticationByAccessToken(String accessToken, String sourceIp) {
    List<RelDTO> relDTOList =
        this.customRelQueryService.findRelDTOListForAccessTokenStr(accessToken);
    ...
    if (null != accessTokenDto
        && StringUtils.equals(StateType.ENABLED.getCode(), accessTokenDto.getState())) {
      List<RelDTO> relUserRoleList =
          this.customRelQueryService.findByLeftCodeGetRole(accessTokenDto.getOwner());
      for (RelDTO relUserRole : relUserRoleList) {
        List<RelDTO> relRoleResList =
            this.customRelQueryService.findByLeftCode(
                relUserRole.getRightCode(), StateType.ENABLED.getCode());
  ...
  public String getAuthenticationByAccessToken(String token) {
    try {
      ...
      log.warn(
          "AccessTokenProvider-getJwtUserName，Failed to decode JWT token: {}",
  ...
  public Authentication getAuthentication(String token) {
    try {
      ...
      Collection<? extends GrantedAuthority> authorities =
          Arrays.stream(jwt.getClaimAsString(AUTHORITIES_KEY).split(","))
      ...
      log.error(
          "AccessTokenProvider-getAuthentication，Failed to decode JWT token: {}",
          ExceptionUtils.getStackTrace(ex));
      return null;
    }
  }
