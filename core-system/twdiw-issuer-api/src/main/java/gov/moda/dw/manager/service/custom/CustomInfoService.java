...
public class CustomInfoService {
  ...
  public CustomVersionInfoResDTO getVersionInfo() {
    ...
    // 取得 ISSUERMGR 環境資訊
    CustomVersionInfoDetailResDTO issuermgrVersionInfoDetailResDTO =
        getVersionInfoDetail(SystemCode.ISSUERMGR);
    customVersionInfoResDTO.setIssuermgr(issuermgrVersionInfoDetailResDTO);

    // 取得 OID4VCI 環境資訊
    CustomVersionInfoDetailResDTO oid4vciVersionInfoDetailResDTO =
        getVersionInfoDetail(SystemCode.OID4VCI);
  ...
  private CustomVersionInfoDetailResDTO getVersionInfoDetail(SystemCode systemCode) {
    ...
    try {
      ...
      if (StringUtils.isNotBlank(versionInfo)) {
        ...
        customVersionInfoDetailResDTO.setAbbrev(
            node.path("git").path("commit").path("id").path("abbrev").asText(null));
        log.info("已取得 {} 版本資料", systemCode.getName());
      }
    } catch (Exception e) {
      log.warn(
          "取得 {} 系統 management info 發生錯誤，原因為:{}",
          systemCode.getName(),
