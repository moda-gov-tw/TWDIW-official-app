...
public class SyncRevokedVcItemDataSchedule {
  ...
  private void run() {
    ...
    try {
      ...
      List<VCCredentialRevokeVcItemDataDTO> revokeVcCredentialList =
          customVCCredentialRepository.findRevokeVcCredentialList(revoked);
      ...
      for (VCCredentialRevokeVcItemDataDTO vcCredentiaDTO : revokeVcCredentialList) {
        ...
        dwSandBoxVC302WService.executeVcActionAndRecord(
            null,
            vcCredentiaDTO.getCid(),
            vcCredentiaDTO.getLastUpdateTime(),
            VcItemDataValidType.REVOKED.getAction());
        successCount++;
      }

      // step3 當 credential 為撤銷且 vc_data_status_log 無撤銷紀錄時，建立一筆 VC 資料狀態紀錄
      List<VCCredentialRevokeVcItemDataDTO> revokeVcCredentialLogList =
          customVCCredentialRepository.findRevokeVcCredentialLogList(revoked);
