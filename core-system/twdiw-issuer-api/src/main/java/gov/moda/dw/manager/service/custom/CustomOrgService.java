...
public class CustomOrgService {
  ...
  private String getDidOrgId() {
    try {
      ...
      ResponseEntity<FindIssuerResDTO> response =
      ...
      if (response.getBody() == null || response.getBody().getCode() != 0) {
        log.warn(
            "CustomOrgService-getAllOrg-front301i查詢DID資訊失敗, code={}, msg={}",
  ...
  public StatusCode createOrg(OrgReqDTO orgReqDTO) {
    ...
    try {
      ...
      // 開始新增組織
      if (orgReqDTO.getVcDataSource() == null) orgReqDTO.setVcDataSource(501L);
  ...
  public OrgUpdateResultDTO updateOrg(OrgReqDTO orgReqDTO) {
    ...
    try {
      ...
      if (opOrg.isPresent()) {
        ...
        // 不能修改組織代號
        if (!orgReqDTO.getOrgId().equals(orgDTO.getOrgId())) {
          log.info("CustomOrgService-updateOrg 組織代號不可修改");
          orgResponseDTO.setStatusCode(StatusCode.ORG_ORGID_CANNOT_BE_EDIT);
          return orgResponseDTO;
        }

        // 不能修改組織英文名稱
        if (!orgReqDTO.getOrgEnName().equals(orgDTO.getOrgEnName())) {
          log.info("CustomOrgService-updateOrg 組織英文名稱不可修改");
          orgResponseDTO.setStatusCode(StatusCode.ORG_ORGENNAME_CANNOT_BE_EDIT);
          return orgResponseDTO;
        }
        ...
        List<OrgDTO> orgDTOList = orgQueryService.findByCriteria(orgCriteria);
        if (CollectionUtils.isNotEmpty(orgDTOList)
            && !orgReqDTO.getId().equals(orgDTOList.get(0).getId())) {
  ...
  public StatusCode deleteOrg(OrgReqDTO orgReqDTO) {
    ...
    try {
      ...
      if (opOrg.isPresent()) {
        ...
        // 該組織有建立VC模板，無法刪除該組織
        if (customVCItemRepository.existsByBusinessId(orgReqDTO.getOrgId())) {
          log.info("CustomOrgService-deleteOrg 該組織有建立VC模板，無法刪除該組織");
          return StatusCode.ORG_ORG_HAS_VC_SCHEMA_CANNOT_BE_DELETE;
        }
        ...
        if (!affectedUsers.isEmpty()) {
          ...
          log.info(
              "CustomOrgService-deleteOrg 已將 {} 個 extendedUser 的 orgId 更新為 'default'",
              affectedUsers.size());
        } else {
          log.info(
              "CustomOrgService-deleteOrg 未發現需要更新的 extendedUser 賬號 orgId={}", orgDTO.getOrgId());
        }
  ...
  private OrgDTO setToOrgDTO(OrgReqDTO orgReqDTO) {
    ...
    orgDTO.setVcDataSource(orgReqDTO.getVcDataSource());
  ...
  private OrgResDTO orgDTOToOrgResDTO(OrgDTO orgDTO) {
    ...
    orgSearchAllResDTO.setVcDataSource(orgDTO.getVcDataSource());
  ...
  public List<OrgTitleResDTO> getOrgList() {
    ...
    if (checkIsPrivilegedAccount(SecurityUtils.getJwtUserObject().get(0).getUserId())) {
      List<Org> orgList = orgRepository.findAllByOrderByOrgId();
      for (Org org : orgList) {
      ...
      Optional<Org> org =
          orgRepository.findByOrgIdOrderByOrgId(SecurityUtils.getJwtUserObject().get(0).getOrgId());
      if (org.isPresent()) {
    ...
    result.sort(
        (o1, o2) -> {
          String s1 = o1.getOrgId();
          String s2 = o2.getOrgId();
          int len1 = s1.length();
          int len2 = s2.length();
