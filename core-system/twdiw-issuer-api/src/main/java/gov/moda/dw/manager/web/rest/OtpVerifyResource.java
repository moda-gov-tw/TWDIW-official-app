...
public class OtpVerifyResource {
   ...
   *     otpVerifyDTO, or with status {@code 400 (Bad Request)} if the otpVerify has already an ID.
  ...
  public ResponseEntity<OtpVerifyDTO> createOtpVerify(@Valid @RequestBody OtpVerifyDTO otpVerifyDTO)
    ...
    if (otpVerifyDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new otpVerify cannot already have an ID", ENTITY_NAME, "idexists");
    }
    otpVerifyDTO = otpVerifyService.save(otpVerifyDTO);
    return ResponseEntity.created(new URI("/api/otp-verifies/" + otpVerifyDTO.getId()))
        .headers(
            HeaderUtil.createEntityCreationAlert(
                applicationName, true, ENTITY_NAME, otpVerifyDTO.getId().toString()))
        .body(otpVerifyDTO);
  }
   ...
   *     otpVerifyDTO, or with status {@code 400 (Bad Request)} if the otpVerifyDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the otpVerifyDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/{id}")
  public ResponseEntity<OtpVerifyDTO> updateOtpVerify(
  ...
  /**
   * {@code PATCH /otp-verifies/:id} : Partial updates given fields of an existing otpVerify, field
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     otpVerifyDTO, or with status {@code 400 (Bad Request)} if the otpVerifyDTO is not valid, or
   *     with status {@code 404 (Not Found)} if the otpVerifyDTO is not found, or with status {@code
   *     500 (Internal Server Error)} if the otpVerifyDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/{id}",
  ...
  public ResponseEntity<OtpVerifyDTO> partialUpdateOtpVerify(
      @PathVariable(value = "id", required = false) final Long id,
      @NotNull @RequestBody OtpVerifyDTO otpVerifyDTO)
      throws URISyntaxException {
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of otpVerifies in
   *     body.
   */
  @GetMapping("")
  public ResponseEntity<List<OtpVerifyDTO>> getAllOtpVerifies(
      OtpVerifyCriteria criteria,
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the otpVerifyDTO,
