...
public class Modadw101wService {
  ...
  @Autowired private RegularExpressionService regularExpressionService;
  ...
  public ResponseDTO<GetIssuerDataResDTO> getInitData(String req) {
    ...
    try {
      ...
      if (!Objects.equals(setting.get().getPropValue(), "")) {
        ...
        ResponseEntity<FindIssuerResDTO> findIssuerResponse =
            restTemplate.exchange(url, HttpMethod.GET, entity, FindIssuerResDTO.class);
        ...
        if (findIssuerResponse.getBody() == null || findIssuerResponse.getBody().getCode() != 0) {
          log.error(
              ...
              findIssuerResponse.getBody().getCode(),
              findIssuerResponse.getBody().getMsg());
        ...
        Optional<Setting> tokenSetting =
            settingService.findOne("access_token.frontend.register_did");
  ...
  public StatusCode register(Modadw101ReqDTO req) {
    try {
      ...
      log.info(
          "Modadw101wService-register 呼叫issuer-vc註冊DID，Request Body:{},",
      ...
      if (!responseBody.has("did")) {
        log.error(
            "Modadw101wService-register issuer-vc註冊DID失敗，錯誤碼: {}, 錯誤訊息: {}",
  ...
  public ResponseDTO<VerifyCertAndDidResDTO> verifyDid(VerifyDidReqDTO reqDTO) {
    ...
    if (!Objects.equals(ivpasResponse.getHeader().getReturnCode(), "0000")) {
      log.error(
          "Modadw101wService-verifyDid IVPAS檢查未通過，代碼:{}, 錯誤訊息:{}",
