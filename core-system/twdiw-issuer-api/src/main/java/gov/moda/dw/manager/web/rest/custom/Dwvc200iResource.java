...
/** 電子證件資料的 REST 控制器 負責處理電子證件資料的查詢、建立和 QR Code 相關操作 包含外部 API 整合和沙盒環境的資料處理 */
@Slf4j
@RestController
@RequestMapping("/api")
...
public class Dwvc200iResource {
  ...
  /**
   * [DWVC-201] 發行端VC卡片單一資料查詢 透過 nonce 向 dwissuer-vc-402i 查詢電子證件 QR Code 狀態 此 API 用於查詢使用者是否已經掃描 QR
   * Code 並完成電子證件的建立
   *
   * @param nonce 交易識別碼
   * @return 電子證件狀態資訊
  ...
  public ResponseEntity<Dwvc201iResDTO> getVCItemDataByNonce(@PathVariable("nonce_id") String nonce)
      throws IOException {
  ...
  /**
   * 傳入 JWT，代為呼叫 dwissuer-oid4vci-101i 取得電子證件資料用的 QR Code 此 API 用於產生電子證件資料的 QR Code
   *
  ...
  public ResponseEntity<Dwvc202iResDTO> getVCItemDataByDataTag(
      @PathVariable("dataTag") String dataTag,
      @org.springdoc.core.annotations.ParameterObject @PageableDefault(size = 10)
          Pageable pageable) {
  ...
  /**
   * 檢查使用者是否為特權帳號 根據使用者 ID 查詢其角色，判斷是否具有特權帳號權限
   *
   * @param userId 使用者 ID
   * @return 是否為特權帳號
   */
