...
public class DateUtils {
    ...
    /** 檢驗input date是否為uuuuMMdd格式 注意，因保險公司輸入的時間為本地時間，所以要用Asia/Taipei時區時間 */
    ...
    /**
     * 檢驗input datetime是否為HHMM格式 注意，因保險公司輸入的時間為本地時間，所以要用Asia/Taipei時區時間
     *
     * @param time: HHMM格式
     */
     ...
     * @return
     */
    public static Instant convertToInstant(String dateStr, DateTimeFormatter formatter, boolean isStartDate) {
        if (StringUtils.isBlank(dateStr)) {
            return null;
        }
        // 1.解析日期字串為LocalDate(傳入的日期)
        LocalDate localDate = LocalDate.parse(dateStr, formatter);
        LocalDateTime localDateTime;

        // 2.根據是否是開始時間設置不同的LocalDateTime
        if (isStartDate) {
            // 開始時間設置為傳入日期的00:00:00
            localDateTime = localDate.atTime(LocalTime.MIN);
        } else {
            // 結束時間設置為傳入日期的23:59:59
            localDateTime = localDate.atTime(LocalTime.MAX);
        }

        // 3. 將 LocalDateTime 轉換為 Instant，這裡使用 UTC 固定時區
     ...
     * @return
     */
    public static Instant calculate(Instant instant, String timeUnit, long num) {
        // 使用系統預設時區，將 Instant 轉為 ZonedDateTime，因為 Instant 本身只表示 UTC 時間戳，沒有年月的概念
        ZonedDateTime zdt = instant.atZone(ZoneId.systemDefault());

        // 根據傳入的時間單位，對 ZonedDateTime 做相應的加法運算，然後再轉回 Instant
        return switch (timeUnit) {
            case "YEAR"   -> zdt.plusYears(num).toInstant();
            case "MONTH"  -> zdt.plusMonths(num).toInstant();
            case "DAY"    -> zdt.plusDays(num).toInstant();
            case "HOUR"   -> zdt.plusHours(num).toInstant();
            case "MINUTE" -> zdt.plusMinutes(num).toInstant();
            default -> instant;
        };
    }
}
