...
public class CustomVCItemDataService extends VCItemDataService {
  ...
  public Page<VCItemDataDTO> findByCriteria(Page<VCItemData> vcItemDataPages) {
    ...
    for (VCItemData vCItemData : vcItemDataPages) {
      // qrcode 存活時間
      Instant crDatetimePlus10Minutes =
          vCItemData.getCrDatetime().plus(Duration.ofMinutes(vcdataQrcodeAliveTimeMin));

      if ((vCItemData.getVcCid() == null || vCItemData.getVcCid().isEmpty())
          && Instant.now().isBefore(crDatetimePlus10Minutes)) {
        DwIssuerVC402iResDTO vc402iRes =
            remoteApiService.callApi402i(vCItemData.getTransactionId());
  ...
  public Optional<VCItemDataDTO> findAndUpdateVCDataStatus(Long id) {
    ...
    if ((vcItemData.getVcCid() == null || vcItemData.getVcCid().isEmpty())
      ...
      if (vc402iRes != null) {
        ...
        // 取得卡片到期日
        Instant expiredDate =
            this.findIssuanceDateByTxIds(Arrays.asList(vcItemData.getTransactionId())).stream()
                .map(VCCredential::getExpirationDate)
                .findFirst()
                .orElse(vcItemData.getExpired());
  ...
  public List<VCItemData> findVcItemDataByScheduleId(
      Long scheduleIdLong, Instant clearScheduleDatetime) {
    return vCItemDataRepository.findByClearScheduleIdAndClearScheduleDatetime(
        scheduleIdLong, clearScheduleDatetime);
  }

  public int updateContentNull(
      Long scheduleIdLong, Instant clearScheduleDatetime, String businessId) {
    if (cleanVCData) {
      return vCItemDataRepository.updateContentNullWithClean(
          scheduleIdLong, clearScheduleDatetime, businessId);
    } else {
      return vCItemDataRepository.updateContentNull(
          scheduleIdLong, clearScheduleDatetime, businessId);
    }
  }

  public int updateValidAndMsgById(VcItemDataValidType valid, VCItemData vCItemData) {
    return vCItemDataRepository.updateValidAndMessageById(
        valid.getCode(), vCItemData.getId(), vCItemData.getScheduleRevokeMessage());
  }

  public int updateMessageById(VCItemData vCItemData) {
    return vCItemDataRepository.updateMessageById(
        vCItemData.getId(), vCItemData.getScheduleRevokeMessage());
  }
  ...
  public VcItemData305iResponseDTO apiForTest901i(VcItemData305iRequestDTO request)
    ...
    if (vcItemData == null || vcItemData.getContent() == null || vcItemData.equals("")) {
      if (vcItemData != null
          && vcItemData.getVcItem() != null
          && vcItemData.getVcItem().getId() != null) {
        List<VCItemField> VCItemField =
            vCItemFieldRepository.findByVcItemId(vcItemData.getVcItem().getId());
      ...
      List<Map<String, String>> dataList =
          objectMapper.readValue(
              vcItemData.getContent(), new TypeReference<List<Map<String, String>>>() {});
  ...
  public DwIssuerOidVci101iRes createQrcode(VCDataQrcodeReqDTO request) {
    ...
    String[] credentialType =
        SandBoxUtil.splitCredentialType(credentialConfigurationId, "createQrcode");
    ...
    boolean privilege =
        checkIsPrivilegedAccount(SecurityUtils.getJwtUserObject().get(0).getUserId());
    if (!SecurityUtils.getJwtUserObject().get(0).getOrgId().equals(vcItem.getBusinessId())
        && !vcItem.getExpose()
        && !privilege) {
      throw new BadRequestAlertException(
  ...
  public Dwvc202iResDTO findVcItemDataByDataTag(String dataTag, Pageable pageable) {
    ...
    Page<Dwvc202iVcData> dwvc202iVcData =
        resultList.map(
            credential -> {
              ...
              vcData.setExpirationDate(
                  DateUtils.convertDate(credential.getExpirationDate(), format));
              return vcData;
            });
  ...
  public Dwvc203iResDTO findVcItemData(Dwvc203iReqDTO request) {
    // 檢核 dataTag
    if (StringUtils.isNotBlank(request.getDataTag())
        && !request.getDataTag().matches("^[A-Za-z0-9]+$")) {
    ...
    if (StringUtils.isNotBlank(credentialStatus)) {
      VcItemDataValidType vcItemDataValidType =
          VcItemDataValidType.getByCodeIgnoreInactiveAndUnknown(credentialStatus);
    ...
    String format = "yyyy-MM-dd HH:mm:ss";
    Page<GetVcItemDataDTO> resultList =
        vCItemDataRepository.findVcItemData(
            request.getDataTag(), request.getVcUid(), credentialStatus, pageable);
    Page<Dwvc203iVcData> dwvc203iVcData =
        resultList.map(
            credential -> {
