...
public class Ams301wService {
  ...
  public boolean doResetBwd(Ams301ReqDTO req) {
    ...
    user =
        this.userRepository
            .findOneByEmailIgnoreCase(req.getMail())
            .orElseThrow(() -> new RuntimeException("請求失敗，請確認輸入的信箱及聯絡電話是否正確，若有疑問，請洽系統管理員。"));

    // 確認用戶聯絡電話是否正確
    exUser =
        this.extendedUserRepository
            .findOneByUserId(req.getMail())
  ...
  private boolean doResetMail(User user, ExtendedUser exUser) {
    log.debug(
        "Ams301wService-doResetMail，準備發送信件，{}", MailAction.FORGOT_PASSWORD_RESET_BWD.getName());
    // 確認Mail範本。
    ...
    if (!StatusCode.SUCCESS.getCode().equals(criteriaResult.getStatusCode().getCode())) {
      log.error(
          "Ams301wService-doResetMail，寄送mail失敗，msg:{}", criteriaResult.getStatusCode().getMsg());
  ...
  private Ams301wMailResultDTO queryMailTemplateByCriteria() {
    ...
    if (templateDTOList == null || templateDTOList.isEmpty()) {
      log.error(
          "Ams301wService-queryMailTemplateByCriteria，發生錯誤，mailTemplate不存在，criteria:{}", criteria);
