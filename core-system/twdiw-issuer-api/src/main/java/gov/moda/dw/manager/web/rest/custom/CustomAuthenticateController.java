...
public class CustomAuthenticateController {
  ...
  public record JWTToken(
      @JsonProperty("id_token") String idToken, @JsonProperty("detail") String detail) {}
  ...
  public ResponseEntity<JWTToken> authorize(@Valid @RequestBody LoginVM loginVM) {
    ...
    try {
      ...
      Ams311wCurrentUserResultDTO ams311wCurrentUserResultDTO =
          ams311wService.queryPersonalInfo(username);

      if (ams311wCurrentUserResultDTO == null) {
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
      }
      Ams311wCurrentUserResDTO currentUser =
          ams311wCurrentUserResultDTO.getAms311wCurrentUserResDTO();
      ...
      UsernamePasswordAuthenticationToken authenticationToken =
          new AmsUsernamePasswordAuthenticationToken(username, pwd, requestNonceId);
      ...
      String jwt =
          amsAuthenticationProvider.createToken(
              authentication, Optional.ofNullable(loginVM.isRememberMe()).orElse(false));
      ...
      if (e.getCause() instanceof UserNotActivatedException
        ...
        if (errorMessage != null && errorMessage.equals("你的密碼已到期")) {
          ...
          return new ResponseEntity<>(
              new JWTToken(null, "密碼已到期，請至忘記密碼頁面更新密碼"), httpHeaders, HttpStatus.UNAUTHORIZED);
        }
        ...
        if (errorMessage != null && errorMessage.equals("AP 帳號無法進行登入")) {
          HttpHeaders httpHeaders = new HttpHeaders();
          return new ResponseEntity<>(
              new JWTToken(null, "AP 帳號無法進行登入"), httpHeaders, HttpStatus.UNAUTHORIZED);
        }
      }
      return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
    } finally {
  ...
  public ResponseEntity<ResponseDTO<?>> isPasswordValid(@RequestBody IsPasswordValidReqDTO reqDTO) {
    ...
    return switch (result) {
      case SUCCESS,
          LOGIN_IS_PASS_WORD_VALID_RULE_NOT_FOUND,
          LOGIN_IS_PASS_WORD_VALID_NO_HISTORY_RECORD -> ResponseEntity.ok()
