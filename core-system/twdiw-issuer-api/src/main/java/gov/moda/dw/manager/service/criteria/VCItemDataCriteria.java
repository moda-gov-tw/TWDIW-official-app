 ...
 * in {@link DwSandBoxVC301WResource} to receive all the possible filtering options from the Http
 * GET request parameters. For example the following could be a valid request: {@code
 * /vc-item-data?id.greaterThan=5&attr1.contains=something&attr2.specified=false} As Spring is
...
public class VCItemDataCriteria implements Serializable, Criteria {
  ...
  public VCItemDataCriteria(VCItemDataCriteria other) {
    ...
    this.clearScheduleDatetime =
        other.optionalClearScheduleDatetime().map(InstantFilter::copy).orElse(null);
    ...
    this.scheduleRevokeMessage =
        other.optionalScheduleRevokeMessage().map(StringFilter::copy).orElse(null);
