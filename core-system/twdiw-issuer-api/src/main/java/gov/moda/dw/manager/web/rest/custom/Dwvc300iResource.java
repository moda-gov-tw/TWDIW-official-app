...
/** 電子證件撤銷的 REST 控制器 負責處理電子證件的撤銷操作，提供撤銷電子證件的 API 端點 此控制器主要用於處理電子證件的狀態變更，例如撤銷或停用 */
@Slf4j
@RestController
@RequestMapping("/api")
...
public class Dwvc300iResource {
  ...
  /**
   * [DWVC-301] 發行端 VC 卡片狀態變更 撤銷或變更電子證件狀態 此 API 用於處理電子證件的撤銷操作，可以根據不同的 action 參數執行不同的操作 例如：撤銷、停用、復用等
   *
   * @param cid 電子證件識別碼
  ...
  @PutMapping(
      value = "/credential/{cid}/{action}",
      consumes = {"application/json", "application/merge-patch+json"})
  public ResponseEntity<DwissuerVC203iResp> updateVcCredentialStatus(
      @PathVariable("cid") String cid, @PathVariable("action") String action) {
    // 記錄請求資訊
  ...
  /**
   * [DWVC-302] 發行端 VC 多筆卡片狀態變更 例如：撤銷、停用、復用等
   *
  ...
  @PreAuthorize("hasAuthority('vc_removeVC_ExtApi')") // 需要具有撤銷電子證件的權限
  @PutMapping(
      value = "/credential/multiaction",
      consumes = {"application/json", "application/merge-patch+json"})
  public ResponseEntity<Dwvc302iResDTO> multiaction(@RequestBody Dwvc302iReqDTO request) {
    // 記錄請求資訊
    log.debug(
        "REST request to change credential multiaction action: {}, cids : {}",
        request.getAction(),
        request.getCids());
