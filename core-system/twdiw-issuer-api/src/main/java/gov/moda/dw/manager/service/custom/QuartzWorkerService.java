...
public class QuartzWorkerService {
  ...
  @Getter private Map<Long, ScheduleDTO> runScheduleMap = new ConcurrentHashMap<>();
  ...
  public String generateCronString(ScheduleDTO scheduleDTO) {
    ...
    switch (scheduleDTO.getType()) {
      ...
      case "Quarterly":
        return time
            + QuarterlyPattern.replace("$date", scheduleDTO.getDate())
                .replace("$month", scheduleDTO.getMonth());
      default:
        return null;
    }
  }
  ...
  public void addCronJob(
      Class<? extends Job> jobClass, String taskName, String cron, String oriTimezone)
