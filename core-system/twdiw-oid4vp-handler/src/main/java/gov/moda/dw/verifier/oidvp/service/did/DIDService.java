...
public class DIDService {
  ...
  public RegisterDidResult registerVerifierDID(Map<String, Object> org) throws OidvpException {
    ...
    DIDGenerateResponse didGenerateResponse =
        didFrontendService.generateDID(new DIDGenerateRequest(didKey), accessToken);
    if (!didGenerateResponse.isSuccess()) {
      ...
      throw new OidvpException(
          ...
          "call frontend generate did error. (code=" + didGenerateResponse.getCode() + ")");
    }
    Map<String, Object> didDocument =
        Optional.ofNullable(didGenerateResponse.getData())
            ...
            .orElseThrow(
                () ->
                    new OidvpException(
                        OidvpError.CALL_FRONTEND_GENERATE_DID_ERROR,
                        "frontend generate did response with empty didDocument"));

    String did = didFrontendService.extractDidFromDidDocument(didDocument);

    // 2. register DID
    DIDRegisterResponse didRegisterResponse =
        didFrontendService.registerDID(didKey, didDocument, org, accessToken);
    if (!didRegisterResponse.isSuccess()) {
      ...
      throw new OidvpException(
          ...
          "call frontend register did error. (code=" + didRegisterResponse.getCode() + ")");
    }
    String _did =
        Optional.ofNullable(didRegisterResponse.getData())
            ...
            .orElseThrow(
                () ->
                    new OidvpException(
                        OidvpError.CALL_FRONTEND_REGISTER_DID_ERROR,
                        "frontend register did response with empty did"));

    if (!did.equals(_did)) {
      throw new OidvpException(
          OidvpError.CALL_FRONTEND_REGISTER_DID_ERROR,
          "register response did is not matched with generated did");
    }

    // 3. review DID
    DIDReviewResponse didReviewResponse =
        didFrontendService.reviewDID(new DIDReviewRequest(did, true), accessToken);
    if (!didReviewResponse.isSuccess()) {
      ...
      throw new OidvpException(
          ...
          "call frontend review did error. (code=" + didReviewResponse.getCode() + ")");
    }
   ...
   * @throws OidvpException if register DID error.
   */
  public RegisterDidResult registerVerifierDID(VerifierRegisterDidRequest request)
    ...
    if (!didGenerateResponse.isSuccess()) {
      ...
      throw new OidvpException(
          OidvpError.CALL_FRONTEND_GENERATE_DID_ERROR,
          "call frontend generate did error. (code="
              + didGenerateResponse.getCode()
              + ", msg="
              + didGenerateResponse.getMsg()
              + ")");
    }
    Map<String, Object> didDocument =
        Optional.ofNullable(didGenerateResponse.getData())
    ...
    DIDCreateResponse didCreateResponse =
        didFrontendService.createDID(didKey, didDocument, org, p7data, accessToken);
    if (!didCreateResponse.isSuccess()) {
      ...
      throw new OidvpException(
          ...
          "call frontend create did error. (code="
              + didCreateResponse.getCode()
              + ", msg="
              + didCreateResponse.getMsg()
              + ")");
    }
    String _did =
        Optional.ofNullable(didCreateResponse.getData())
            .map(DIDResponseData::getId)
            .filter(didValue -> !didValue.isEmpty())
            .orElseThrow(
                () ->
  ...
  public String getDID() {
    try {
      ...
      throw new OidvpRuntimeException(
          ...
          "get DID error: DID not exist. The DID registration might not have been successfully completed.");
    } catch (SQLException e) {
      LOGGER.error("get DID error: {}", e.getMessage());
      throw new OidvpRuntimeException(
          OidvpError.GET_DID_ERROR, "get DID error: " + e.getMessage(), e);
    }
  }
  ...
  public ECKey getDIDKey() throws OidvpException {
    try {
      ...
      throw new OidvpException(
          OidvpError.GET_DID_ERROR,
          "get DID key error: DID key not exist. The DID registration might not have been successfully completed.");
    } catch (JWKStoreOperationException e) {
