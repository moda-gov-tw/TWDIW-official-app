...
public class DIDFrontendService {
  ...
  public DIDFrontendService(OidvpConfig oidvpConfig, OidvpPropertyDAO oidvpPropertyDAO) {
    Assert.notNull(
        oidvpConfig.getFrontendGenerateDidURI(),
        OidvpConfig.FRONTEND_GENERATE_DID_PREDICATE + " must be specified.");
    Assert.notNull(
        oidvpConfig.getFrontendCreateDidURI(),
        OidvpConfig.FRONTEND_CREATE_DID_PREDICATE + " must be specified.");
    this.oidvpConfig = oidvpConfig;
    this.oidvpPropertyDAO = oidvpPropertyDAO;
    if (oidvpConfig.isHttpProxyEnable()) {
      this.httpUtils =
          new HttpUtils.HttpUtilsBuilder(3, oidvpConfig.getVerifierBusinessHttpTimeout())
              .setProxy(
                  oidvpConfig.getHttpProxyProtocol(),
                  oidvpConfig.getHttpProxyUrl(),
                  oidvpConfig.getHttpProxyPort())
              .build();
    } else {
      this.httpUtils =
          new HttpUtils.HttpUtilsBuilder(3, oidvpConfig.getVerifierBusinessHttpTimeout()).build();
    }
  }

  public DIDGenerateResponse generateDID(DIDGenerateRequest request, String accessToken)
    ...
    try {
      ResponseEntity<DIDGenerateResponse> response =
          httpUtils.doPost(
              frontendGenerateDidURI.toString(), request, authHeader, DIDGenerateResponse.class);
      if (response.getBody() == null) {
        throw new OidvpException(
            OidvpError.CALL_FRONTEND_GENERATE_DID_ERROR, "frontend generate DID response null");
      } else {
        return response.getBody();
      }
    } catch (RestClientResponseException e) {
      String errorMessage =
          "fail to generate DID (http="
      ...
      throw new OidvpException(
          OidvpError.CALL_FRONTEND_GENERATE_DID_ERROR,
          "call frontend generate did error, " + e.getMessage(),
          e);
    }
  }

  @Deprecated
  public DIDRegisterResponse registerDID(
      ECKey didKey, Map<String, Object> didDocument, Map<String, Object> org, String accessToken)
      throws OidvpException {
    if (didKey == null) {
    ...
    DIDRegisterRequest request =
        new DIDRegisterRequest(signedDidDocument.serialize(), VERIFIER_ORG_TYPE, org);
    URI frontendRegisterDidURI = oidvpConfig.getFrontendRegisterDidURI();
    HttpHeaders authHeader = getAuthHeader(accessToken);
    try {
      ResponseEntity<DIDRegisterResponse> response =
          httpUtils.doPost(
              frontendRegisterDidURI.toString(), request, authHeader, DIDRegisterResponse.class);
      if (response.getBody() == null) {
        throw new OidvpException(
            OidvpError.CALL_FRONTEND_REGISTER_DID_ERROR, "frontend register DID response null");
      } else {
        return response.getBody();
      }
    } catch (RestClientResponseException e) {
      String errorMessage =
          "fail to register DID (http="
              + e.getStatusCode().value()
              + ", response=["
              + e.getResponseBodyAsString()
              + "])";
      ...
      throw new OidvpException(
          OidvpError.CALL_FRONTEND_REGISTER_DID_ERROR,
          "call frontend register did error" + e.getMessage(),
          e);
    }
  }

  @Deprecated
  public DIDReviewResponse reviewDID(DIDReviewRequest request, String accessToken)
      throws OidvpException {
    if (request == null) {
    ...
    HttpHeaders authHeader = getAuthHeader(accessToken);
    try {
      ResponseEntity<DIDReviewResponse> response =
          httpUtils.doPost(
              frontendReviewDidURI.toString(), request, authHeader, DIDReviewResponse.class);
      if (response.getBody() == null) {
        throw new OidvpException(
            OidvpError.CALL_FRONTEND_REVIEW_DID_ERROR, "frontend review DID response null");
      } else {
        return response.getBody();
      }
    } catch (RestClientResponseException e) {
      String errorMessage =
          "fail to review DID (http="
              + e.getStatusCode().value()
              + ", response=["
              + e.getResponseBodyAsString()
              + "])";
      ...
      throw new OidvpException(
          OidvpError.CALL_FRONTEND_REVIEW_DID_ERROR,
          "call frontend review did error" + e.getMessage(),
          e);
    }
  }

  public DIDCreateResponse createDID(
      ECKey didKey,
    ...
    DIDCreateRequest didCreateRequest =
        new DIDCreateRequest(signedDidDocument.serialize(), VERIFIER_ORG_TYPE, org, p7data);
    URI frontendCreateDidURI = oidvpConfig.getFrontendCreateDidURI();
    HttpHeaders authHeader = getAuthHeader(accessToken);
    try {
      ResponseEntity<DIDCreateResponse> response =
          httpUtils.doPost(
              frontendCreateDidURI.toString(),
              didCreateRequest,
              authHeader,
              DIDCreateResponse.class);
      if (response.getBody() == null) {
        throw new OidvpException(
            OidvpError.CALL_FRONTEND_CREATE_DID_ERROR, "frontend create DID response null");
      } else {
        return response.getBody();
      }
    } catch (RestClientResponseException e) {
      String errorMessage =
          "fail to create DID (http="
              + e.getStatusCode().value()
              + ", response=["
              + e.getResponseBodyAsString()
              + "]";
      ...
      throw new OidvpException(
          OidvpError.CALL_FRONTEND_CREATE_DID_ERROR,
          "call frontend create did error" + e.getMessage(),
          e);
    }
  }
  ...
  public String extractDidFromDidDocument(Map<String, Object> didDocument) throws OidvpException {
    if (!didDocument.containsKey("id")) {
      throw new OidvpException(
          OidvpError.PARSE_DID_FROM_DOCUMENT_ERROR, "'id' not found in did document");
    }
    String did = (String) didDocument.get("id");
    if (did == null || did.isEmpty()) {
      throw new OidvpException(
          OidvpError.PARSE_DID_FROM_DOCUMENT_ERROR, "empty did in did document");
    }
    return did;
  }
  ...
  public String getAccessToken() {
    try {
      ...
      throw new OidvpRuntimeException(
          OidvpError.REGISTER_DID_ERROR, "get access_token error: " + e.getMessage(), e);
    }
  }
