...
public class OidvpConfig {
  ...
  public static final String VP_PRESENTATION_VALIDATION_URI_PREDICATE =
      "url.vp.presentation-validation";
  ...
  public static final String VERIFY_RESULT_DELETE_AFTER_QUERY_PREDICATE =
      "verify-result.delete-after-query";
  public static final String VP_PRESENTATION_VALIDATION_HTTP_TIMEOUT_PREDICATE =
      "vp.presentation-validation.http-timeout";
  ...
  public static final String FRONTEND_SERVICE_TIMEOUT_HTTP_PREDICATE =
      "frontend.service.http-timeout";
  public static final String FRONTEND_CREATE_DID_PREDICATE = "url.frontend.create-did";

  // verifier business service
  public static final String VERIFIER_BUSINESS_HTTP_TIMEOUT_PREDICATE =
      "verifierBusiness.http-timeout";
  public static final String CALLBACK_DOMAIN_LIST_PREDICATE =
      "callback.domain-list"; // split by ","
  ...
  private void buildProperty() throws Exception {
    try {
      ...
      String oidvpDomainUri =
          _oidvpDomainUri.endsWith("/") ? _oidvpDomainUri : _oidvpDomainUri.concat("/");

      clientId = getPropertyValue(CLIENT_ID_PREDICATE);
      redirectUri =
          new URI(
              Optional.ofNullable(getPropertyValue(REDIRECT_URI_PREDICATE))
      ...
      sessionExpiredTime =
          Integer.valueOf(
              Optional.ofNullable(getPropertyValue(SESSION_EXPIRED_TIME_PREDICATE)).orElse("5"));
      // unit: minute
      verifyResultExpiredTime =
          Integer.valueOf(
              Optional.ofNullable(getPropertyValue(VERIFY_RESULT_EXPIRED_TIME_PREDICATE))
                  .orElse("5"));
      verifyResultDeleteAfterQuery =
          Boolean.parseBoolean(
              Optional.ofNullable(getPropertyValue(VERIFY_RESULT_DELETE_AFTER_QUERY_PREDICATE))
                  .orElse("true"));
      sessionDeleteCron =
          Optional.ofNullable(getPropertyValue(SESSION_DELETE_CRON_PREDICATE))
              .orElse("0 0 2 * * ?");
      verifyResultDeleteCron =
          Optional.ofNullable(getPropertyValue(VERIFY_RESULT_DELETE_CRON_PREDICATE))
              .orElse("0 0 * * * ?");

      // unit: second
      vpPresentationValidationHttpTimeout =
          Integer.valueOf(
              Optional.ofNullable(
                      getPropertyValue(VP_PRESENTATION_VALIDATION_HTTP_TIMEOUT_PREDICATE))
                  .orElse("30"));
      frontendServiceHttpTimeout =
          Integer.valueOf(
              Optional.ofNullable(getPropertyValue(FRONTEND_SERVICE_TIMEOUT_HTTP_PREDICATE))
                  .orElse("5"));
      verifierBusinessHttpTimeout =
          Integer.valueOf(
              Optional.ofNullable(getPropertyValue(VERIFIER_BUSINESS_HTTP_TIMEOUT_PREDICATE))
                  .orElse("60"));
      ...
      httpProxyEnable =
          Boolean.parseBoolean(
              Optional.ofNullable(getPropertyValue(HTTP_PROXY_ENABLE_PREDICATE)).orElse("false"));
      if (httpProxyEnable) {
        httpProxyProtocol =
            Optional.ofNullable(getPropertyValue(HTTP_PROXY_PROTOCOL_PREDICATE))
                .filter(p -> p.equalsIgnoreCase("http") || p.equalsIgnoreCase("https"))
                .orElse("http");
        httpProxyUrl =
            Optional.ofNullable(getPropertyValue(HTTP_PROXY_URL_PREDICATE)).orElse("127.0.0.1");
        httpProxyPort =
            Integer.valueOf(
                Optional.ofNullable(getPropertyValue(HTTP_PROXY_PORT_PREDICATE)).orElse("8080"));
      }

      String callbackDomainListValue = getPropertyValue(CALLBACK_DOMAIN_LIST_PREDICATE);
      callbackDomainList =
          callbackDomainListValue == null
              ? Collections.emptyList()
              : Arrays.stream(callbackDomainListValue.split(",")).toList();
