...
public class PresentationSubmissionValidator extends AbstractValidator<PresentationSubmission> {
  ...
  public ValidateResult validate(PresentationSubmission ps) {
    ...
    if (ps.getDefinitionId() == null || ps.getDefinitionId().isEmpty()) {
      return new ValidateResult(
          false, "presentation_submission 'definition_id' should not be empty");
    }
  ...
  private static String getMessage(Result result) {
    ...
    switch (result.errorCode()) {
      case ID_OF_EACH_LEVEL_IS_NOT_THE_SAME -> message =
          "invalid presentation_submission: 'descriptor_map id' is not the same for each level of nesting";

      case NESTED_LEVEL_TOO_BIG -> message =
          "invalid presentation_submission: nesting path_nested level bigger than 1";

      case INVALID_JSONPATH -> message =
          "invalid presentation_submission: 'path' is not the valid jsonpath";
    }
    return message;
  }
