...
public class CredentialFormatHandler extends PresentationDefinitionHandler {
  ...
  private void checkFormat(
      InputDescriptor inputDescriptor, FormatRegistry vpFormat, FormatRegistry vcFormat)
      throws VcValidatedException {
  ...
  private void checkIsAcceptedFormat(
      Map<FormatRegistry, FormatDescription> acceptedFormat,
    ...
    if (!formatCheckResult.vcMatched()) {
      String message =
          "vc format("
              + vcFormat.getValue()
              + ") is not match the requirement of presentation_definition";
  ...
  private FormatCheckResult checkIsFormatMatched(
      Map<FormatRegistry, FormatDescription> acceptedFormat,
      FormatRegistry vpFormat,
      FormatRegistry vcFormat) {
    ...
    return new FormatCheckResult(
        !isConstrainContainVp || vpMatched, !isConstrainContainVc || vcMatched);
  }
