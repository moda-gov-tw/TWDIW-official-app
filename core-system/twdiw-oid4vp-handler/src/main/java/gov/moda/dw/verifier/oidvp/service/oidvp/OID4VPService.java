...
public class OID4VPService {
  ...
  public JWT getRequestObject(
      String clientId, String nonce, String state, String pd, AuthorizationRequestOpts opts)
    ...
    if (opts.getExpiredTime() == null) {
      throw new BadOidvpParamException(
          "'expiredTime' in AuthorizationRequestOpts must be specified when creating request object.");
    }
    PresentationDefinition presentationDefinition =
        PresentationPOJOUtils.getPresentationDefinition(pd);
    return getRequestObject(
        clientId, new Nonce(nonce), new State(state), presentationDefinition, opts);
  }

  private OidvpAuthorizationRequest getAuthorizationRequest(
      String transactionId,
      ClientIdScheme clientIdScheme,
      String ref,
      JARType jarType,
    ...
    if (opts.getCallbackUri() != null
        && !verifierBusinessService.validateCallbackURI(opts.getCallbackUri())) {
    ...
    if (opts.getExpiredTime() == null) {
      ...
      if (opts.getExpiredTime().isBefore(LocalDateTime.now())) {
        throw new BadOidvpParamException(
            "expiredTime for AuthorizationRequest must not before current time.");
      }
  ...
  private OidvpAuthorizationRequest getAuthorizationRequestUsingRequestURI(
      ClientIdScheme clientIdScheme, URI requestURI, AuthorizationRequestOpts opts) {
    LOGGER.info(
        "build AuthorizationRequest using request_uri...client_id_scheme=[{}]",
        clientIdScheme.getValue());
    AuthorizationRequestParam requestParam = buildRequestParamForJAR(null, requestURI, opts);
    return oid4VP.getAuthorizationRequest(clientIdScheme, requestParam);
  }

  private OidvpAuthorizationRequest getAuthorizationRequestUsingRequest(
      ClientIdScheme clientIdScheme, JWT requestObject, AuthorizationRequestOpts opts) {
    LOGGER.info(
        "build AuthorizationRequest using request...client_id_scheme=[{}]",
        clientIdScheme.getValue());
    AuthorizationRequestParam requestParam = buildRequestParamForJAR(requestObject, null, opts);
    return oid4VP.getAuthorizationRequest(clientIdScheme, requestParam);
  }

  private JWT getRequestObject(
      String clientId,
      Nonce nonce,
      State state,
      PresentationDefinition pd,
  ...
  private AuthorizationRequestParam buildRequestParam(
      Nonce nonce,
      State state,
      PresentationDefinition pd,
  ...
  private AuthorizationRequestParam buildRequestParamForJAR(
      JWT requestObject, URI requestURI, AuthorizationRequestOpts opts) {
  ...
  private VerifierMetadata getMetadata() throws OidvpException {
    try {
      ...
      throw new OidvpException(
          OidvpError.GET_METADATA_ERROR, "get metadata error: " + e.getMessage(), e);
    }
  }

  private PresentationDefinitionDTO getPresentationDefinitionFromVpItem(String ref)
      throws SQLException, PresentationEvaluationException {
    String presentationDefinition = presentationDefinitionDao.getPresentationDefinition(ref);
    PDSchemaResult result =
        presentationSchemaUtils.isValidPresentationDefinitionSchema(presentationDefinition);
    if (result.isValid()) {
  ...
  private URI getRequestUri(URI baseRequestUri, State state) {
    ...
    return UriComponentsBuilder.fromUri(baseRequestUri).path(state.getValue()).build().toUri();
  }
