...
public class HttpUtils {
  ...
  public <T> ResponseEntity<T> doPost(
      String url, Object body, HttpHeaders headers, Class<T> responseType) {
    return doPost(url, body, headers, responseType, (Object) null);
  }

  private <T> ResponseEntity<T> doPost(
      String url, Object body, HttpHeaders headers, Class<T> responseType, Object... uriVariables) {
  ...
  public static class CustomHttpRequestRetryStrategy extends DefaultHttpRequestRetryStrategy {
    ...
    private static final List<Class<? extends IOException>> nonRetriableIOExceptionClasses =
        Arrays.asList(
  ...
  public static class HttpUtilsBuilder {
    ...
    public HttpUtilsBuilder setRetry(int retryTimes, int duration) {
      if (retryTimes > 1 && duration > 0) {
        this.httpClientBuilder.setRetryStrategy(
            new CustomHttpRequestRetryStrategy(
                retryTimes, TimeValue.of(duration, TimeUnit.SECONDS)));
      }
      return this;
    }
