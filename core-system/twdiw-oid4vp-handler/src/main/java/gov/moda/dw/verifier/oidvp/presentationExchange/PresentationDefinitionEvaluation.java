...
public class PresentationDefinitionEvaluation {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(PresentationDefinitionEvaluation.class);

  /**
   * To evaluate vp(decoded jwt_vp/jwt_vp_json、ldp_vp)、vc(decoded jwt_vc/jwt_vc_json、ldp_vc) against
   * the presentation_definition.
   *
   * <p>NOTE: Some credential format and Relational Constraint Feature in presentation_definition
   ...
   * @return if vc is satisfied the presentation_definition will return
   *     PresentationDefinitionHandlerResult, otherwise will throw the exception.
   * @throws PresentationEvaluationException when presentation_definition or presentation_submission
   *     is in the wrong format.
   * @throws VcValidatedException if vc didn't satisfy the presentation_definition.
   */
  public PresentationDefinitionHandlerResult evaluatePresentations(
      PresentationDefinition pd, PresentationSubmission ps, Map<CredentialPath, CredentialInfo> vcs)
  ...
  private PresentationDefinitionHandlerResult evaluateAndCheckPresentations(
      PresentationDefinition pd, PresentationSubmission ps, Map<CredentialPath, CredentialInfo> vcs)
      throws VcValidatedException, PresentationEvaluationException {
    if (!isDefinitionIdMatchedPresentationDefinition(pd, ps)) {
      throw new VcValidatedException(
          OidvpError.PS_NOT_MATCH_PD,
          "invalid presentation_submission's definition_id: not match with presentation_definition's id");
    }
    ...
    if (!isVcNumberMatchPresentationSubmission(psDTOs, vcs)) {
      throw new VcValidatedException(
          OidvpError.CREDENTIAL_NUMBER_NOT_MATCH,
          "number of vp/vc doesn't match the vp/vc described in presentation_submission");
    }
  ...
  private PresentationDefinitionHandlerResult evaluatePresentationsAgainstSubmission(
      PresentationDefinition pd, List<SubmissionInfo> submissionInfos)
      throws VcValidatedException, PresentationEvaluationException {
    ...
    PresentationDefinitionHandlerResult evaluationResult =
        handlerClient.evaluate(pd, submissionInfos);
    LOGGER.info("[check is vc satisfied presentation_definition]: PASS");
    return evaluationResult;
  }

  public boolean isDefinitionIdMatchedPresentationDefinition(
      PresentationDefinition pd, PresentationSubmission ps) {
  ...
  private List<InputDescriptor> validateIsSubmissionSatisfySubmissionRequirements(
      PresentationDefinition pd, List<SubmissionInfo> submissionInfos)
      throws VcValidatedException, PresentationEvaluationException {
    Map<String, Integer> groupCount = parseGroupCountMap(pd);
    try {
      EvaluateRequirementsResult result =
          evaluateSubmissionRequirements(
              pd.getSubmissionRequirements(), groupCount, submissionInfos);
      if (result.errorInfoList().isEmpty()) {
        return result.selected();
      } else {
        result.errorInfoList().forEach(error -> LOGGER.warn(error.getEvaluationErrorMessage()));
        throw new VcValidatedException(
            OidvpError.VC_NOT_MATCH_PRESENTATION_DEFINITION,
            "received vc not satisfy submission_requirements");
      }
  ...
  private EvaluateRequirementsResult evaluateSubmissionRequirements(
    ...
    return evaluateSubmissionRequirements(
        submissionRequirements, groupCount, submissionInfos, nestedIndexStack);
  }
  ...
  private EvaluateRequirementsResult evaluateSubmissionRequirements(
      ...
      Map<String, Integer> groupCount,
      List<SubmissionInfo> submissionInfos,
      Deque<Integer> nestedIndexStack)
      throws PresentationEvaluationException {
    int matched = 0;
    ...
    for (int i = 0; i < submissionRequirements.size(); i++) {
      ...
      if (sr.getFrom() != null && !sr.getFrom().isBlank()) {
        ...
        if (errors.isEmpty()) {
          ...
          errorInfoList.add(
              new RequirementsErrorMessage(new ArrayDeque<>(nestedIndexStack), i, errors));
        }
      } else if (sr.getFromNested() != null && !sr.getFromNested().isEmpty()) {
        nestedIndexStack.addLast(i);
        EvaluateRequirementsResult recursiveResult =
            evaluateSubmissionRequirements(
                sr.getFromNested(), groupCount, submissionInfos, nestedIndexStack);
        ...
        throw new PresentationEvaluationException(
            OidvpError.INVALID_PRESENTATION_DEFINITION,
            "invalid submission_requirement: must contain either 'from' or 'from_nested'");
      }
  ...
  private List<InputDescriptor> getMatchedInputDescriptor(
      SubmissionRequirement sr, List<SubmissionInfo> submissionInfos)
      throws PresentationEvaluationException {
    ...
    for (SubmissionInfo submissionInfo : submissionInfos) {
      ...
      if (inputDescriptor.getGroup() == null) {
        throw new PresentationEvaluationException(
            OidvpError.INVALID_PRESENTATION_DEFINITION,
            "invalid input_descriptor: submission_requirements is present, input_descriptor must contain 'group'");
      }
  ...
  private List<CountErrorInfo> checkCount(
      SubmissionRequirement sr, int count, Map<String, Integer> groupCount)
      throws PresentationEvaluationException {
    ...
    if (SubmissionRequirementRule.PICK.equals(sr.getRule())) {
      ...
      if (_count != null) {
        if (_count < 1) {
          throw new PresentationEvaluationException(
              OidvpError.INVALID_PRESENTATION_DEFINITION,
              "invalid submission_requirement: the value of 'count' is invalid");
      ...
      if (min != null) {
        if (min < 0) {
          throw new PresentationEvaluationException(
              OidvpError.INVALID_PRESENTATION_DEFINITION,
              "invalid submission_requirement: the value of 'min' is invalid");
        } else if (min > count) {
      ...
      if (max != null) {
        if (max < 1) {
          throw new PresentationEvaluationException(
              OidvpError.INVALID_PRESENTATION_DEFINITION,
              "invalid submission_requirement: the value of 'max' is invalid");
        } else if (max < count) {
      ...
      throw new PresentationEvaluationException(
          OidvpError.INVALID_PRESENTATION_DEFINITION,
          "invalid submission_requirement: invalid 'rule' value");
    }
    return errorInfos;
  }

  private ArrayList<PresentationSubmissionDTO> parsePresentationSubmission(
      PresentationSubmission ps) throws PresentationEvaluationException {
    ...
    for (DescriptorMap descriptorMap : ps.getDescriptorMap()) {
      ...
      FormatRegistry vcFormat =
          Optional.ofNullable(descriptorMap.getPathNested())
      ...
      String vpPath =
          Optional.ofNullable(descriptorMap.getPath())
              .orElseThrow(
                  () ->
                      new PresentationEvaluationException(
                          OidvpError.INVALID_PRESENTATION_SUBMISSION));
      String vcPath =
          Optional.ofNullable(descriptorMap.getPathNested())
  ...
  private boolean isVcNumberMatchPresentationSubmission(
      List<PresentationSubmissionDTO> psDTOs, Map<CredentialPath, CredentialInfo> vcs) {
    List<CredentialPath> distinctVcList =
        psDTOs.stream().map(PresentationSubmissionDTO::credentialPath).distinct().toList();
    return distinctVcList.size() == vcs.size();
  }

  private InputDescriptor checkIsPresentationDefinitionContainInputDescriptor(
      PresentationDefinition pd, PresentationSubmissionDTO psDTO)
      throws PresentationEvaluationException {
  ...
  private ArrayList<SubmissionInfo> toSubmissionInfo(
      PresentationDefinition pd,
      List<PresentationSubmissionDTO> psDTOs,
      Map<CredentialPath, CredentialInfo> vcs)
      throws PresentationEvaluationException, VcValidatedException {
    ...
    for (PresentationSubmissionDTO psDTO : psDTOs) {
      ...
      InputDescriptor inputDescriptor =
          checkIsPresentationDefinitionContainInputDescriptor(pd, psDTO);
      ...
      submissionInfos.add(
          new SubmissionInfo(
              inputDescriptor,
              vpFormat,
              new DecodedVerifiableCredential(
                  vcPath, vcFormat, vc, credentialInfo.limitDisclosure())));
    }
    return submissionInfos;
  }

  // record
  public record PresentationSubmissionDTO(
      CredentialPath credentialPath, DescriptorMapInfo descriptorMapInfo) {}
  ...
  public record SubmissionInfo(
      InputDescriptor inputDescriptor,
      FormatRegistry vpFormat,
      DecodedVerifiableCredential decodedVc) {
  ...
  private record EvaluateRequirementsResult(
      int matched, List<InputDescriptor> selected, List<RequirementsErrorMessage> errorInfoList) {}
}
