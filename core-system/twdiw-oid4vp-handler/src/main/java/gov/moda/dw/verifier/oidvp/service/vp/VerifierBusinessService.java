...
public class VerifierBusinessService {
  ...
  public VerifierBusinessService(OidvpConfig oidvpConfig) {
    httpUtils =
        new HttpUtils.HttpUtilsBuilder(3, oidvpConfig.getVerifierBusinessHttpTimeout())
            .setRetry(retryTimes, duration)
            .build();
    VALID_CALLBACK_DOMAIN_LIST =
        oidvpConfig.getCallbackDomainList() == null || oidvpConfig.getCallbackDomainList().isEmpty()
            ? DEFAULT_CALLBACK_DOMAIN_LIST
            : oidvpConfig.getCallbackDomainList();
  }

  @Async("CallbackTaskExecutor")
  public void doVerifyResultCallbackAsync(
      String callbackUrl, VerifyResult verifyResult, String transactionId, String vpUid) {
    try {
  ...
  public VerifierBusinessCallbackResponse doVerifyResultCallback(
      String callbackUrl, VerifyResult verifyResult, String transactionId, String vpUid)
    ...
    VerifierBusinessCallbackRequest verifierBusinessCallbackRequest =
        new VerifierBusinessCallbackRequest(transactionId, verifyResult, vpUid);
    try {
      ResponseEntity<VerifierBusinessCallbackResponse> responseEntity =
          httpUtils.doPost(
              callbackUrl,
              verifierBusinessCallbackRequest,
              null,
              VerifierBusinessCallbackResponse.class);
      VerifierBusinessCallbackResponse response = responseEntity.getBody();
      if (response == null) {
        throw new OidvpException(
            OidvpError.VERIFIER_BUSINESS_CALLBACK_RESPONSE_ERROR,
            "verifier business callback response null");
      }
      return response;
      ...
      VerifierBusinessCallbackResponse response =
          Optional.of(e.getResponseBodyAsString())
              ...
              .orElseThrow(
                  () -> {
                    LOGGER.error(
                        "call verifier business callback error, http={}, response={}",
                        e.getStatusCode().value(),
                        e.getResponseBodyAsString());
                    return new RestClientException(
                        "call verifier business callback error.(http="
                            + e.getStatusCode().value()
                            + ", response=["
                            + e.getResponseBodyAsString()
                            + "])",
      ...
      throw new OidvpException(
          OidvpError.VERIFIER_BUSINESS_CALLBACK_RESPONSE_ERROR,
          "verifier business callback response error, (http="
              + e.getStatusCode().value()
              + ", code="
              + response.getCode()
              + ", message="
      ...
      LOGGER.error(
          "call verifier business callback error(callback url={}), {}",
          callbackUrl,
          e.getMessage());
      throw new OidvpException(
          OidvpError.CALL_VERIFIER_BUSINESS_CALLBACK_ERROR,
          "call verifier business callback error, " + e.getMessage(),
          e);
    }
  }
