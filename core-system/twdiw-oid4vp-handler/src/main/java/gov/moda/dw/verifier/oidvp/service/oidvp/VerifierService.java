...
public class VerifierService {
  ...
  public VerifierService(
      ...
      PresentationDefinitionDao presentationDefinitionDao,
      PresentationDefinitionEvaluation presentationDefinitionEvaluation,
      PresentationSchemaUtils presentationSchemaUtils,
      PresentationValidationService presentationValidationService,
      VerifyResultDao verifyResultDao,
      VerifierBusinessService verifierBusinessService) {
  ...
  public VerifyResult verify(
      SessionJpa session, AuthorizationResponse authorizationResponse, String responseCode)
      throws SQLException {
    ...
    if (authorizationResponse instanceof OidvpAuthorizationResponse oidvpAuthorizationResponse) {
      ...
      throw new UnsupportedOperationException(
          "unsupported AuthorizationResponse type: " + authorizationResponse.getClass());
    }
  ...
  public VerifyResult verify(
      VerifyInfoDTO verifyInfoDTO, OidvpAuthorizationResponse authorizationResponse) {
    VerifyResult verifyResult;
    if (authorizationResponse.isSuccess()) {
      verifyResult =
          verifyPresentation(
              verifyInfoDTO,
              authorizationResponse.getVpToken(),
              authorizationResponse.getPresentationSubmission());
    } else { // wallet return error
      String _error = StringUtils.removeIllegalChars(authorizationResponse.getError());
      String _errorDescription =
          StringUtils.removeIllegalChars(authorizationResponse.getErrorDescription());
      LOGGER.error("wallet return error: {}, {}", _error, _errorDescription);
      String errorMessage =
          "wallet response error: "
              + Optional.ofNullable(_errorDescription).map(ed -> _error + ";" + ed).orElse(_error);
  ...
  public VerifyResult verifyPresentation(
      VerifyInfoDTO verifyInfoDTO, String vpToken, String presentationSubmission) {
    ...
    try {
      ...
      PSSchemaResult psResult =
          presentationSchemaUtils.isValidPresentationSubmissionSchema(presentationSubmission);
      if (psResult.isValid()) {
        ps = psResult.ps();
      } else {
        String errorMessage = "invalid presentation_submission: " + psResult.message();
        throw new PresentationEvaluationException(
            OidvpError.INVALID_PRESENTATION_SUBMISSION, errorMessage);
      }
      ...
      List<VpValidatedResult> vpValidatedResults =
          presentationValidationService.validateVpToken(
              oidvpConfig.getVpVerifyUri(), vpTokens, verifyInfoDTO.transactionId());
      ...
      PresentationDefinitionHandlerResult evaluationResult =
          presentationDefinitionEvaluation.evaluatePresentations(pd, ps, vcs);
      LOGGER.info("[check if vc match presentation_definition]: PASS");

      LOGGER.info("verify success!");
      List<VcResponseObjectDTO> vcClaims =
          VcResponseObjectDTO.toVcResponseObjectDTOs(evaluationResult);
  ...
  public VerifyResult getVerifyResult(String transactionId, String responseCode)
      throws SQLException {
    ...
    if (responseCode != null) {
      verifyResultJpa =
          verifyResultDao.getVerifyResultNotExpiredByResponseCode(
              responseCode, verifyResultAliveTime);
      if (!transactionId.equals(verifyResultJpa.getTransactionId())) {
        throw new InvalidParameterException("invalid transaction_id");
      }
    } else {
      verifyResultJpa =
          verifyResultDao.getVerifyResultNotExpired(transactionId, verifyResultAliveTime);
    }
  ...
  public Response modifyPresentationDefinitionData(PresentationDataRequest request)
      throws SQLException {
    if (request == null) {
    ...
    if (Mode.save.equals(mode)) {
      ...
      PDSchemaResult result =
          presentationSchemaUtils.isValidPresentationDefinitionSchema(pdJsonNode);
      if (!result.isValid()) {
        LOGGER.error(
            "invalid presentation_definition - business_id={}, serial_no={}", businessId, serialNo);
      ...
      presentationDefinitionDao.savePresentationDefinition(
          businessId, serialNo, presentationDefinition);
      LOGGER.info(
          "save presentation_definition success - business_id={}, serial_no={}, presentation_definition={}",
          businessId,
          serialNo,
      ...
      LOGGER.info(
          "delete presentation_definition success - business_id={}, serial_no={}",
          businessId,
          serialNo);
    }
  ...
  private VerifyResultJpa saveVerifyResult(
      SessionJpa sessionJpa, String responseCode, VerifyResult verifyResult) throws SQLException {
    ...
    VerifyResultJpa result =
        verifyResultDao.updateSessionAndSaveResult(sessionJpa, responseCode, verifyResult);
  ...
  private void doVerifyResultCallbackAsync(SessionJpa session, VerifyResult verifyResult) {
    ...
    verifierBusinessService.doVerifyResultCallbackAsync(
        session.getCallback(), verifyResult, session.getTransactionId(), session.getRef());
  }

  private boolean validateClientIdAndNonce(
      List<VpValidatedResult> vpValidatedResults, VerifyInfoDTO verifyInfoDTO)
      throws VpValidatedException {
    for (VpValidatedResult validatedResult : vpValidatedResults) {
      if (validatedResult.getClientId() == null) {
        throw new VpValidatedException(
            OidvpError.ID_NONCE_NOT_MATCH, "vp does not contain client_id");
      }
  ...
  private HashMap<CredentialPath, CredentialInfo> toCredentialMap(
      VpValidatedResult vpValidatedResult) throws VpValidatedException {
    ...
    for (VcData vcData : vpValidatedResult.getVcs()) {
      ...
      if (vc == null || vc.isNull()) {
        throw new VpValidatedException(
            OidvpError.VP_RESPONSE_ERROR, "vp validate response is not contained 'credential'");
      }
      ...
      if (limitDisclosureSupported == null) {
        throw new VpValidatedException(
            OidvpError.VP_RESPONSE_ERROR,
            "vp validate response is not specified 'LimitDisclosureSupported'");
      }
  ...
  private HashMap<CredentialPath, CredentialInfo> toCredentialMap(
      List<VpValidatedResult> vpValidatedResults) throws VpValidatedException {
    HashMap<CredentialPath, CredentialInfo> vcs = new HashMap<>();
  ...
  private VerifyResult validateCustomData(
      VerifyResult verifyResult, OidvpAuthorizationResponse authorizationResponse) {
    if (verifyResult.getVerifyResult() && authorizationResponse.getCustomData() != null) {
      ...
      CustomDataValidateResult validateResult =
          CustomDataValidator.validate(verifyResult.getHolderDid(), customData);
      if (validateResult.validateResult()) {
        ...
        return VerifyResult.fail(
            verifyResult.getHolderDid(),
            OidvpError.INVALID_CUSTOM_DATA,
  ...
  public record VerifyInfoDTO(
    ...
    public VerifyInfoDTO(SessionJpa sessionJpa) {
      this(
          sessionJpa.getTransactionId(),
          sessionJpa.getClientId(),
          sessionJpa.getNonce(),
          sessionJpa.getPresentationDefinition(),
          sessionJpa.getRef());
    }
  }
}
