...
public class OidvpAuthorizationResponse extends AuthorizationResponse {
  ...
  public static OidvpAuthorizationResponse parse(Map<String, List<String>> params) {
    ...
    return new OidvpAuthorizationResponse(
        state, vpToken, presentationSubmission, error, errorDescription, customData);
  }

  public OidvpAuthorizationResponse(
      String state,
      String vpToken,
    ...
    if (!StringUtils.hasText(vpToken) && !StringUtils.hasText(error)) {
      throw new IllegalArgumentException(
          "'vp_token' and 'error' must not be null at the same time.");
    }

    if (vpToken != null && error != null) {
      throw new IllegalArgumentException(
          "'vp_token' and 'error' cannot be present at the same time.");
    }
  ...
  public String toString() {
    ...
    if (customData != null) {
      sb.append("\"custom_data\":\"").append(customData).append('\"');
    }
    if (vpToken != null) {
      sb.append(",\"vp_token\":\"").append(vpToken).append('\"');
    }
    if (presentationSubmission != null) {
      sb.append(",\"presentation_submission\":\"").append(presentationSubmission).append('\"');
    }
    if (state != null) {
