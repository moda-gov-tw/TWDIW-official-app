  ...
  public static JWK extractPublicKeyFromDID(String did) throws OidvpException {
    ...
    if (parts.length != 3) {
      throw new OidvpRuntimeException(
          OidvpError.PARSE_DID_ERROR, "the did format not match did:key:xxxxx");
    }
    ...
    try {
      ...
      if (mbBytes.length == 0 || !Arrays.equals(Arrays.copyOfRange(mbBytes, 0, 3), prefix)) {
        throw new OidvpRuntimeException(
            OidvpError.PARSE_DID_ERROR, "fail to decode issuer's DID (invalid hex data)");
      }
