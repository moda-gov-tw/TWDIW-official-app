...
public class SubmissionRequirementsValidator
    extends AbstractValidator<List<SubmissionRequirement>> {
  ...
  private ValidateResult validateSubmissionRequirement(
    ...
    for (SubmissionRequirement sr : submissionRequirements) {
      if (sr.getFrom() != null) {
        ...
        return new ValidateResult(
            false, "submission_requirement must have 'from' or 'from_nested' property");
      }
  ...
  private ValidateResult validateSubmissionRequirement(SubmissionRequirement sr) {
    ...
    //        if (rule == null) {
    //            return new ValidateResult(false, "invalid submission_requirement: 'rule' must be
    // present");
    //        }
    //
    //        if (rule.equals(SubmissionRequirementRule.PICK) && sr.getCount() != null &&
    // sr.getCount() <= 0) {
    //            return new ValidateResult(false, "invalid submission_requirement: 'count' must
    // greater than 0");
    //        }
    //        if (rule.equals(SubmissionRequirementRule.PICK) && sr.getMin() != null && sr.getMin()
    // < 0) {
    //            return new ValidateResult(false, "invalid submission_requirement: 'min' must be
    // positive");
    //        }
    //        if (rule.equals(SubmissionRequirementRule.PICK) && sr.getMax() != null && sr.getMax()
    // <= 0) {
    //            return new ValidateResult(false, "invalid submission_requirement: 'max' must
    // greater than 0");
    //        }
    ...
    if (rule.equals(SubmissionRequirementRule.PICK)
      ...
      return new ValidateResult(
          false,
          "invalid submission_requirement: 'rule.pick' should have property of 'count','min','max'");
    }
