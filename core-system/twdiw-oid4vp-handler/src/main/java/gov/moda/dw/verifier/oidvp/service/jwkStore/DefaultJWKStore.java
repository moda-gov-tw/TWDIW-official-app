...
public class DefaultJWKStore implements JWKStore {
  ...
  public DefaultJWKStore(OidvpPropertyDAO oidvpPropertyDAO, String encryptKey) {
    ...
    if (encryptKey == null) {
      ...
      throw new IllegalArgumentException(
          "invalid encryptKey length, encryptKey's length must be 32.");
    } else {
  ...
  private String encryptJWK(JWK jwk) throws JWKStoreOperationException {
    try {
      byte[] encrypted =
          AESUtils.encryptGCM(
              encryptKey.getBytes(StandardCharsets.UTF_8), jwk.toJSONString(), null);
  ...
  private void initCheck() {
    ...
    try {
      ...
      throw new IllegalStateException(
          "check existed jwk error: query db error, " + e.getMessage(), e);
    }
      ...
      if (property.getKey().startsWith(JWK_PREDICATE)) {
        ...
        if (jwkValue.startsWith("{") && jwkValue.contains("\"kty\":")) {
          try {
            ...
            throw new IllegalStateException(
                "check existed jwk error: encrypted existed raw jwk error, " + e.getMessage(), e);
          }
        } else {
          try {
            ...
            throw new IllegalStateException(
                "check existed jwk error: parse existed encrypted jwk error, " + e.getMessage(), e);
          }
        }
      }
    }
