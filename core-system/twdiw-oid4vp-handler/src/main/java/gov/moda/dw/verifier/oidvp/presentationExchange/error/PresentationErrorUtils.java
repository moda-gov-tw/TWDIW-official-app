...
public class PresentationErrorUtils {
  ...
  public static ClaimErrorMessage getErrorMessage(
      ErrorType type,
      SubmissionInfo submissionInfo,
      List<String> paths,
      DocumentContext jsonDocument) {
    String requiredClaimName =
        Optional.ofNullable(paths.get(0))
            .map(n -> n.split("\\."))
            .map(s -> s[s.length - 1])
            .orElse(null);
    String vcType = getVcType(submissionInfo.decodedVc(), jsonDocument);
    return new ClaimErrorMessage(
        type, requiredClaimName, submissionInfo.inputDescriptor().getId(), vcType);
  }
  ...
  private static String getVcType(DecodedVerifiableCredential vc, DocumentContext vcJsonDocument) {
    DocumentContext _vcJsonDocument =
        (vcJsonDocument == null)
            ? JsonPathUtils.getJsonDocument(vc.getJsonString())
            : vcJsonDocument;
