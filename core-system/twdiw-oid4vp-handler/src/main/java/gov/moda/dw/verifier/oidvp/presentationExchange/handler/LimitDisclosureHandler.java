...
public class LimitDisclosureHandler extends PresentationDefinitionHandler {
  ...
  public void handle(PresentationDefinition pd, List<SubmissionInfo> submissionInfos)
    ...
    for (SubmissionInfo submissionInfo : submissionInfos) {
      ...
      DisclosureLimitation disclosureLimitation =
          Optional.of(submissionInfo)
              .map(SubmissionInfo::inputDescriptor)
              .map(InputDescriptor::getConstraints)
              .map(Constraints::getLimitDisclosure)
              .orElse(null);

      if (!isVcSupportedLimitDisclosure
          && DisclosureLimitation.REQUIRED.equals(disclosureLimitation)) {
        final String message =
            "received vc is not supported limit_disclosure but presentation_definition's limit_disclosure is 'required'";
      ...
      // complement claims to result when not setting limit_disclosure or limit_disclosure is
      // preferred
  ...
  private ObjectNode getVcCredentialSubject(SubmissionInfo submissionInfo)
      throws VcValidatedException {
    // for vc
    switch (submissionInfo.decodedVc().getFormat()) {
      case LDP_VC, JWT_VC, JWT_VC_JSON -> {
        JsonNode credentialSubject =
            Optional.of(submissionInfo.decodedVc())
                .map(DecodedVerifiableCredential::getCredential)
                .map(c -> c.get(VC_CREDENTIAL_SUBJECT_PREDICATE))
                .orElseThrow(
                    () ->
                        new VcValidatedException(
                            OidvpError.INVALID_VC, "can not find 'credentialSubject' field in vc"));
        ...
        if (credentialSubject.isObject()) {
          ...
          throw new VcValidatedException(
              OidvpError.INVALID_VC, "vc credentialSubject is not json object");
        }
      }
  ...
  private void complementClaims(String inputDescriptorId, ObjectNode credentialSubject) {
    ...
    if (claimInfo == null) {
      // claims may not have been submitted or is invalid to submit, if claims is not found doing
      // nothing.
      LOGGER.warn(
          "claims for input_descriptor id({}) is not found from claims map", inputDescriptorId);
      return;
    }
