...
public class OID4VP {
  ...
  public OidvpAuthorizationRequest getAuthorizationRequest(
      ClientIdScheme clientIdScheme, AuthorizationRequestParam requestParam) {
    OidvpClientIdentifier oidvpClientIdentifier =
        specifyClientIdentifier(clientIdScheme, requestParam);
  ...
  public JWT getRequestObject(
      ...
      throws OidvpException {
    OidvpAuthorizationRequest authorizationRequest =
        getAuthorizationRequest(clientIdScheme, requestParam);
    return createRequestObject(
        authorizationRequest,
        requestObjectOpts,
        OID4VP.resolveRequestObjectType(clientIdScheme, false));
  }

  public JWT getRequestObject(
      String oidvpClientId,
      AuthorizationRequestParam requestParam,
      RequestObjectOpts requestObjectOpts)
      throws OidvpException {
    OidvpClientIdentifier oidvpClientIdentifier = OidvpClientIdentifier.parse(oidvpClientId);
    OidvpAuthorizationRequest authorizationRequest =
        createAuthorizationRequest(oidvpClientIdentifier, requestParam);
    return createRequestObject(
        authorizationRequest,
        requestObjectOpts,
        OID4VP.resolveRequestObjectType(oidvpClientIdentifier.getClientIdScheme(), false));
  }

  public JWT getRequestObject(
      OidvpAuthorizationRequest oidvpAuthorizationRequest, RequestObjectOpts requestObjectOpts)
      throws OidvpException {
    OidvpClientIdentifier oidvpClientIdentifier =
        OidvpClientIdentifier.parse(oidvpAuthorizationRequest.getClientID().getValue());
    return createRequestObject(
        oidvpAuthorizationRequest,
        requestObjectOpts,
        OID4VP.resolveRequestObjectType(oidvpClientIdentifier.getClientIdScheme(), false));
  }

  public static RequestObjectType resolveRequestObjectType(
      ClientIdScheme clientIdScheme, boolean encrypted) {
    return resolveRequestObjectType(clientIdScheme, true, encrypted);
  }

  public static RequestObjectType resolveRequestObjectType(
      ClientIdScheme clientIdScheme, boolean signed, boolean encrypted) {
    if (isAuthorizationRequestMustBeSigned(clientIdScheme)
        || (signed && isAuthorizationRequestCanBeSigned(clientIdScheme))) {
  ...
  private OidvpAuthorizationRequest createAuthorizationRequest(
      OidvpClientIdentifier oidvpClientIdentifier, AuthorizationRequestParam requestParam) {
    ...
    if (requestParam.specifiesRequestObject()) {
      if (requestParam.getRequestURI() != null) {
        ...
        if (!isCurrentClientIdMatchedRequestObjectClientId(
            clientID, requestParam.getRequestObject())) {
          throw new BadOidvpParamException(
              OidvpError.OIDVP_CLIENT_ID_NOT_MATCH_ERROR,
              "create authorization request error: The client_id does not match request object's client_id, the client_id might have been changed.");
        }
      ...
      builder =
          new OidvpAuthorizationRequest.Builder(responseType, clientID, requestParam.getNonce());
    }

    if (oidvpClientIdentifier.getClientIdScheme() == ClientIdScheme.REDIRECT_URI
        && !oidvpClientIdentifier
            .getOriginalClientID()
            .getValue()
            .equals(requestParam.getRedirectOrResponseURI().toString())) {
      throw new BadOidvpParamException(
          "redirect_uri/response_uri must be the same with orig_client_id when client_id_scheme is 'redirect_uri'.");
    }
    ...
    if (pdp != null) {
      if (pdp.getPresentationDefinition() != null) {
        ...
        throw new BadOidvpParamException(
            "presentation_definition or presentation_definition_uri must be specified");
      }
    }

    return builder
        .endpointURI(requestParam.getAuthorizationEndpoint())
  ...
  private JWT createRequestObject(
      OidvpAuthorizationRequest authorizationRequest,
      RequestObjectOpts requestObjectOpts,
    ...
    OidvpClientIdentifier oidvpClientIdentifier =
        OidvpClientIdentifier.parse(authorizationRequest.getClientID().getValue());

    String iss = oidvpClientIdentifier.getOriginalClientID().getValue();
    JWTClaimsSet.Builder builder =
        new JWTClaimsSet.Builder(authorizationRequest.toJWTClaimsSet()).issuer(iss).audience(iss);
    ...
    if (requestObjectOpts.getExpiredTime() != null) {
      if (requestObjectOpts.getExpiredTime().after(Timestamp.from(Instant.now()))) {
        ...
        throw new IllegalArgumentException(
            "Bad expiredTime value, expiredTime is before current time.");
      }
    }
    ...
    switch (requestObjectType) {
      ...
      default -> throw new UnsupportedOperationException(
          "unsupported requestObjectType: " + requestObjectType);
    }

    return requestObjectJWT;
  }

  private OidvpClientIdentifier specifyClientIdentifier(
      ClientIdScheme clientIdScheme, AuthorizationRequestParam requestParam) {
    if (requestParam == null) {
      throw new IllegalArgumentException("AuthorizationRequestParam must not be null");
    }
    OidvpClientIdProvider oidvpClientIdProvider =
        oidvpClientIdProviderFactory.createClientIdProvider(
            clientIdScheme, requestParam.getResponseMode4VP());
    return new OidvpClientIdentifier(oidvpClientIdProvider);
  }

  private boolean isCurrentClientIdMatchedRequestObjectClientId(
      ClientID currentClientId, JWT requestObject) {
    try {
  ...
  private JWK getActiveSigningJWK(OidvpClientIdentifier oidvpClientIdentifier) {
    ...
    JWK currentSigningJWK =
        oidvpClientIdProviderFactory.createClientIdProvider(clientIdScheme, null).getSigningJWK();
    if (currentSigningJWK == null) {
      throw new BadOidvpParamException(
          "signed request object error: get signing jwk error, jwk must not be null.");
    }

    boolean matched;
    if (clientIdScheme == ClientIdScheme.DID) {
      try {
        JWK publicKeyFromDID =
            DIDUtils.extractPublicKeyFromDID(
                oidvpClientIdentifier.getOriginalClientID().getValue());
        matched =
            publicKeyFromDID.getRequiredParams().equals(currentSigningJWK.getRequiredParams());
      } catch (Exception e) {
    ...
    if (!matched) {
      throw new BadOidvpParamException(
          OidvpError.OIDVP_CLIENT_ID_NOT_MATCH_ERROR,
          "create request object error: client_id not match signing jwk, the client_id or signing jwk might have been changed.");
    }
    return currentSigningJWK;
  }

  private void checkRequestObjectType(
      RequestObjectType requestObjectType, ClientIdScheme clientIdScheme) {
    if (RequestObjectType.PLAIN == requestObjectType) {
      if (isAuthorizationRequestMustBeSigned(clientIdScheme)) {
        throw new BadOidvpParamException(
            "request must be singed when using client_id_scheme:"
                + clientIdScheme
                + ", but the request object type is set to "
                + requestObjectType);
      }
    } else if (RequestObjectType.SINGED == requestObjectType
        || RequestObjectType.SINGED_AND_ENCRYPTED == requestObjectType) {
      if (!isAuthorizationRequestCanBeSigned(clientIdScheme)) {
        throw new BadOidvpParamException(
            "request must not be singed when using client_id_scheme:"
                + clientIdScheme
                + ", but the request object type is set to "
                + requestObjectType);
      }
