...
public class PresentationDefinitionValidator extends AbstractValidator<PresentationDefinition> {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(PresentationDefinitionValidator.class);
  ...
  private Result checkGroup(PresentationDefinition pd) {
    ...
    if (from.equals(group)) {
      ...
      LOGGER.error(
          "invalid pd schema: the value of 'group' must match one of the strings listed in the 'from' of submission_requirements object");
  ...
  private String getMessage(Result result) {
    ...
    switch (result.errorCode) {
      case INPUT_DESCRIPTORS_NOT_GROUPED -> message =
          "invalid pd schema: when submission_requirement is present input_descriptor must be grouped";
      case GROUP_AND_FROM_NOT_MATCH -> message =
          "invalid pd schema: the value of 'group' must match one of the strings listed in the 'from' of submission_requirements object";
    }
    return message;
  }
