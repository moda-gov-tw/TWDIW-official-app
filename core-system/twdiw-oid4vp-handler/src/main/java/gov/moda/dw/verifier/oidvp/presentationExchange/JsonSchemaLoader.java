...
public class JsonSchemaLoader {
  ...
  static {
    try {
      PRESENTATION_DEFINITION_SCHEMA =
          JsonUtils.getJsonStringFromFile(
              new ClassPathResource("schema/presentation_definition_schema.json").getInputStream());
      LOGGER.info("Loading presentation_definition_schema.json ... OK");
      PRESENTATION_SUBMISSION_SCHEMA =
          JsonUtils.getJsonStringFromFile(
              new ClassPathResource("schema/presentation_submission_schema.json").getInputStream());
      ...
      LOGGER.info(
          "Loading presentation_definition_schema.json/presentation_submission_schema.json ... Fail");
  ...
  public JsonSchema getPresentationDefinitionSchema() {
    JsonSchema schema =
        JSON_SCHEMA_FACTORY_V7.getSchema(PRESENTATION_DEFINITION_SCHEMA, VALIDATORS_CONFIG);
  ...
  public JsonSchema getPresentationSubmissionSchema() {
    JsonSchema schema =
        JSON_SCHEMA_FACTORY_V7.getSchema(PRESENTATION_SUBMISSION_SCHEMA, VALIDATORS_CONFIG);
