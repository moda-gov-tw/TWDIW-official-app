...
/**
 * TOTP（基於時間的一次性密碼）服務 此服務類別提供 TOTP 的相關功能，包含： 1. 產生 TOTP 金鑰 2. 產生 TOTP 碼 3. 驗證 TOTP 碼 4. 檢核輸入的 TOTP
 * 金鑰是否符合
 *
 * <p>本服務實作 RFC 6238 標準，使用 HMAC-SHA256 演算法， 結合時間戳記和金鑰來產生一次性密碼。
 *
 * <p>主要特點： - 使用 256 位元（32 位元組）的金鑰長度 - 提供 6 位數的 TOTP 碼 - 預設 60 秒的有效時間 - 允許前後 30 秒的時間偏移
 */
@Service
  ...
  public String generateTOTP(String secretKey, int i) {
    try {
      ...
      long currentTimeStep =
          (currentTime + appConfiguration.getTotpTimeOffset()) / appConfiguration.getTotpTimeout();
