...
public class VPVerifyResultResource {
   ...
   *     vPVerifyResultDTO, or with status {@code 400 (Bad Request)} if the vPVerifyResult has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("")
  @PreAuthorize("hasAuthority('verifier_CreateVP')")
  public ResponseEntity<VPVerifyResultDTO> createVPVerifyResult(
      @Valid @RequestBody VPVerifyResultDTO vPVerifyResultDTO) throws URISyntaxException {
    log.debug("REST request to save VPVerifyResult : {}", vPVerifyResultDTO);
    if (vPVerifyResultDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new vPVerifyResult cannot already have an ID", ENTITY_NAME, "idexists");
    }
    ...
    return ResponseEntity.created(new URI("/api/vp-verify-results/" + vPVerifyResultDTO.getId()))
        .headers(
            HeaderUtil.createEntityCreationAlert(
                applicationName, true, ENTITY_NAME, vPVerifyResultDTO.getId().toString()))
   ...
   *     vPVerifyResultDTO, or with status {@code 400 (Bad Request)} if the vPVerifyResultDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the vPVerifyResultDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/{id}")
  ...
  /**
   * {@code PATCH /vp-verify-results/:id} : Partial updates given fields of an existing
   * vPVerifyResult, field will ignore if it is null
   *
   * @param id the id of the vPVerifyResultDTO to save.
   * @param vPVerifyResultDTO the vPVerifyResultDTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     vPVerifyResultDTO, or with status {@code 400 (Bad Request)} if the vPVerifyResultDTO is not
   *     valid, or with status {@code 404 (Not Found)} if the vPVerifyResultDTO is not found, or
   *     with status {@code 500 (Internal Server Error)} if the vPVerifyResultDTO couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
  ...
  public ResponseEntity<VPVerifyResultDTO> partialUpdateVPVerifyResult(
      ...
      throws URISyntaxException {
    log.debug(
        "REST request to partial update VPVerifyResult partially : {}, {}", id, vPVerifyResultDTO);
    if (vPVerifyResultDTO.getId() == null) {
      throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
    }
    if (!Objects.equals(id, vPVerifyResultDTO.getId())) {
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of vPVerifyResults
   *     in body.
   */
  @GetMapping("")
  @PreAuthorize("hasAuthority('verifier_CreateVP')")
   ...
   *     vPVerifyResultDTO, or with status {@code 404 (Not Found)}.
   */
  @GetMapping("/{id}")
  @PreAuthorize("hasAuthority('verifier_CreateVP')")
