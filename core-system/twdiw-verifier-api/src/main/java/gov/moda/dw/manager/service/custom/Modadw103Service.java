...
public class Modadw103Service {
   ...
   */
  public Modadw103Service(
      CustomOidvpConfigRepository customOidvpConfigRepository,
      CustomOrgKeySettingRepository customOrgKeySettingRepository,
      RemoteApiService remoteApiService,
      OrgKeySettingQueryService orgKeySettingQueryService,
   ...
   * @return
   */
  public Page<OrgKeySettingResDTO> getAllOrgKeySetting(
      OrgKeySettingCriteria criteria, Pageable pageable) {
    // 僅能看到自己組織的金鑰
    StringFilter sf = new StringFilter();
    sf.setEquals(SecurityUtils.getJwtUserObject().get(0).getOrgId());
    criteria.setOrgId(sf);

    Page<OrgKeySettingDTO> orgKeySettingDtoPage =
        orgKeySettingQueryService.findByCriteria(criteria, pageable);
  ...
  public OrgKeyDetailResDTO getKeyItem(Long id) {
    ...
    // 查詢指定 ID 的金鑰
    OrgKeySetting keyItem =
        customOrgKeySettingRepository
            .findById(id)
            .orElseThrow(
                () -> {
  ...
  public void setOrgKeyActive(Long id) {
    ...
    // 1. 查詢指定 ID 的金鑰
    OrgKeySetting keyToActivate =
        customOrgKeySettingRepository
            .findByIdAndOrgId(id, loginUserOrgId)
            .orElseThrow(
                () -> {
                  throw new BadRequestAlertException("10304", "id", "key not found");
                });
    ...
    // 建立第一筆金鑰時跳過此檢核
    if (keyCount > 1) {
      List<OrgKeySetting> currentActiveKeys =
          customOrgKeySettingRepository.findByOrgIdAndIsActiveTrue(loginUserOrgId);
