...
public class Ams301wResource {
  ...
  @Autowired private Ams301wService ams301wService;
  ...
  public ResponseEntity<Nonce> requestBwdReset(@RequestBody Ams301ReqDTO req) {
    log.debug("[重置密碼]/api/modadw301w/reset-bwd/init");
    ...
    try {
      ...
      Nonce nonce =
          nonceRepository
              .findOneBySIdAndCaptchaCode(req.getNonce().getsId(), req.getNonce().getCaptchaCode())
              .orElseThrow(
                  () -> {
                    // 當驗證碼錯誤時，先嘗試找到該筆 nonce 並刪除
                    this.nonceRepository
                        .findOneBySId(req.getNonce().getsId())
                        .ifPresent(n -> this.nonceRepository.delete(n));
