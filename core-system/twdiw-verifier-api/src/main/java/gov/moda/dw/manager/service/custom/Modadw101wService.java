...
public class Modadw101wService {
  ...
  @Autowired private CustomOrgService customOrgService;

  @Autowired private OidvpConfigService oidvpConfigService;

  @Autowired private CustomExtendedUserService customExtendedUserService;

  @Autowired private CustomAccessTokenRepository customAccessTokenRepository;

  @Autowired private Modadw102wService modadw102wService;
  ...
  public ResponseDTO<GetVerifierDataResDTO> getInitData(String req) {
    ...
    try {
      ...
      if (!Objects.equals(oidvpConfig.get().getPropertyValue(), "")) {
        ...
        ResponseEntity<FindVerifierResDTO> findVerifierResponse =
            restTemplate.exchange(url, HttpMethod.GET, entity, FindVerifierResDTO.class);

        if (findVerifierResponse.getBody() == null
            || findVerifierResponse.getBody().getCode() != 0) {
          log.error(
              "Modadw101wService-getInitData-front301i查詢DID資訊失敗，錯誤碼: {}, 錯誤訊息: {}",
              findVerifierResponse.getBody().getCode(),
              findVerifierResponse.getBody().getMsg());
        ...
        Optional<OidvpConfig> token =
            oidvpConfigService.findOne("access_token.frontend.register_did");
        if (token.isEmpty()) {
        ...
        xssRegex.setRegularExpression(
            "<.*?(script|img|iframe|object|embed|style|base|link|meta).*?>|javascript:|data:text");
        ...
        regexSql.setRegularExpression(
            "('|\\-\\-|;|\\/\\*|\\*\\/|xp_|drop|select|insert|delete|update|union|exec|execute|alter|create|shutdown)");
  ...
  public StatusCode register(Modadw101ReqDTO req) {
    try {
      // 檢查DID註冊欄位
      if (!req.getOrg().getTaxId().matches("^[0-9\\-]+$")
      ...
      if (req.getOrg().getX509_subject() != null
          && req.getOrg().getX509_serial() != null
          && req.getOrg().getX509_type() != null) {
      ...
      log.info(
          "Modadw101wService-register 呼叫verifier-oidvp註冊DID，Request Body:{},",
      ...
      if (!responseBody.has("did")) {
        log.error(
            "Modadw101wService-register verifier-oidvp註冊DID失敗，錯誤碼: {}, 錯誤訊息: {}",
            responseBody.get("code").asText(),
      ...
      String base64LogoRectangle =
          modadw102wService.getLogoWithNameGenerator(req.getOrg().getName());
      ...
      // 修改預設登入帳號為該組織
      List<ExtendedUser> defaultUsers =
          customExtendedUserService.findAllByOrgId(OrgType.DEFAULT_ORG.getCode());
      for (ExtendedUser user : defaultUsers) {
        ...
        ExtendedUser extendedSaveResult =
            customExtendedUserService.updateExtendedUser(
                ams311wReqDTO, AccountActionType.UPDATE, user.getState().equals("1"));
        if (extendedSaveResult == null) {
          log.error(
              "Modadw101wService-register 更新使用者組織失敗:{}", StatusCode.DID_UPDATE_ORG_ERROR.getMsg());
          return StatusCode.DID_UPDATE_ORG_ERROR;
        }
      }

      // 修改預設 Access Token 組織
      List<AccessToken> defaultTokens =
          customAccessTokenRepository.findAllByOrgId(OrgType.DEFAULT_ORG.getCode());
      for (AccessToken token : defaultTokens) {
        ...
        try {
          ...
          log.error(
              "Modadw101wService-register 更新 access token 失敗:{}",
              StatusCode.DID_UPDATE_ACCESSTOKEN_ERROR.getMsg());
  ...
  public ResponseDTO<VerifyCertAndDidResDTO> verifyCert(VerifyCertReqDTO reqDTO, String type) {
    ...
    if (certResponse.getResultErrorCode() != 0) {
      log.error(
          "Modadw101wService-verifyCert 憑證驗簽失敗，VA錯誤代碼:{}, VA錯誤訊息:{}, 詳細訊息:{}",
          certResponse.getResultErrorCode(),
          certResponse.getResultMSG(),
          certResponse.getResultException());
      VerifyCertAndDidResDTO verifyCertAndDidResDTO =
          new VerifyCertAndDidResDTO(
              String.valueOf(certResponse.getResultErrorCode()), certResponse.getResultMSG(), null);
  ...
  public ResponseDTO<VerifyCertAndDidResDTO> verifyDid(VerifyDidReqDTO reqDTO) {
    ...
    if (!Objects.equals(ivpasResponse.getHeader().getReturnCode(), "0000")) {
      log.error(
          "Modadw101wService-verifiyDID IVPAS檢查未通過，代碼:{}, 錯誤訊息:{}",
          ivpasResponse.getHeader().getReturnCode(),
          ivpasResponse.getHeader().getReturnDesc());
      VerifyCertAndDidResDTO verifyCertAndDidResDTO =
          new VerifyCertAndDidResDTO(
              ivpasResponse.getHeader().getReturnCode(),
              ivpasResponse.getHeader().getReturnDesc(),
    ...
    try {
      oidvpConfigService.update(
          new OidvpConfig("access_token.frontend.register_did", reqDTO.getToken()));
    } catch (Exception ex) {
    ...
    VerifyCertAndDidResDTO verifyCertAndDidResDTO =
        new VerifyCertAndDidResDTO(
            ivpasResponse.getHeader().getReturnCode(),
            ivpasResponse.getHeader().getReturnDesc(),
            ivpasResponse.getBody().getOrgEnName());
  ...
  private IvpasVerifyResDTO callIvpasService(VerifyDidReqDTO reqDTO) {
    ...
    try {
      ...
      HttpEntity<Map<String, Object>> entity = new HttpEntity<>(requestBody, headers);
      RestTemplate restTemplate = new RestTemplate();
      ResponseEntity<IvpasVerifyResDTO> response =
          restTemplate.exchange(url, HttpMethod.POST, entity, IvpasVerifyResDTO.class);
  ...
  public ResponseEntity<String> callFront302i() {
    ...
    try {
      ...
      log.warn(
          "Modadw101wService-callFront302i 發生 HTTP 錯誤，狀態碼: {}, 原因: {}",
          ex.getStatusCode(),
  ...
  public ResponseEntity<String> callIvpasTest() {
    ...
    try {
      ...
      ResponseEntity<String> response =
          restTemplate.exchange(ivpasHealthUrl, HttpMethod.GET, null, String.class);
      log.info("Modadw101wService-callIvpasTest ivpas API 呼叫結果: {}", response.getStatusCode());
      return response;
    } catch (HttpStatusCodeException ex) {
      log.warn(
          "Modadw101wService-callIvpasTest 發生 HTTP 錯誤，狀態碼: {}, 原因: {}",
          ex.getStatusCode(),
          ExceptionUtils.getStackTrace(ex));
      return ResponseEntity.status(ex.getStatusCode()).body("呼叫外部 API 時發生錯誤");
    } catch (Exception ex) {
