...
public class Dwvp404iService {
  ...
  public Dwvp404iResDTO getDecryptionData(Dwvp404iReqDTO request) {
    ...
    try {
      ...
      // 根據 keyId 查詢金鑰設定
      List<OrgKeySetting> orgKeySettings =
          customOrgKeySettingRepository.findByKeyIdAndIsActiveTrue(request.getKeyId());
  ...
  private boolean validateRequiredFields(JsonNode dataNode) {
    return dataNode != null && dataNode.has("totp") && dataNode.get("totp").asText() != null;
  }
}
