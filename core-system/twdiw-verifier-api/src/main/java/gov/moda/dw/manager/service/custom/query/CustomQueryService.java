...
public class CustomQueryService<ENTITY> extends QueryService<ENTITY> {
  ...
  protected Specification<ENTITY> likeUpperSpecification(
      Function<Root<ENTITY>, Expression<String>> metaclassFunction, String value, char escape) {
    String escapedValue = value.replace("_", escape + "_").replace("%", escape + "%");
    return (root, query, builder) ->
        builder.like(
            builder.upper(metaclassFunction.apply(root)), wrapLikeQuery(escapedValue), escape);
  }
}
