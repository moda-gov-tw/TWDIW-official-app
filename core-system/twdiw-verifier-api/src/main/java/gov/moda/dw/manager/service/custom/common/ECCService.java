...
/**
 * ECC 加密服務 使用 Curve25519 演算法進行非對稱加密，結合 ECDH 金鑰交換和 ChaCha20-Poly1305 對稱加密
 *
 * <p>主要功能： 1. 產生 ECC 金鑰對 2. 使用公鑰加密資料 3. 使用私鑰解密資料 4. 檢核輸入的 ECC 金鑰是否符合
 *
 * <p>加密流程： 1. 產生臨時金鑰對 2. 使用 ECDH 產生共享金鑰 3. 使用 SHA-256 作為 KDF 衍生對稱金鑰 4. 使用 ChaCha20-Poly1305 加密資料 5.
 * 組合臨時公鑰、Nonce 和加密資料
 *
 * <p>解密流程： 1. 分離臨時公鑰、Nonce 和加密資料 2. 使用 ECDH 重建共享金鑰 3. 使用 SHA-256 作為 KDF 衍生對稱金鑰 4. 使用
 * ChaCha20-Poly1305 解密資料
 */
@Service
public class ECCService {
  ...
  public String[] generateKeyPair()
    ...
    // 將公鑰與私鑰進行 Base64 編碼
    final byte[] pubKey = kp.getPublic().getEncoded();
    final byte[] prvKey = kp.getPrivate().getEncoded();
    String pubKeyB64 = Base64.getEncoder().encodeToString(pubKey);
    String prvKeyB64 = Base64.getEncoder().encodeToString(prvKey);

    // 回傳 [公鑰, 私鑰]
    return new String[] {pubKeyB64, prvKeyB64};
  }
  ...
  public String encrypt(String plaintext, String receiverPublicKeyBase64) throws Exception {
    ...
    PublicKey receiverPublicKey =
        kf.generatePublic(
            new java.security.spec.X509EncodedKeySpec(
                Base64.getDecoder().decode(receiverPublicKeyBase64)));
    ...
    byte[] output =
        new byte[keyLength.length + ephemeralPubKey.length + nonce.length + ciphertext.length];
   ...
   * 使用私鑰解密密文
   *
   * @param encryptedBase64 Base64 編碼的加密資料，格式為：[ephemeral 公鑰長度(4 bytes) + ephemeral 公鑰 + nonce +
   *     加密資料]
  ...
  public String decrypt(String encryptedBase64, String privateKeyBase64) throws Exception {
    try {
      ...
      PublicKey ephemeralPublicKey =
          kf.generatePublic(new java.security.spec.X509EncodedKeySpec(ephemeralPubKeyBytes));
   ...
   * 檢核輸入的 ECC 金鑰是否符合
   *
   * @param key Base64 編碼的公鑰或私鑰的字串
   * @param isPublic 公鑰為 true，私鑰為 false
   * @return 如果 ECC 金鑰符合則回傳 true，否則回傳 false
   */
  public static boolean validKey(String key, boolean isPublic) {
    try {
      // Base64 解碼取得原始加密資料
      ...
      if (isPublic) {
        // 公鑰使用 X509EncodedKeySpec 格式解析並產生公鑰物件
        kf.generatePublic(new X509EncodedKeySpec(keyBytes));
      } else {
        // 私鑰使用 PKCS8EncodedKeySpec 格式解析並產生私鑰物件
