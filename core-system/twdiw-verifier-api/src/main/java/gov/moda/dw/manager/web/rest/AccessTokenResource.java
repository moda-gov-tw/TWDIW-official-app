...
import gov.moda.dw.manager.service.AccessTokenService;
...
public class AccessTokenResource {
   ...
   *     accessTokenDTO, or with status {@code 400 (Bad Request)} if the accessToken has already an
   *     ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("")
  public ResponseEntity<AccessTokenDTO> createAccessToken(
      @Valid @RequestBody AccessTokenDTO accessTokenDTO) throws URISyntaxException {
    log.debug("REST request to save AccessToken : {}", accessTokenDTO);
    if (accessTokenDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new accessToken cannot already have an ID", ENTITY_NAME, "idexists");
    }
    ...
    return ResponseEntity.created(new URI("/api/access-tokens/" + accessTokenDTO.getId()))
        .headers(
            HeaderUtil.createEntityCreationAlert(
                applicationName, true, ENTITY_NAME, accessTokenDTO.getId().toString()))
        .body(accessTokenDTO);
  }
   ...
   *     accessTokenDTO, or with status {@code 400 (Bad Request)} if the accessTokenDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the accessTokenDTO couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/{id}")
  ...
  /**
   * {@code PATCH /access-tokens/:id} : Partial updates given fields of an existing accessToken,
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     accessTokenDTO, or with status {@code 400 (Bad Request)} if the accessTokenDTO is not
   *     valid, or with status {@code 404 (Not Found)} if the accessTokenDTO is not found, or with
   *     status {@code 500 (Internal Server Error)} if the accessTokenDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/{id}",
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of accessTokens in
   *     body.
   */
  @GetMapping("")
   ...
   *     accessTokenDTO, or with status {@code 404 (Not Found)}.
   */
  @GetMapping("/{id}")
