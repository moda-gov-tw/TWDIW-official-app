...
/** Service Implementation for managing {@link gov.moda.dw.manager.domain.OtpVerify}. */
@Service
@Transactional
public class OtpVerifyService {
  ...
  public Optional<OtpVerifyDTO> partialUpdate(OtpVerifyDTO otpVerifyDTO) {
    ...
    return otpVerifyRepository
        .findById(otpVerifyDTO.getId())
        .map(
            existingOtpVerify -> {
        ...
        .map(otpVerifyRepository::save)
        .map(otpVerifyMapper::toDto);
  }
