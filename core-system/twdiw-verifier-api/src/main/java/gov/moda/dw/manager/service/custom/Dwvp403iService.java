...
public class Dwvp403iService {
  ...
  public void createOrgSetting(CustomOrgKeySettingDTO request) {
    ...
    Optional<OidvpConfig> oidvpConfig =
        customOidvpConfigRepository.findByPropertyKey("verifier.did");
    if (oidvpConfig.isEmpty()) {
      throw new DWException(StatusCode.DWVP_DID_NOT_REGISTERED);
    }

    FindVerifierResDTO findVerifierResDTO =
        remoteApiService.getDIDInfoById(oidvpConfig.get().getPropertyValue());
    ...
    Optional<OrgKeySetting> existingKey =
        customOrgKeySettingRepository.findByOrgIdAndKeyId(loginUserOrgId, request.getKeyId());
    ...
    // 找出同組織其它有啟用的金鑰
    List<OrgKeySetting> sameOrgList =
