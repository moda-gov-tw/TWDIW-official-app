...
public class CustomRelService {
   ...
   * @return
   */
  public List<ResDTO> roleAction(
      RoleDTO roleDTO, List<Ams321wConferRes> newResList, AuthorityAction authorityAction) {
    log.info(
   ...
   * @param authorityAction
   */
  public List<RoleDTO> accountAction(
      User user, List<RoleDTO> roleDTOList, AuthorityAction authorityAction) {
    log.info(
   ...
   * @param actionType
   */
  public StatusCode accessTokenAction(
      Ams351wRelReqDTO ams351wRelReqDTO, AccessToken accessToken, AuthorityAction actionType) {
    log.info(
  ...
  private StatusCode updateRelForAccessToken(
      Ams351wRelReqDTO ams351wRelReqDTO, AccessToken accessToken) {
    // 要確認state
  ...
  public List<RoleDTO> accountConferRole(User user, List<RoleDTO> roleDTOList) {
    ...
    entityManager.flush();
    boolean accountConferRoleSyncOk =
        syncUserAuthoritiesService.syncUserAuthorities(user.getLogin());
  ...
  private void deleteUserRole(User user) {
    ...
    relCriteria.setRightTbl(
        StringFilterUtils.toEqualStringFilter(RelType.USERTOROLE.getRightTbl()));
  ...
  public List<RoleDTO> addUserRole(User user, List<RoleDTO> roleDTOList) {
    ...
    if (addDefaultRole) {
      ...
      if (roleOp.isPresent()) {
        ...
        log.warn(
            "CustomRelService-addUserRole Role表找不到預設角色, 無法賦予角色, role={}", RoleType.DEFAULT_ROLE);
        return null;
      }
    }
  ...
  /**
   * 存入預設角色 User::Role 並同步權限到 jhi_user_authority
   *
   * @param user
   */
  ...
  public List<ResDTO> roleConferRes(RoleDTO roleDTO, List<Ams321wConferRes> newResList) {
    log.info(
        "CustomRelService-roleConferRes 開始執行賦予角色功能 roleId={}, 功能清單={}",
        roleDTO.getRoleId(),
        newResList);
  ...
  /**
   * 角色啟停 角色停用的話要刪除帳號權限(User::Role), 但rel表 Role::Res的關係依然保留 角色啟用 不更動任何關聯, 因Role::Res的關係依然保留, 不須重新設定;
   * User::Role依照需求重新關聯於帳號(web帳號管理操作)
   *
   * @param roleDTO
   * @param authorityAction
   * @return
  ...
  public List<ResDTO> roleChangeState(RoleDTO roleDTO, AuthorityAction authorityAction) {
    ...
    if (AuthorityAction.ROLE_DISABLED == authorityAction) {
      ...
      // 刪除無使用的 Authority, 只有停用時才需要檢查並刪除無使用的 Authority
  ...
  /**
   * 刪除角色在rel原有權限 從user刪除 User::Role
   *
   * @param user
   */
  ...
  public List<RelDTO> delUserRoleByRole(String roleId) {
    log.info(
        "CustomRelService-delUserRoleByRole 刪除使用該角色的帳號 由RoleId刪除 User::Role roleId={},", roleId);
  ...
  public boolean syncUserAuthorities(List<RelDTO> userList) {
    ...
    for (RelDTO user : userList) {
      ...
      if (!isOK) {
        allOk = false;
        log.warn(
            "CustomRelService-syncUserAuthorities 同步 jhi_user_authority 有失敗 RelDTO user={}", user);
      }
    }
    return allOk;
  }
