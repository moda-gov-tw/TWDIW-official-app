 ...
 * in {@link gov.moda.dw.manager.web.rest.VPItemFieldResource} to receive all the possible filtering
 * options from the Http GET request parameters. For example the following could be a valid request:
 * {@code /vp-item-fields?id.greaterThan=5&attr1.contains=something&attr2.specified=false} As Spring
...
public class VPItemFieldCriteria implements Serializable, Criteria {
  ...
  public VPItemFieldCriteria(VPItemFieldCriteria other) {
    ...
    this.vcCategoryDescription =
        other.optionalVcCategoryDescription().map(StringFilter::copy).orElse(null);
