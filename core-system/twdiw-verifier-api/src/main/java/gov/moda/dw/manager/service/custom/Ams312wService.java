...
public class Ams312wService {
  ...
  public Ams312wAccountChangeResDTO getExtendedUserLog(
      Pageable pageable, Ams312wReqDTO ams312wReqDTO) {
    ...
    try {
      ...
      // 查詢
      Page<ExtendedUserLogDTO> findAll =
          extendedUserLogQueryService.findByCriteria(extendedUserLogCriteria, pageable);
      ...
      for (ExtendedUserLogDTO extendedUserLogDTO : extendedUserLogDTOList) {
        ExtendedUserLogDTO resultExtendedUserLogDTO =
            this.toResultExtendedUserLogDTO(extendedUserLogDTO);
      ...
      log.error(
          "Ams312wService-getExtendedUserLog 發生錯誤, 錯誤原因為={}", ExceptionUtils.getStackTrace(ex));
      return null;
    }
  }
  ...
  private ExtendedUserLogCriteria setCriteriaFromSearch(Ams312wReqDTO ams312wReqDTO) {
    ...
    if (StringUtils.isNotEmpty(ams312wReqDTO.getLogType())) {
      extendedUserLogCriteria.setLogType(
          StringFilterUtils.toEqualStringFilter(ams312wReqDTO.getLogType()));
    }

    if (StringUtils.isNotEmpty(ams312wReqDTO.getUserId())) {
      extendedUserLogCriteria.setUserId(
          StringFilterUtils.toContainStringFilter(ams312wReqDTO.getUserId()));
    }
    ...
    if (beginDate != null && endDate != null) {
      if (endDate.isAfter(beginDate)) {
        extendedUserLogCriteria.setLogTime(
            InstantFilterUtils.toInstantFilterBetween("logTime", beginDate, endDate));
      } else {
    ...
    log.info(
        "Ams312wService-setCriteriaFromSearch 查詢條件 extendedUserLogCriteria={}",
        extendedUserLogCriteria);
