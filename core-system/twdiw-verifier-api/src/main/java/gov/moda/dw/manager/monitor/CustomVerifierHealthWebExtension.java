...
@Component
@Primary
public class CustomVerifierHealthWebExtension extends HealthEndpointWebExtension {

  private static final Logger log = LoggerFactory.getLogger(CustomVerifierHealthWebExtension.class);
  ...
  @Value("${vp.internal.url}")
  private String vpInternalUrl;

  @Value("${oid4vp.internal.url}")
  private String oid4vpInternalUrl;

  public CustomVerifierHealthWebExtension(
  ...
  public WebEndpointResponse<HealthComponent> health(
    ...
    if (buildProperties != null) {
      log.info(
          "CustomVerifierHealthWebExtension: Health check triggered, twdiw-release={}, image-version={}",
          buildProperties.get("twdiw-release.version"),
          buildProperties.getVersion());
    } else {
      log.info(
          "CustomVerifierHealthWebExtension: Health check triggered, but buildProperties is null.");
    }
    ...
    if (body instanceof Health health) {
      ...
      if (buildProperties != null) {
        twdiwReleaseMap.put(
            ...
            (String) buildProperties.get("twdiw-release.version"));
        imageVersionMap.put(
            "moda-digitalwallet-docker-verifier-manager-service", buildProperties.getVersion());
      }

      // 取得 vp 和 oid4vp 服務的版本資訊
      Map<String, String> vpServiceInfo = getServiceHealthInfo(vpInternalUrl, "vp");
      if (vpServiceInfo != null) {
        if (vpServiceInfo.containsKey("twdiw-release")) {
          twdiwReleaseMap.put(
              "moda-digitalwallet-docker-verifier-vp-service", vpServiceInfo.get("twdiw-release"));
        }
        if (vpServiceInfo.containsKey("image-version")) {
          imageVersionMap.put(
              "moda-digitalwallet-docker-verifier-vp-service", vpServiceInfo.get("image-version"));
        }
      }

      Map<String, String> oid4vpServiceInfo = getServiceHealthInfo(oid4vpInternalUrl, "oid4vp");
      if (oid4vpServiceInfo != null) {
        if (oid4vpServiceInfo.containsKey("twdiw-release")) {
          twdiwReleaseMap.put(
              "moda-digitalwallet-docker-verifier-oid4vp-service",
              oid4vpServiceInfo.get("twdiw-release"));
        }
        if (oid4vpServiceInfo.containsKey("image-version")) {
          imageVersionMap.put(
              "moda-digitalwallet-docker-verifier-oid4vp-service",
              oid4vpServiceInfo.get("image-version"));
        }
      }
      ...
      if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
        ...
        if (detailsNode != null) {
          ...
          if (detailsNode.has("twdiw-release")) {
            ...
            log.warn(
                "Health response for {} service does not contain twdiw-release in details",
                serviceName);
          }
          ...
          if (detailsNode.has("image-version")) {
            ...
            log.warn(
                "Health response for {} service does not contain image-version in details",
                serviceName);
          }

          if (!serviceInfo.isEmpty()) {
            log.debug(
                "Successfully retrieved health info for {} service: {}", serviceName, serviceInfo);
            return serviceInfo;
          } else {
            log.warn(
                "Health response for {} service does not contain required fields in details",
                serviceName);
          }
        } else {
        ...
        log.warn(
            "Health endpoint for {} service returned non-2xx status: {}",
            serviceName,
