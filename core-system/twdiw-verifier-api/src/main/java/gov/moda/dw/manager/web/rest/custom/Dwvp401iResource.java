...
public class Dwvp401iResource {
  ...
  public ResponseEntity<BaseHttpResDTO> getOfflineQrcodeByVpUid(
      @PathVariable("vpUid") String vpUid) {
    log.debug("REST request to get offline qrcode by vpUid : {}", vpUid);
    try {
      return ResponseEntity.ok(
          BaseHttpResDTO.success(dwvp401iService.getOfflineQrcodeByVpUid(vpUid)));
    } catch (Exception ex) {
