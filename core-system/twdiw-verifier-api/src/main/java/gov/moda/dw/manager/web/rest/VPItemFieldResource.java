...
public class VPItemFieldResource {
   ...
   *     vPItemFieldDTO, or with status {@code 400 (Bad Request)} if the vPItemField has already an
   *     ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("")
  @PreAuthorize("hasAuthority('verifier_CreateVP')")
  public ResponseEntity<VPItemFieldDTO> createVPItemField(
      @Valid @RequestBody VPItemFieldDTO vPItemFieldDTO) throws URISyntaxException {
    log.debug("REST request to save VPItemField : {}", vPItemFieldDTO);
    if (vPItemFieldDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new vPItemField cannot already have an ID", ENTITY_NAME, "idexists");
    }
    ...
    return ResponseEntity.created(new URI("/api/vp-item-fields/" + vPItemFieldDTO.getId()))
        .headers(
            HeaderUtil.createEntityCreationAlert(
                applicationName, true, ENTITY_NAME, vPItemFieldDTO.getId().toString()))
        .body(vPItemFieldDTO);
  }
   ...
   *     vPItemFieldDTO, or with status {@code 400 (Bad Request)} if the vPItemFieldDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the vPItemFieldDTO couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/{id}")
  ...
  /**
   * {@code PATCH /vp-item-fields/:id} : Partial updates given fields of an existing vPItemField,
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     vPItemFieldDTO, or with status {@code 400 (Bad Request)} if the vPItemFieldDTO is not
   *     valid, or with status {@code 404 (Not Found)} if the vPItemFieldDTO is not found, or with
   *     status {@code 500 (Internal Server Error)} if the vPItemFieldDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/{id}",
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of vPItemFields in
   *     body.
   */
  @GetMapping("")
  @PreAuthorize("hasAuthority('verifier_CreateVP')")
   ...
   *     vPItemFieldDTO, or with status {@code 404 (Not Found)}.
   */
  @GetMapping("/{id}")
  @PreAuthorize("hasAuthority('verifier_CreateVP')")
