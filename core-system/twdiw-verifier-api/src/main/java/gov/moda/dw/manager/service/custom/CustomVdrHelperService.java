...
public class CustomVdrHelperService {
  ...
  public CustomVdrHelperService(
      RemoteApiService remoteApiService, VcManagerOrgRepository vcManagerOrgRepository) {
  ...
  public List<CategoryVdrSimpleDto> getCategoryList() {
    ...
    if (inSandbox) {
      result =
          vcManagerOrgRepository.findAll().stream()
              .filter(e -> !OrgType.DEFAULT_ORG.getCode().equals(e.getOrgId()))
              .map(CategoryVdrSimpleDto::valueOf)
              .toList();
    } else {
      result =
          category.getData().getDids().stream()
              .filter(
                  e ->
                      (StringUtils.equals(OrgDataType.ISSUER.getType(), e.getOrgType())
                          && e.getStatus() == 1L
