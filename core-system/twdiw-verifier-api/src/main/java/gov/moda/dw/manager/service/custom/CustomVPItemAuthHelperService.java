...
public class CustomVPItemAuthHelperService {
  ...
  public CustomVPItemAuthHelperService(
      CustomVdrHelperService customVdrHelperService,
      ...
      VcManagerVCItemRepository vcManagerVCItemRepository,
      VcManagerUserRepository vcManagerUserRepository,
  ...
  public List<VCItemVdr> queryVCFilterByAuth(String taxId, List<VCItemVdr> vcItemVdrs) {
    ...
    if (customVdrHelperService.getInSandbox()) {
      ...
      if (isCommonAccount()) {
        exposeVcItem =
            vcManagerVCItemRepository
                .findByBusinessIdAndExposeOrCrUser(taxId, true, findUserIdFromVcManager())
                .stream()
                .collect(
                    Collectors.toMap(
                        vcItem -> vcItem.getBusinessId() + "_" + vcItem.getSerialNo(), e -> e));
      } else {
        exposeVcItem =
            vcManagerVCItemRepository
                .findByBusinessIdAndExposeOrBusinessId(taxId, true, getBusinessId())
                .stream()
                .collect(
                    Collectors.toMap(
                        vcItem -> vcItem.getBusinessId() + "_" + vcItem.getSerialNo(), e -> e));
      ...
      result =
          vcItemVdrs.stream()
              .filter(
                  vcItemVdr ->
                      exposeVcItem.containsKey(
                          vcItemVdr.getBusinessId() + "_" + vcItemVdr.getSerialNo()))
  ...
  public void checkVpItemAuthByVpId(Long vpId, String actionMsg) {
    ...
    if (vpByIdOpt.isPresent()) {
      ...
      } else if (isNotAdmin(currentLoginUser)
          && !vpByIdOpt.get().getBusinessId().equals(currentLoginOrgId)) {
        throw new AccessDeniedException(
            "You do not have permission to access this resource ( when " + actionMsg + " ). ");
      }
    }
  ...
  public void checkVpItemAuthByTxId(String txId, String actionMsg) {
    Optional<VPVerifyResult> verifyResultOpt =
        customVPVerifyResultRepository.findByTransactionId(txId).stream().findFirst();
