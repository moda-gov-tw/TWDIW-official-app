...
public class CustomCategoryService {
  ...
  public CustomCategoryService(
      RemoteApiService remoteApiService, CustomVdrHelperService customVdrHelperService) {
  ...
  public Map<String, String> getAllCategoryToDtoMap() {
    ...
    categoryRespFromVDR.getData().getDids().stream()
        .filter(
            issuer ->
                (StringUtils.equals(OrgDataType.ISSUER.getType(), issuer.getOrgType())
                    && issuer.getStatus() == 1L
                    && (issuer.getOrg().getIssuerMetadataBaseURL() != null
