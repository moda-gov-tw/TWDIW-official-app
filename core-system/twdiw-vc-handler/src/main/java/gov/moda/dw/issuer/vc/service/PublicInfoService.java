...
public class PublicInfoService {
    ...
    try {
      ...
      if (credentialType == null || credentialType.isBlank()) {
        throw new VcException(
            VcException.ERR_INFO_INVALID_CREDENTIAL_TYPE, "invalid credential type");
      }

      if (groupName == null || groupName.isBlank()) {
        throw new VcException(
            VcException.ERR_INFO_INVALID_GROUP_NAME, "invalid status list group name");
      }
      ...
      StatusList statusList =
          statusListRepository.queryLatestStatusList(credentialType, statusListType.name(), gid);
      if (statusList == null) {
      ...
      if (e.getCode() == VcException.ERR_INFO_STATUS_LIST_NOT_FOUND) LOGGER.warn(e.getMessage(), e);
      ...
      VcException vcException =
          new VcException(VcException.ERR_SL_QUERY_STATUS_LIST_ERROR, "fail to query status list");
  ...
  public Tuple.Pair<String, HttpStatus> getSchema(String schemaName) {
    ...
    try {
      ...
      String schema = Optional.ofNullable(policy).map(Policy::getSchemaContent).orElse(null);
      ...
      LOGGER.error(e.getMessage(), e);
      VcException vcException =
          new VcException(VcException.ERR_INFO_SCHEMA_NOT_FOUND, "schema not found");
      result = new ErrorResponseDTO(vcException).toString();
      httpStatus = vcException.toHttpStatus();
    }
  ...
  public Tuple.Pair<String, HttpStatus> getPublicKeys() {
    ...
    try {
      ...
      if (issuerKeyInDB == null || issuerKeyInDB.isBlank()) {
        throw new VcException(
            VcException.ERR_INFO_PUBLIC_KEY_NOT_FOUND, "issuer's public key not found");
      }
      ...
      if (statusListKeyInDB == null || statusListKeyInDB.isBlank()) {
        throw new VcException(
            VcException.ERR_INFO_PUBLIC_KEY_NOT_FOUND, "status list signing key not found");
      }
      ...
      Map<String, Object> statusListPublicKey =
          JsonUtils.jsToMap(statusListEcKey.toPublicJWK().toJSONString());
      ...
      VcException vcException =
          new VcException(
              VcException.ERR_INFO_PUBLIC_KEY_NOT_FOUND, "fail to show issuer's public key");
      result = new ErrorResponseDTO(vcException).toString();
      httpStatus = vcException.toHttpStatus();
    }
  ...
  private Policy loadCredentialPolicy(String schemaName) {
    ...
    return new Policy()
        ...
        .setSchemaContent(
            "{\"$id\":\""
                + id
                + "\",\"$schema\":\"https://json-schema.org/draft/2020-12/schema#\",\"title\":\"VirtualCardCredential\",\"description\":\"VirtualCardCredential using JsonSchema\",\"type\":\"object\",\"properties\":{\"credentialSubject\":{\"title\":\"credentialSubject\",\"type\":\"object\",\"properties\":{\"id\":{\"title\":\"id\",\"type\":\"string\",\"format\":\"uri\"},\"id_number\":{\"title\":\"id_number\",\"type\":\"string\"},\"full_name\":{\"title\":\"full_name\",\"type\":\"string\"},\"address\":{\"title\":\"address\",\"type\":\"string\"}}}}}")
