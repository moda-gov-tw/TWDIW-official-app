...
public interface CredentialPolicyRepository extends JpaRepository<CredentialPolicyEntity, String> {
  ...
  @Query(
      nativeQuery = true,
      value = "DELETE FROM credential_policy cp WHERE cp.credential_type = :credentialType")
  int deleteByCredentialType(@Param("credentialType") String credentialType);

  @Query(
      nativeQuery = true,
      value = "SELECT func_switch FROM credential_policy WHERE credential_type = :credential_type")
