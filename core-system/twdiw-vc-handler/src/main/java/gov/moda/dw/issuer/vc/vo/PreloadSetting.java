...
public class PreloadSetting {
  ...
  public static final String NAME_URL_FRONTEND_GET_ISSUER_INFO_DID =
      "url.frontend.get_issuer_info_did";
  ...
  // access token
  public static final String NAME_ACCESS_TOKEN_FRONTEND_REGISTER_DID =
      "access_token.frontend.register_did";
    ...
    try {
      ...
      for (Setting setting : settingList) {
        ...
        switch (propName) {
          ...
          case NAME_HTTP_MAX_CONNECT_TIMEOUT -> httpMaxConnectTimeout =
              (propValue != null) ? Integer.parseInt(propValue) : VALUE_HTTP_MAX_CONNECT_TIMEOUT;

            // http - max retry time
          case NAME_HTTP_MAX_RETRY_TIMES -> httpMaxRetryTimes =
              (propValue != null) ? Integer.parseInt(propValue) : VALUE_HTTP_MAX_RETRY_TIMES;
          ...
          case NAME_HTTP_PROXY_ENABLE -> httpProxyEnable = Boolean.parseBoolean(propValue);
            ...
            // http - proxy url
          case NAME_HTTP_PROXY_URL -> httpProxyUrl =
              (propValue != null) ? propValue : VALUE_HTTP_PROXY_URL;

            // http - proxy port
          case NAME_HTTP_PROXY_PORT -> httpProxyPort =
              (propValue != null) ? Integer.parseInt(propValue) : VALUE_HTTP_PROXY_PORT;

            // http - proxy protocol
          case NAME_HTTP_PROXY_PROTOCOL -> httpProxyProtocol =
              (propValue != null) ? propValue : VALUE_HTTP_PROXY_PROTOCOL;
          ...
          case NAME_ISSUER_ENABLE_SELF_VERIFY -> issuerEnableSelfVerify =
          ...
          case NAME_LOG_BACKUP_N_DAYS_AGO -> logBackupNDaysAgo =
              (propValue != null) ? Integer.parseInt(propValue) : VALUE_LOG_BACKUP_N_DAYS_AGO;

            // log backup - single query size
          case NAME_LOG_BACKUP_SINGLE_QUERY_SIZE -> logBackupSingleQuerySize =
              (propValue != null)
                  ? Integer.parseInt(propValue)
                  : VALUE_LOG_BACKUP_SINGLE_QUERY_SIZE;
          ...
          case NAME_ACCESS_TOKEN_FRONTEND_REGISTER_DID -> accessTokenFrontendRegisterDid =
              propValue;
    ...
    try {
      ...
      for (Setting setting : settingList) {
        ...
        switch (propName) {
            ...
            // http - max connect timeout
          case NAME_HTTP_MAX_CONNECT_TIMEOUT -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getHttpMaxConnectTimeout()), propValue)));

            // http - max retry time
          case NAME_HTTP_MAX_RETRY_TIMES -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getHttpMaxRetryTimes()), propValue)));

            // http - enable log
          case NAME_HTTP_ENABLE_LOG -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(isHttpEnableLog()), propValue)));

            // http - proxy enable
          case NAME_HTTP_PROXY_ENABLE -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(isHttpProxyEnable()), propValue)));

            // http - proxy url
          case NAME_HTTP_PROXY_URL -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(getHttpProxyUrl()), propValue)));

            // http - proxy port
          case NAME_HTTP_PROXY_PORT -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(getHttpProxyPort()), propValue)));

            // http - proxy protocol
          case NAME_HTTP_PROXY_PROTOCOL -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getHttpProxyProtocol()), propValue)));

            // issuer - DID
          case NAME_ISSUER_DID -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(getIssuerDid()), propValue)));

            // issuer - name
          case NAME_ISSUER_NAME -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(getIssuerName()), propValue)));

            // issuer - organization information
          case NAME_ISSUER_ORG_INFO -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(getIssuerOrgInfo()), propValue)));

            // issuer - enable self verify
          case NAME_ISSUER_ENABLE_SELF_VERIFY -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(isIssuerEnableSelfVerify()), propValue)));

            // issuer - key
          case NAME_ISSUER_KEY -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(getIssuerKey()), propValue)));

            // log backup - n days ago
          case NAME_LOG_BACKUP_N_DAYS_AGO -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getLogBackupNDaysAgo()), propValue)));

            // log backup - single query size
          case NAME_LOG_BACKUP_SINGLE_QUERY_SIZE -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getLogBackupSingleQuerySize()), propValue)));

            // schedule - cron expression of status list renew task
          case NAME_SCHEDULE_CRON_SL_RENEW -> settingComparisonMap.put(
              propName,
              new ArrayList<>(
                  List.of(String.valueOf(getScheduleCronStatusListRenew()), propValue)));

            // schedule - cron expression of log backup task
          case NAME_SCHEDULE_CRON_LOG_BACKUP -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getScheduleCronLogBackup()), propValue)));

            // selective disclosure - activate
          case NAME_SD_ACTIVATE -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(isSdActivate()), propValue)));

            // sd - SD-JWT enable decoy
          case NAME_SD_SDJWT_ENABLE_DECOY -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(isSdSdjwtEnableDecoy()), propValue)));

            // url - frontend service generate DID
          case NAME_URL_FRONTEND_GENERATE_DID -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getUrlFrontendGenerateDid()), propValue)));

            // url - frontend service register DID
          case NAME_URL_FRONTEND_REGISTER_DID -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getUrlFrontendRegisterDid()), propValue)));

            // url - frontend service review DID
          case NAME_URL_FRONTEND_REVIEW_DID -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getUrlFrontendReviewDid()), propValue)));

            // url - push service notify app
          case NAME_URL_PUSH_NOTIFY_STATUS_CHANGE -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getUrlPushNotifyStatusChange()), propValue)));

            // url - demo service get holder's data
          case NAME_URL_DEMO_GET_HOLDER_DATA -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getUrlDemoGetHolderData()), propValue)));

            // url - vc service base path
          case NAME_URL_VC_BASE_PATH -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(getUrlVcBasePath()), propValue)));

            // status list - signing key
          case NAME_STATUS_LIST_KEY -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(getStatusListKey()), propValue)));

            // url - frontend service get issuer info DID
          case NAME_URL_FRONTEND_GET_ISSUER_INFO_DID -> settingComparisonMap.put(
              propName,
              new ArrayList<>(
                  List.of(String.valueOf(getUrlFrontendGetIssuerInfoDid()), propValue)));

            // schedule - cron expression of delete expired data task
          case NAME_SCHEDULE_CRON_DELETE_DATA -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getScheduleCronDeleteData()), propValue)));

            // delete data - n minutes ago
          case NAME_DATA_DELETE_N_MINUTES_AGO -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getDataDeleteNMinutesAgo()), propValue)));

            // url - get token
          case NAME_URL_GET_TOKEN -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(getUrlGetToken()), propValue)));

            // status list - max retry times
          case NAME_STATUS_LIST_MAX_RETRY_TIMES -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getStatusListMaxRetryTimes()), propValue)));

            // status list - delay
          case NAME_STATUS_DELAY -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(getStatusListDelay()), propValue)));

            // url - vp validation
          case NAME_URL_VP_VALIDATION -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(getUrlVpValidation()), propValue)));

            // url - oidvci qrcode
          case NAME_URL_OIDVCI_QRCODE -> settingComparisonMap.put(
              propName, new ArrayList<>(List.of(String.valueOf(getUrlOidvciQrcode()), propValue)));

            // access token - frontend service register DID
          case NAME_ACCESS_TOKEN_FRONTEND_REGISTER_DID -> settingComparisonMap.put(
              propName,
              new ArrayList<>(
                  List.of(String.valueOf(getAccessTokenFrontendRegisterDid()), propValue)));

            // url - frontend service create DID
          case NAME_URL_FRONTEND_CREATE_DID -> settingComparisonMap.put(
              propName,
              new ArrayList<>(List.of(String.valueOf(getUrlFrontendCreateDid()), propValue)));
        }
      }
