...
public class SettingService {
  ...
  public Tuple.Pair<String, HttpStatus> checkSetting() {
    ...
    try {
      Map<String, List<String>> settingComparisonMap =
          preloadSetting.getPreloadSettingValueAndDBValue();
      if (settingComparisonMap == null || settingComparisonMap.isEmpty()) {
        throw new VcException(
            VcException.ERR_SYS_GET_PRELOAD_SETTING_ERROR,
            "fail to get preload setting value and db value");
      }
      ...
      VcException vcException =
          new VcException(VcException.ERR_SYS_CHECK_SETTING_ERROR, "fail to check setting value");
  ...
  public Tuple.Pair<String, HttpStatus> updateSetting(
      SettingUpdateRequestDTO settingUpdateRequest) {
    ...
    try {
      ...
      if (settingUpdateRequest == null || !settingUpdateRequest.validate()) {
        throw new VcException(
            VcException.ERR_SETTING_INVALID_UPDATE_SETTING_REQUEST,
            "invalid update setting request");
      }
      ...
      LOGGER.error(e.getMessage(), e);
      VcException vcException =
          new VcException(VcException.ERR_SYS_UPDATE_SETTING_ERROR, "fail to update setting value");
      result = new ErrorResponseDTO(vcException).toString();
      httpStatus = vcException.toHttpStatus();
    }
