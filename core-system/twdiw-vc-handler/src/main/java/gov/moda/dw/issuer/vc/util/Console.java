...
/**
 * show specific data through console
 *
 * @version 20240902
 */
public class Console {
    ...
    if (bytes == null) {
      ...
      for (int i = 0; i < bytes.length; i++) {
        ...
        result =
            result.concat(
                (""
                        + "0123456789ABCDEF".charAt(0x0F & bytes[i] >> 4)
                        + "0123456789ABCDEF".charAt(bytes[i] & 0x0F))
                    + " ");
   ...
   * @param <T> type of element in list
   */
  public static <T extends Number> void showListByHex(
      List<T> inputList, String title, int unitLength) {
