...
public class StatusListPrepareTask {
    ...
    try {
      ...
      if (slType == null) {
        throw new VcException(
            VcException.ERR_SL_INPUT_STATUS_LIST_TYPE_ERROR, "invalid statusList type");
      }
      claims.put(NAME_STATUS_PURPOSE, slType.name());
      claims.put(NAME_ENCODED_LIST, encodedList);

      CredentialSubject credentialSubject =
          CredentialSubject.builder().id(statusListSubjectId).claims(claims).build();
      ...
      LocalDateTime expirationTime =
          DateUtils.calculate(issuanceTime, duration.getUnit(), duration.getValue());
      ...
      VerifiableCredential vc =
          VerifiableCredential.builder()
              ...
              // credential id, ex:
              // https://digitalwallet.moda:8443/vc/api/status-list/VirtualCardCredential/r123
              .id(statusListId)
      ...
      LOGGER.error(e.getMessage(), e);
      throw new VcException(
          VcException.ERR_SL_PREPARE_STATUS_LIST_ERROR, "fail to prepare status list");
    }
  }
  ...
  private String generateEncodedList(
      String latestStatusList, int statusListIndex, VcStatus vcStatus) {
    ...
    try {
      if (latestStatusList == null) {
        ...
        JwtVerifiableCredential jwtVc =
            JwtVerifiableCredential.fromCompactSerialization(latestStatusList);
