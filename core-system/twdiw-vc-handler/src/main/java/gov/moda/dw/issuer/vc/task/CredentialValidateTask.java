...
public class CredentialValidateTask {
  ...
  public Result start() throws VcException {
    ...
    try {
      ...
      JwtVerifiableCredential jwtVc =
          JwtVerifiableCredential.fromCompactSerialization(credentialJwt);
  ...
  private boolean validateJwtProof(ByteVerifier verifier, JwtVerifiableCredential jwtVc)
      throws Exception {
  ...
  private boolean validateJwtContent(String credentialType, JwtVerifiableCredential jwtVc) {
    ...
    if (credentialTypeList.contains(credentialType)
        && credentialTypeList.contains(Definition.DEFAULT_VERIFIABLE_CREDENTIAL_TYPE)) {
    ...
    List<Map<String, Object>> statusLists =
        (List<Map<String, Object>>) vc.getJsonObject().get("credentialStatus");
