...
public class IssuerDidService {
    ...
    try {
      ...
      if (issuerDidRequest == null || !issuerDidRequest.validate()) {
        throw new VcException(
            VcException.ERR_DID_REGISTER_DID_REQUEST, "invalid DID registration request");
      }

      Map<String, Object> org = issuerDidRequest.getOrg();
      // if p7data is null or blank, let p7data be ""
      String p7data =
          Optional.ofNullable(issuerDidRequest.getP7data()).filter(p7 -> !p7.isBlank()).orElse("");
      ...
      String accessToken =
          settingRepository.queryByPropName(PreloadSetting.NAME_ACCESS_TOKEN_FRONTEND_REGISTER_DID);
      if (accessToken == null || accessToken.isBlank()) {
        throw new VcException(
            VcException.ERR_SYS_NOT_SET_FRONTEND_ACCESS_TOKEN_YET_ERROR,
            "issuer has not yet set access token for frontend");
      }
      ...
      DidGenerateResponseDTO didGenerateResponseDTO =
          callFrontendToGenerateDid(didGenerateRequestDTO, accessToken);
      ...
      Map<String, Object> didDocument =
          Optional.ofNullable(data).map(DidGenerateResponseDTO.Data::getDid).orElse(null);
      ...
      // step 3.2: create DID
      DidCreateRequestDTO didCreateRequestDTO =
          new DidCreateRequestDTO(signedData, orgInfo, Definition.FRONTEND_ORG_TYPE, p7data);
      Console.showJson(didCreateRequestDTO.toString(), "DID create request");
      DidCreateResponseDTO didCreateResponseDTO =
          callFrontendToCreateDid(didCreateRequestDTO, accessToken);
      ...
      // JsonUtils.jsToMap(preloadSetting.getIssuerOrgInfo());
      //
      //            // step 3.2: register DID
      //            DidRegisterRequestDTO didRegisterRequestDTO = new
      // DidRegisterRequestDTO(signedData, orgInfo, Definition.FRONTEND_ORG_TYPE);
      //            Console.showJson(didRegisterRequestDTO.toString(), "DID registration request");
      //            DidRegisterResponseDTO didRegisterResponseDTO =
      // callFrontendToRegisterDid(didRegisterRequestDTO, accessToken);
      //            Console.showJson(didRegisterResponseDTO.toString(), "DID registration
      // response");
      //
      //            // check content
      ...
      //                throw new VcException(VcException.ERR_DID_FRONTEND_REGISTER_DID_ERROR,
      // errorMessage);
      //            }
      //
      //            // extract did information
      ...
      //            DidReviewResponseDTO didReviewResponseDTO =
      // callFrontendToReviewDid(didReviewReqeustDTO, accessToken);
      ...
      //                throw new VcException(VcException.ERR_DID_FRONTEND_REVIEW_DID_ERROR,
      // errorMessage);
      //            }
      ...
      int count =
          settingRepository.updateTwoPropValuesByPropName(
      ...
      VcException vcException =
          new VcException(VcException.ERR_DID_REGISTER_DID_ERROR, "fail to register DID");
   ...
   */
  private DidGenerateResponseDTO callFrontendToGenerateDid(
      DidGenerateRequestDTO request, String accessToken) throws VcException {
    ...
    try {
      ...
      String errorMessage =
          "fail to generate DID. Because access token contains invalid characters";
      throw new VcException(VcException.ERR_DID_FRONTEND_GENERATE_DID_ERROR, errorMessage);
    }
    ...
    if (statusCode != HttpStatus.OK.value() || content == null || content.isBlank()) {
      String errorMessage =
          "fail to generate DID. (http status code, response content) = ("
              + statusCode
              + ", "
              + content
              + ")";
   ...
   */
  private DidRegisterResponseDTO callFrontendToRegisterDid(
      DidRegisterRequestDTO request, String accessToken) throws VcException {
    ...
    if (statusCode != HttpStatus.OK.value() || content == null || content.isBlank()) {
      String errorMessage =
          "fail to register DID. (http status code, response content) = ("
              + statusCode
              + ", "
              + content
              + ")";
   ...
   */
  private DidReviewResponseDTO callFrontendToReviewDid(
      DidReviewReqeustDTO request, String accessToken) throws VcException {
    ...
    if (statusCode != HttpStatus.OK.value() || content == null || content.isBlank()) {
      String errorMessage =
          "fail to review DID. (http status code, response content) = ("
              + statusCode
              + ", "
              + content
              + ")";
  ...
  private String signDidDocument(ECKey ecKey, Map<String, Object> didDocument) throws VcException {
    ...
    try {
      ...
      return signJwt(byteSigner, jwtClaimsSet, null, publicKeyJwk, kid);

    } catch (Exception e) {
      LOGGER.error(e.getMessage(), e);
      throw new VcException(
          VcException.ERR_DID_SIGN_JWT_ERROR, "fail to sign did jwt (for DID registration)");
    }
  }
   ...
   * @return identifier
   */
  private String extractIdentifierFromDidDocument(Map<String, Object> didDocument)
      throws VcException {

    if (!didDocument.containsKey("id")) {
      throw new VcException(
          VcException.ERR_DID_PARSE_DID_FROM_DOCUMENT_ERROR, "`id` not found in DID document");
    }
   ...
   */
  private DidCreateResponseDTO callFrontendToCreateDid(
      DidCreateRequestDTO request, String accessToken) throws VcException {
    ...
    if (statusCode != HttpStatus.OK.value() || content == null || content.isBlank()) {
      String errorMessage =
          "fail to create DID. (http status code, response content) = ("
              + statusCode
              + ", "
              + content
              + ")";
