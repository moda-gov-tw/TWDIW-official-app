...
import gov.moda.dw.issuer.vc.repository.custom.CustomExtendedUserRepository;
import gov.moda.dw.issuer.vc.security.jwt.JwtUserObject;
...
public class AmsAuthenticationProvider extends DaoAuthenticationProvider {
  ...
  private ExtendedUser getExtendedUser(String username) {
    return customExtendedUserRepository
        .findOneByUserId(username)
        .orElseThrow(
            () -> {
              log.error(
                  "AmsAuthenticationProvider-authenticate 找不到該 user 在 extendedUser 的資料. username={}",
                  username);
