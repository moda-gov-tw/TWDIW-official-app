...
public class AccessTokenFilter extends GenericFilterBean {
  ...
  public void doFilter(
    ...
    if (!isTokenExisted) {
      ...
      log.debug(
          "AccessTokenFilter-doFilter，取得此次請求中的AccessToken，請求IP位置:{}，accessToken(masked):{}",
          sourceIp,
          maskToken(accessToken));

      // 驗證AccessToken有效性。
      String resMsg = "";
      //            StatusCode accessTokenValidated =
      // this.accessTokenProvider.validateToken(accessToken);

      //            if (StatusCode.SUCCESS.getCode().equals(accessTokenValidated.getCode())) {
      //                Authentication authentication =
      // this.accessTokenProvider.getAuthenticationByAccessToken(accessToken, sourceIp);
      //                SecurityContextHolder.getContext().setAuthentication(authentication);
      //
      //                servletRequest.setAttribute("FROM", ((AccessTokenUserObject)
      // authentication.getPrincipal()).getOrgId());
