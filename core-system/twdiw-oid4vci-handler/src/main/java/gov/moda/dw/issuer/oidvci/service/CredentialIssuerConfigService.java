...
public class CredentialIssuerConfigService {
  ...
  public JSONArray getProofSigningAlgValuesSupported(
      String credential_issuer_meta, String credential_configuration_id) throws ParseException {

    // 驗證參數
    if (credential_issuer_meta == null || credential_issuer_meta.isEmpty()) {
      throw new IllegalArgumentException(
          buildErrorMessage("credential_issuer_meta cannot be null or empty"));
    }
    if (credential_configuration_id == null || credential_configuration_id.isEmpty()) {
      throw new IllegalArgumentException(
          buildErrorMessage("credential_configuration_id cannot be null or empty"));
    }

    // 解析 JSON 字符串
    JSONObject credential_issuer_meta_json;
    try {
      credential_issuer_meta_json =
          (JSONObject)
              new JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE).parse(credential_issuer_meta);
    } catch (ParseException e) {
      throw new IllegalArgumentException(
          buildErrorMessage("credential_issuer_meta is not a valid JSON string"));
    }
    ...
    if (!credential_configurations_supported_json.containsKey(credential_configuration_id)) {
      ...
      errorResponse.put(
          ...
          "credential_configuration_id '"
              + credential_configuration_id
              + "' not found in 'credential_configurations_supported'");
    ...
    JSONObject vc_type_json =
        (JSONObject) credential_configurations_supported_json.get(credential_configuration_id);

    // 檢查 proof_types_supported 是否存在
    if (!vc_type_json.containsKey("proof_types_supported")) {
      JSONObject errorResponse = new JSONObject();
      errorResponse.put(
          "resp_message", "Missing 'proof_types_supported' in credential configuration");
    ...
    if (!jwt_json.containsKey("proof_signing_alg_values_supported")) {
      JSONObject errorResponse = new JSONObject();
      errorResponse.put(
          "resp_message", "Missing 'proof_signing_alg_values_supported' in JWT configuration");
  ...
  public CredentialIssuerConfigEntity updateMetaData(String org_id, String metadata) {
    // 使用 ID 查找要更新的 User
    CredentialIssuerConfigEntity credentialIssuerConfigEntity =
        cic_repository.findById(org_id).orElseThrow(() -> new RuntimeException("User not found"));
