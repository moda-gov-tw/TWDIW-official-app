...
public class CredentialIssuer {
  ...
  public CredentialIssuer(
      CredentialIssuerConfigService credential_issuer_config_service,
      PreAuthCodeService pre_auth_code_service,
      AccessTokenService access_token_service,
      CredentialPolicyService credential_policy_service,
      HTTPService http_service,
      OidvciSettingService oidvci_setting_service,
  ...
  private JSONArray getCredentialConfigurationIds(String credential_offer) throws ParseException {
    JSONObject credential_offer_json =
        (JSONObject) new JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE).parse(credential_offer);
  ...
  public ResponseEntity<String> GetQRCode(
    ...
    try {
      ...
      log.debug("[{}]: request keys: {}", api, req_data.keySet());
      ...
      try {
        if (req_data.containsKey("authenticated"))
          ...
          return ResponseEntity.badRequest()
              ...
              .body(
                  makeReturnObj(api, error_code, extra_error_msg)
                      .toJSONString(JSONStyle.LT_COMPRESS));
        }

        if (req_data.containsKey("id_token")) id_token = req_data.getAsString("id_token");
        else {
        ...
        return ResponseEntity.badRequest()
            .contentType(MediaType.APPLICATION_JSON)
            .body(makeReturnObj(api, error_code).toJSONString(JSONStyle.LT_COMPRESS));
      }
      ...
      if (authenticated) {
        ...
        } else // 收到id_token，解析並產製pre-auth code
        {
          ...
          String tx_code = null;
          JSONObject IDT_payload_obj =
              (JSONObject)
                  new JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE)
                      .parse(Base64.getDecoder().decode(IDT_payload));

          try {
            if (IDT_payload_obj.containsKey("sub")) uid = IDT_payload_obj.getAsString("sub");

            if (IDT_payload_obj.containsKey("aud")) client_id = IDT_payload_obj.getAsString("aud");

            if (IDT_payload_obj.containsKey("exp"))
              exp = Integer.toUnsignedLong((int) IDT_payload_obj.get("exp")) * 1000;

            if (IDT_payload_obj.containsKey("nonce")) nonce = IDT_payload_obj.getAsString("nonce");

            if (IDT_payload_obj.containsKey("credential_configuration_id"))
              credential_configuration_id =
                  IDT_payload_obj.getAsString("credential_configuration_id");
          } catch (ClassCastException e1) {
            error_code = 11003;
          ...
          List<CredentialPolicyEntity> credential_policy_entity =
              credential_policy_service.getCredentialPolicyByType(credential_configuration_id);
          if (credential_policy_entity == null || credential_policy_entity.isEmpty()) {
            ...
            return ResponseEntity.badRequest()
                .contentType(MediaType.APPLICATION_JSON)
                .body(
                    makeReturnObj(api, error_code, errorMessage)
                        .toJSONString(JSONStyle.LT_COMPRESS));
          }
          ...
          if (switch_str_Qrcode != null && !switch_str_Qrcode.isEmpty()) {
            try {
              ...
              return ResponseEntity.badRequest()
                  .contentType(MediaType.APPLICATION_JSON)
                  .body(
                      makeReturnObj(api, errorCode, errorMsg).toJSONString(JSONStyle.LT_COMPRESS));
            }
          }
          ...
          //						error_code = 11004;
          //						return
          // ResponseEntity.badRequest().contentType(MediaType.APPLICATION_JSON).body(makeReturnObj(api, error_code).toJSONString(JSONStyle.LT_COMPRESS));
          //		    		}

          // Get DB config
          List<CredentialIssuerConfigEntity> credential_issuer_config_entity =
              credential_issuer_config_service.getCredentialIssuerConfig(id);

          if (credential_issuer_config_entity == null
              || credential_issuer_config_entity.isEmpty()) {
            error_code = 11901;
          ...
          String credential_issuer_identifier =
              credential_issuer_config_entity.get(0).getCredentialIssuerIdentifier();
          ...
          String credential_offer_uri_path =
              credential_issuer_config_entity.get(0).getCredentialOfferUriPath();
          ...
          // Check if credential_configuration_id is in Credential Offer's
          // credential_configuration_ids
          JSONObject credential_offer_json =
              (JSONObject)
                  new JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE).parse(credential_offer_str);
          log.info(
              "[{}]: credential_offer: {}",
              api,
              credential_offer_json.toJSONString(JSONStyle.LT_COMPRESS));
          JSONArray credential_configuration_ids_array =
              (JSONArray) credential_offer_json.get("credential_configuration_ids");

          // IDT中的credential_configuration_id沒有在Credential Offer 的
          // credential_configuration_ids中，代表不支援此種VC
          ...
          if (cidObj instanceof List<?>) {
            ...
            if (list.isEmpty()) {
              ...
              for (Object obj : list) {
                ...
                try {
                  List<CredentialEntity> credential_entity =
                      credential_service.getCredentialCID(obj_str);
          ...
          log.info(
              "[{}]: Full Response: {}", api, responseJson.toJSONString(JSONStyle.LT_COMPRESS));
          ...
          boolean generated =
              pre_auth_code_service.genPreAuthCode(
                  uid_enc,
                  nonce,
                  client_id,
                  create_time,
          ...
          String qr_code = null;
          String credential_offer_uri =
              credential_issuer_identifier
                  + credential_offer_uri_path
                  + "?nonce="
                  + nonce
                  + "&sub="
                  + uid_enc;
          String link =
              app_url_scheme
                  ...
                  + URLEncoder.encode(credential_offer_uri, "UTF-8");
          ...
          return ResponseEntity.ok()
              .contentType(MediaType.APPLICATION_JSON)
              .body(ret_json.toJSONString(JSONStyle.LT_COMPRESS));
        }
      } else {
        error_code = 11002;
  ...
  public ResponseEntity<String> GetCredentialOffer(
    ...
    try {
      ...
      JSONObject pre_auth_code_json =
          (JSONObject) grants_json.get("urn:ietf:params:oauth:grant-type:pre-authorized_code");
      ...
      if (cidsNode.isArray()) {
        for (JsonNode cid : cidsNode) {
          ...
          if (retBody == null || retBody.isEmpty()) {
            ...
            String errorMessage =
                "HTTP request to " + cleanUrl + " returned null or empty response";
            log.error("[{}]: {}", api, errorMessage);
            return ResponseEntity.badRequest()
                .contentType(MediaType.APPLICATION_JSON)
                .body(
          ...
          try {
            ...
            String credentialStatus =
                innerBody.has("credentialStatus")
                    ? innerBody.get("credentialStatus").asText()
                    : null;

            if (credentialStatus == null || !"REVOKED".equalsIgnoreCase(credentialStatus)) {
              error_code = 12038; // 現有錯誤碼：憑證狀態無效
              String errorMessage =
                  "Revocation response invalid or status not REVOKED: "
                      + (credentialStatus != null ? credentialStatus : "null");
              log.error("[{}]: {}", api, errorMessage);
              return ResponseEntity.badRequest()
                  .contentType(MediaType.APPLICATION_JSON)
                  .body(
      ...
      String credential_configuration_id =
          pre_auth_code_entity.get().getCredentialConfigurationId();
      if (credential_configuration_id == null || credential_configuration_id.isEmpty()) {
        error_code = 12011;
      ...
      if (switch_str_credential_offer != null && !switch_str_credential_offer.isEmpty()) {
        try {
          JsonObject switchObj =
              JsonParser.parseString(switch_str_credential_offer).getAsJsonObject();
          if (switchObj.has("enable_tx_code")) {
      ...
      if (OTP_Switch_credential_offer) {
        List<OidvciSettingEntity> oidvci_setting_entity =
            oidvci_setting_service.getOidvciSetting("tx_code_credential_offer");
        if (oidvci_setting_entity == null || oidvci_setting_entity.isEmpty()) {
          ...
          return ResponseEntity.badRequest()
              .contentType(MediaType.APPLICATION_JSON)
              .body(
                  makeReturnObj(api, error_code, errorMessage).toJSONString(JSONStyle.LT_COMPRESS));
        }
        ...
        JSONObject credential_offer_txcode_json =
            (JSONObject)
                new JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE).parse(credential_offer_txcode);
      ...
      synchronized (this) {
        ...
        String code_status =
            pre_auth_code_service.checkPreAuthCodeValidity(pre_auth_code_entity.get());
      ...
      return ResponseEntity.ok()
          .contentType(MediaType.APPLICATION_JSON)
          .body(credential_offer_json.toJSONString(JSONStyle.LT_COMPRESS));
    } catch (Exception e) {
      log.error("[{}]: {}", api, e.getMessage());
      e.printStackTrace();
      error_code = 11500;
  ...
  public ResponseEntity<String> GetCredentialIssuerMetadata(@PathVariable String id) {
    ...
    try {
      // ObjectMapper objectMapper = new ObjectMapper();
      // String json_string = objectMapper.writeValueAsString(request_map);
      // req_data = (JSONObject)new
      // JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE).parse(json_string);
      ...
      String credential_issuer_meta =
          credential_issuer_config_entity.get(0).getCredentialIssuerMeta();
  ...
  public ResponseEntity<String> GetAccessToken(
    ...
    log.info(
        "[{}]: request data {grant_type={}, client_id={}, pre_authorized_code={}, authorization_details={},tx_code={}",
        api,
        grant_type,
        client_id,
        pre_auth_code,
    ...
    try {
      ...
      pre_auth_code_entity =
          pre_auth_code_service.getPreAuthCodeByClientIDAndPreAuthCode(client_id, pre_auth_code);
      if (pre_auth_code_entity.isEmpty()) {
        error_code = 11007;
      ...
      try {
        authorization_details_json =
            (JSONArray)
                new JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE)
                    .parse(URLDecoder.decode(authorization_details, "UTF-8"));
      } catch (Exception e1) {
        error_code = 11001;
        error_value = "invalid_request";
      ...
      if (OTP_Switch_Token) {
        ...
        if (tx_code == null
            || tx_code.isEmpty()
            || DB_tx_code == null
            || !DB_tx_code.equals(tx_code)) {
          int errorCode = 11032;
          String errorMsg =
              "tx_code is required and must match DB_tx_code when enable_tx_code is true";
      ...
      while (iterator.hasNext()) {
        ...
        String details_credential_configuration_id =
            authorization_json.getAsString("credential_configuration_id");
        ...
        if (!details_credential_configuration_id.equals(credential_configuration_id)) {
          ...
          log.error(
              "[{}]: {}",
              api,
              "credential_configuration_id not equal details_credential_configuration_id");
          return ResponseEntity.badRequest()
              .contentType(MediaType.APPLICATION_JSON)
              .body(ret_json.toJSONString(JSONStyle.LT_COMPRESS));
        }
      ...
      resp_authorization_details_field_json.put(
      ...
      boolean generated =
          access_token_service.genAccessToken(
              ...
              c_nonce_create_time,
              c_nonce_expire_time,
              resp_authorization_details_json.toJSONString(JSONStyle.LT_COMPRESS),
              pre_auth_code,
              CODE_VALID,
              "");
      ...
      return ResponseEntity.ok()
          .contentType(MediaType.APPLICATION_JSON)
          .body(token_response.toJSONString(JSONStyle.LT_COMPRESS));
    } catch (Exception e) {
      log.error("[{}]: {}", api, e.getMessage());
      e.printStackTrace();
      error_code = 11500;
  ...
  public ResponseEntity<String> GetCredential(
      @RequestHeader(value = "Authorization", required = false) String auth_header,
      @RequestBody Map<String, Object> request_map,
      @PathVariable String id) {
    ...
    try {
      ...
      // Read DB config
      credential_issuer_config_entity =
          credential_issuer_config_service.getCredentialIssuerConfig(id);
      ...
      if (access_token.isEmpty()) {
        ...
        return ResponseEntity.status(401)
            .header("WWW-Authenticate", error_header_content)
            .contentType(MediaType.APPLICATION_JSON)
            .body(makeReturnObj(api, error_code).toJSONString(JSONStyle.LT_COMPRESS));
      }
      ...
      // 檢查access_token有效性
      String token_status =
          access_token_service.checkAccessTokenValidity(access_token_entity.get());
      ...
      JSONArray authorization_details_json_db =
          (JSONArray)
              new JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE)
                  .parse(access_token_entity.get().getAuthorizationDetails());
      ...
      try {
        ...
        JSONObject header_obj =
            (JSONObject)
                new JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE)
                    .parse(Base64.getDecoder().decode(header_str));
        ...
        String did = null;
        if (kid_token.length == 3) did = kid_token[2];
        else {
          String updated_cnonce =
              access_token_service.updateCNonceAndTTL(access_token_entity.get(), TOKEN_TTL);
          error_code = 11016;
        ...
        String did_hex_prefix =
            CryptoHelper.convertBinaryToHexString(Base58.decode(did.substring(1)));
        ...
        holder_pub_key =
            (JSONObject) new JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE).parse(did_jwk);
        ...
        JSONArray proof_signing_alg_values =
            credential_issuer_config_service.getProofSigningAlgValuesSupported(
                credential_issuer_meta, credential_identifier);
        ...
        if (proof_signing_alg_values.get(0) instanceof JSONObject
          ...
          return ResponseEntity.badRequest()
              .contentType(MediaType.APPLICATION_JSON)
              .body(
                  ((JSONObject) proof_signing_alg_values.get(0))
                      .toJSONString(JSONStyle.LT_COMPRESS));
        }
        ...
        JSONObject payload_obj =
            (JSONObject)
                new JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE)
                    .parse(Base64.getDecoder().decode(payload_str));
      ...
      request.put(
          "holderUid", crypto_util.GCMDecrypt(access_token_entity.get().getUserID(), db_secret));
      ...
      String ret_body =
          http_service.sendPostRequest(url, request.toJSONString(JSONStyle.LT_COMPRESS));
      JSONObject ret_body_json =
          (JSONObject) new JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE).parse(ret_body);
      int resp_code = (int) ret_body_json.get("resp_code");
      log.debug("[{}]: credential API response code: {}", api, resp_code);
      if (resp_code == 10000) {
        ...
        JSONObject resp_credential_json =
            (JSONObject) new JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE).parse(resp_credential);
        log.debug("[{}]: credential issued successfully", api);
        return ResponseEntity.ok()
            .contentType(MediaType.APPLICATION_JSON)
            .body(resp_credential_json.toJSONString(JSONStyle.LT_COMPRESS));
      } else {
        error_code = resp_code;
        log.error(
            "[{}]: Failed to process request. Response code: {}, Error code: {}, Original response: {}",
            api,
            resp_code,
            error_code,
            ret_body_json);
