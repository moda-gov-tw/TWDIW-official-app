...
/**
 * AttributeEncryptor class used to encrypt and decrypt entity attributes for storing securely in
 * the database.
 *
 * <p>This class implements the AttributeConverter interface to provide custom conversion logic for
 * entity attributes.
 *
 * <p>It uses AES encryption with a secret key.
 *
 * <p>AttributeEncryptor 類用於加密和解密實體屬性，以便安全地存儲在數據庫中。
 *
 * <p>此類實現 AttributeConverter 接口以提供實體屬性的自定義轉換邏輯。
 *
 * <p>使用帶有密鑰的 AES 加密。
 *
 * @author Alex Chang
...
public class AttributeEncryptor implements AttributeConverter<String, String> {
  ...
  public AttributeEncryptor(
      @Value("${applications.config.attributeencryptor_secret}") String secret) throws Exception {
    if (StringUtils.isBlank(secret)) {
      throw new IllegalStateException(
          "applications.config.attributeencryptor_secret is required and must be 256-bit Base64");
    }
    byte[] secretBytes;
    try {
      ...
      throw new IllegalStateException(
          "applications.config.attributeencryptor_secret must be valid Base64", ex);
    }
    if (secretBytes.length != SECRET_LENGTH_BYTES) {
      throw new IllegalStateException(
          "applications.config.attributeencryptor_secret must be 256-bit (32 bytes) after Base64 decoding");
    }
