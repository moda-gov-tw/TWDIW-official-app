...
public class AccessTokenProvider {
    ...
    public static final String INNER_ACCESS_TOKEN = "12345678";
    ...
    public StatusCode validateToken(String accessToken) {
        ...
        if (accessTokenDTO == null) {
            log.error("AccessTokenProvider-validateToken，AccessToken不存在，accessToken:{}", accessToken);
            return StatusCode.TOKEN_NOT_EXISTS_FOR_FILTER;
        }
        if (!AC_TYPE_UNLIMITED.equals(accessTokenDTO.getActype())) {
            log.error("AccessTokenProvider-validateToken，AccessToken限期異常，accessTokenDTO:{}", accessTokenDTO);
            return StatusCode.TOKEN_AC_TYPE_EXCEPTION;
        }
        if (StateType.DISABLED.getEnName().equals(accessTokenDTO.getState())) {
            log.error("AccessTokenProvider-validateToken，AccessToken未啟用，accessTokenDTO:{}", accessTokenDTO);
