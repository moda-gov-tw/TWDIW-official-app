...
public class ResResource {
  ...
  public ResResource(
      ResService resService, ResRepository resRepository, ResQueryService resQueryService) {
   ...
   *     resDTO, or with status {@code 400 (Bad Request)} if the res has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  //  @PostMapping("")
  //  public ResponseEntity<ResDTO> createRes(@Valid @RequestBody ResDTO resDTO) throws
  // URISyntaxException {
  //    log.debug("REST request to save Res : {}", resDTO);
  //    if (resDTO.getId() != null) {
  //      throw new BadRequestAlertException("A new res cannot already have an ID", ENTITY_NAME,
  // "idexists");
  //    }
  //    resDTO = resService.save(resDTO);
  //    return ResponseEntity.created(new URI("/api/res/" + resDTO.getId()))
  //      .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME,
  // resDTO.getId().toString()))
  //      .body(resDTO);
  //  }
   ...
   *     resDTO, or with status {@code 400 (Bad Request)} if the resDTO is not valid, or with status
   *     {@code 500 (Internal Server Error)} if the resDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  //  @PutMapping("/{id}")
  //  public ResponseEntity<ResDTO> updateRes(@PathVariable(value = "id", required = false) final
  // Long id, @Valid @RequestBody ResDTO resDTO)
  ...
  /**
   * {@code PATCH /res/:id} : Partial updates given fields of an existing res, field will ignore if
   * it is null
   *
   * @param id the id of the resDTO to save.
   * @param resDTO the resDTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     resDTO, or with status {@code 400 (Bad Request)} if the resDTO is not valid, or with status
   *     {@code 404 (Not Found)} if the resDTO is not found, or with status {@code 500 (Internal
   *     Server Error)} if the resDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
   ...
   */
  //  @GetMapping("")
  //  public ResponseEntity<List<ResDTO>> getAllRes(ResCriteria criteria,
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the resDTO, or
