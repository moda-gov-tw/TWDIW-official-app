...
public class RelResource {
  ...
  public RelResource(
      RelService relService, RelRepository relRepository, RelQueryService relQueryService) {
   ...
   *     relDTO, or with status {@code 400 (Bad Request)} if the rel has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  //  @PostMapping("")
  //  public ResponseEntity<RelDTO> createRel(@Valid @RequestBody RelDTO relDTO) throws
  // URISyntaxException {
  //    log.debug("REST request to save Rel : {}", relDTO);
  //    if (relDTO.getId() != null) {
  //      throw new BadRequestAlertException("A new rel cannot already have an ID", ENTITY_NAME,
  // "idexists");
  //    }
  //    relDTO = relService.save(relDTO);
  //    return ResponseEntity.created(new URI("/api/rels/" + relDTO.getId()))
  //      .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME,
  // relDTO.getId().toString()))
  //      .body(relDTO);
  //  }
   ...
   *     relDTO, or with status {@code 400 (Bad Request)} if the relDTO is not valid, or with status
   *     {@code 500 (Internal Server Error)} if the relDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  //  @PutMapping("/{id}")
  //  public ResponseEntity<RelDTO> updateRel(@PathVariable(value = "id", required = false) final
  // Long id, @Valid @RequestBody RelDTO relDTO)
  ...
  /**
   * {@code PATCH /rels/:id} : Partial updates given fields of an existing rel, field will ignore if
   * it is null
   *
   * @param id the id of the relDTO to save.
   * @param relDTO the relDTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     relDTO, or with status {@code 400 (Bad Request)} if the relDTO is not valid, or with status
   *     {@code 404 (Not Found)} if the relDTO is not found, or with status {@code 500 (Internal
   *     Server Error)} if the relDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
   ...
   */
  //  @GetMapping("")
  //  public ResponseEntity<List<RelDTO>> getAllRels(RelCriteria criteria,
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the relDTO, or
