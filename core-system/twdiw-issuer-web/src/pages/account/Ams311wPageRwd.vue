              ...
              <q-card>
                <q-card-section>
                  ...
                  <q-select
                    ...
                    :options="[
                      { label: t('enabled'), value: '啟用' },
                      { label: t('disabled'), value: '停用' }
                    ]"
                    outlined
                    hide-bottom-space
                    class="q-mb-sm"
      ...
      <template v-slot:body-cell-state="props">
        <q-td style="width: 100px; min-width: 100px">
          ...
          >
            ...
            >
              <q-tooltip :delay="700"> {{ t("account.resend") }} </q-tooltip>
            </q-btn>
          </div>
          ...
          <div v-else class="row items-center">
            ...
            >
              <q-tooltip :delay="700">
                {{
                  props.value === "0"
                    ? t("account.state.enabled")
                    : t("account.state.disabled")
                }}
              </q-tooltip>
            </q-btn>
          </div>
...
const handleStateToggle = async (row) => {
  ...
  try {
    ...
    const confirmMessage = t("account.accountInfo", {
      action: action,
      input: row.login
    }); // 根據 action 顯示正確的訊息
...
const handleResetBwd = async (row) => {
  ...
  try {
    $d.confirm(
      t("confirm"),
      t("account.resetBwd", { input: row.login }),
      () => {
        resetBwd(row);
      }
    );
...
const handleDeleteAccount = async (row) => {
  ...
  try {
    $d.confirm(
      t("confirm"),
      t("account.deleteAccount", { input: row.login }),
      () => {
        deleteAccount(row);
      }
    );
...
const handleResendActivationEmail = async (row) => {
  ...
  try {
    ...
    $d.confirm(
      t("confirm"),
      t("account.resendInfo", { input: row.login }),
      async () => {
