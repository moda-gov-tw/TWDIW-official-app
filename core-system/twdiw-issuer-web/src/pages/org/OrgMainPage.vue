    ...
    >
      <template v-slot:top>
        <div class="row q-ma-none q-pa-none items-center">
          <!-- 組織管理 -->
          <p class="titleRwd q-mb-md">{{ t("org.title") }}</p>
          <q-space></q-space>
...
const onDeleteOrg = (row) => {
  $d.confirm(
    t("confirm"),
    t("org.deleteMessage", { field: row.orgTwName }),
    async () => {
      const { isDeleteOK } = await deleteOrg(row);
      if (isDeleteOK) {
        $n.success(t("org.success.delete", { field: row.orgTwName }));
      }
    }
  );
};
