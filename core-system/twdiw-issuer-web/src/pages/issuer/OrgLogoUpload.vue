<template>
  <basic-page>
    ...
    <div class="row q-ma-none q-py-sm q-pl-md items-center">
      <!-- 組織 LOGO 上傳 -->
      <p class="titleRwd q-mb-md">{{ t("orgLogo.title") }}</p>
      <div v-if="!issuerDID" :class="['alert-text', isMobile ? '' : 'q-ml-lg']">
        {{ t("orgLogo.notice.firstComplete") }}
      </div>
      <div
        v-else-if="!isDidOrg"
        :class="['alert-text', isMobile ? '' : 'q-ml-lg']"
      >
        {{ t("orgLogo.notice.noUploadPermission") }}
      </div>
      <q-space></q-space>
    </div>
      ...
      >
        ...
        <q-dialog v-model="showMobileNotice">
          <q-card style="min-width: 300px">
            ...
            <q-card-section>
              <p class="text-body2">{{ t("orgLogo.square.logoSize") }}</p>
            </q-card-section>
        ...
        <div class="row justify-end q-mb-xs">
          <q-btn
            :disable="!issuerDID || loading || !isDidOrg"
            unelevated
...
import { useOrgLogoStore } from "stores/orgLogo";
...
const { issuerDID, orgInfo, loading, rectangleSampleLogo } =
...
const getLabel = (name) => {
  switch (name) {
    case "orgId":
      return t("org.table.orgId");
    case "orgCname":
      return t("org.table.orgTwName");
    case "orgEname":
      return t("org.table.orgEnName");
    default:
      return "";
  }
};
...
const fileLabel = computed(() => {
  return formData.logoSquare ? t("orgLogo.logo.true") : t("orgLogo.logo.false");
});
...
const onOKClick = async () => {
  ...
  if (formData.orgId === "default") {
    return $n.error(t("orgLogo.notice.default"));
  }
...
const isDidOrg = computed(() => {
  if (!issuerDID.value) return false;
