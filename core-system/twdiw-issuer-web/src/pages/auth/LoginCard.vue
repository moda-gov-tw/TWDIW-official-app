    ...
    <q-form @submit="onLogin" class="column fit q-pa-lg">
      <div class="q-pa-sm">
        ...
        >
          ...
          >
            {{ t("login.button.operationManual") }}
          </q-btn>
        </div>
      </div>
    </q-form>
...
const valid_phone = [
  (val) => !!val || t("required"),
  (val) => (val || "").length >= 10 || t("login.dialog.valid.lessThan"),
  (val) => (val || "").length <= 20 || t("login.dialog.valid.greaterThan")
];
  ...
  api
    ...
    .catch((error) => {
      ...
      if (error.response && error.response.data && error.response.data.detail) {
        $d.alert(t("login.dialog.confirm"), error.response.data.detail);
      }
    });
};
  ...
  try {
    ...
    if (response.data && response.data.captchaCode === "pass") {
      ...
      if (error.response && error.response.data && error.response.data.detail) {
        ...
        $d.alert(
          t("login.dialog.confirm"),
          t("login.dialog.message.graphicError")
        );
      }
      genResetNonce();
      resetCaptcha.value = "";
    ...
    resetCaptcha.value = "";
    if (detail) {
      $d.alert(t("login.dialog.confirm"), detail);
    } else if (msg) {
...
onMounted(() => {
  if (localStorage.getItem("sessionExpired")) {
    ...
    $d.alert(t("login.error.mistake"), t("login.error.expired"));
  }
  removeToken();
  genNonce();
});
