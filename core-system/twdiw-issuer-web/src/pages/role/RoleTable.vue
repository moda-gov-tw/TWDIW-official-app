  ...
  >
    ...
    <template #top>
      ...
      <div class="row q-ma-none q-pa-none items-center">
        <p class="titleRwd q-mb-md">{{ t("role.title") }}</p>
        <q-space></q-space>
      ...
      <q-td style="width: 100px; min-width: 100px">
        <template v-if="props.value === '0'">
          <div class="row items-center">
            ...
            >
              <q-tooltip :delay="700">{{ t("role.state.enabled") }}</q-tooltip>
            </q-btn>
          </div>
        </template>
        <template v-else-if="props.value === '1'">
          <div class="row items-center">
            ...
            <q-btn
              ...
              "
            >
              <q-tooltip :delay="700">{{ t("role.state.disabled") }}</q-tooltip>
            </q-btn>
          </div>
        </template>
        <span v-else>
...
const onConfirm = (model) => {
  ...
  if (model.value) {
    if (model.value["roleId.contains"]) {
      ...
      searchQuery.value =
        model.value["state.equals"] === 1 ? t("enabled") : t("disabled");
    } else {
...
const disableRole = (row) => {
  ...
  $d.confirm(t("confirm"), confirmMessage(t(action), name), async () => {
    const isUpdateOK = await updateState(row);
    if (isUpdateOK.isUpdateOK) {
      notifySuccess(t(action), name);
      refreshTable();
    }
  });
};
