          ...
          <div class="row items-center">
            ...
            >
              <q-tooltip :delay="700">
                {{
                  props.value === "0"
                    ? t("func.state.enabled")
                    : t("func.state.disabled")
                }}
              </q-tooltip>
            </q-btn>
          </div>
...
const submitSearch = async () => {
  ...
  if (isValid) {
    ...
    if (searchForm.value.resId) {
      ...
      searchForm.value.typeId?.value === "node" ||
      searchForm.value.typeId?.value === "WEB"
    ) {
      searchQuery.value = t(`func.type.${searchForm.value.typeId.value}`);
    } else if (
      searchForm.value.state?.value === "啟用" ||
      searchForm.value.state?.value === "停用"
    ) {
      searchQuery.value = t("func.stateLabel", {
