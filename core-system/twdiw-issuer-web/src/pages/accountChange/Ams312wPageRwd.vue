                  ...
                  <q-card>
                    <q-card-section>
                      ...
                      <q-select
                        ...
                        :options="[
                          {
                            label: t('accountChange.logType.add'),
                            value: 'add'
                          },
                          {
                            label: t('accountChange.logType.modify'),
                            value: 'modify'
                          },
                          {
                            label: t('accountChange.logType.delete'),
                            value: 'delete'
                          }
                        ]"
                        outlined
            ...
            <q-badge
              ...
              :color="
                props.value === 'modify'
                  ...
                  : 'indigo-6'
              "
              :label="t(`accountChange.logType.${props.value}`)"
            />
          </div>
        </q-td>
      </template>
...
const columns = [
  ...
  },
  {
    ...
    format: (val) =>
      val === "default"
        ? t("accountChange.level.default")
        : t("accountChange.level.Ap"),
    sortable: true,
    field: "userTypeId",
    align: "left"
  },
...
const submitSearch = async () => {
  ...
  if (isValid) {
    ...
    if (searchForm.value.userId) {
      ...
      searchQuery.value = t("searchQuery.add");
    } else if (searchForm.value.logType?.value === "modify") {
      searchQuery.value = t("searchQuery.modify");
    } else if (searchForm.value.beginDate && searchForm.value.endDate) {
      searchQuery.value = t("searchQuery.range");
    } else {
