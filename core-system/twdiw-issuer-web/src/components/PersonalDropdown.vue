    ...
    .catch((error) => {
      ...
      if (error.response && error.response.data) {
        ...
        if (detail) {
          // 檢查具體的錯誤信息
          if (detail === "密碼輸入不正確") {
            $d.alert(t("account.confirm"), t("account.error.wrongBwd"));
            ...
            $d.alert(t("account.confirm"), detail);
          }
        } else {
          $d.alert(t("account.error.title"), t("account.error.unknown"));
        }
      } else {
        // 如果沒有 response，則顯示連接錯誤
        $d.alert(t("account.error.title"), t("account.error.noResponse"));
      }
    });
};
