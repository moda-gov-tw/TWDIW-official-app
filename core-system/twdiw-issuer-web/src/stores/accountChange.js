...
export const useAccountChangeStore = defineStore("accountChange", () => {
  //plugin settings
  const { t } = useI18n();
  const $n = useNotify();
  ...
  async function getAccountChanges(
    ...
    const finalOptions = {
      ...
      logType: options.logType || currentFilter.value.logType,
    ...
    const queryParams = new URLSearchParams({
      ...
      actionType: finalOptions.logType || "",
    ...
    try {
      const response = await api.get(
        `/api/modadw312w/search?${queryParams.toString()}`,
        {
          headers: {
      ...
      accountChanges.value = responseData.map((item) => {
        return {
          ...item,
      ...
      if (error.status === HttpStatusCode.Forbidden) {
        $n.error(t("accountChange.error.permissionDenied"));
      } else {
  ...
  function selectAccountChange(accountChange) {
    selected.value = accountChange;
  }
